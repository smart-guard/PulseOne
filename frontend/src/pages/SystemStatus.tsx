...
const SystemStatus: React.FC = () => {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        setServices(response.data.services.details);
        setHierarchy(response.data.hierarchy || []);
        setUnassignedCollectors(response.data.unassigned_collectors || []);
  ...
  const handleServiceControl = async (service: ServiceInfo, action: 'start' | 'stop' | 'restart') => {
    ...
    try {
      ...
      const response = await DashboardApiService.controlService(service.name, action);

      if (response.success) {
        console.log(`${service.displayName} ${action} 완료`);
        await fetchStatus(); // 상태 새로고침
      } else {
        alert(`${service.displayName} ${action} 실패: ${response.message}`);
      }
    } catch (error) {
  ...
  const coreServices = services.filter(s => !s.name.startsWith('collector'));
          ...
          <td style={{ padding: '16px 24px', textAlign: 'right', width: '15%' }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              {!service.controllable ? (
                <span style={{ fontSize: '0.75rem', color: '#cbd5e0', fontStyle: 'italic' }}>SYSTEM LOCKED</span>
              ) : (
                <>
