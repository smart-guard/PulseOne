...
# ============================================================================
# 🚀 MQTT 드라이버 완전 테스트 - 한번에 실행
# ============================================================================

set -e

echo "🧪 PulseOne MQTT Driver Complete Test"
echo "====================================="
...
apt-get update -qq && apt-get install -y mosquitto mosquitto-clients > /dev/null 2>&1
mosquitto -d 2>/dev/null || service mosquitto start 2>/dev/null || true
sleep 2
echo "✅ MQTT broker started"

# 2. 완전한 테스트 코드 생성
echo "📝 Creating complete test code..."

cat > tests/complete_mqtt_test.cpp << 'EOF'
// ============================================================================
...
# 3. 컴파일
echo "🔨 Compiling MQTT test..."
g++ -std=c++17 -Wall -Wextra -g \
    -I./include \
    tests/complete_mqtt_test.cpp \
    src/Drivers/Mqtt/MqttDriver.cpp \
    src/Utils/ConfigManager.cpp \
    src/Utils/LogManager.cpp \
    -lpaho-mqttpp3 -lpaho-mqtt3c \
    -lpq -lhiredis -ljsoncpp \
    -pthread \
    -o tests/complete_mqtt_test

echo "✅ Compilation successful!"

# 4. 테스트 실행
echo ""
echo "🧪 Running Complete MQTT Driver Test..."
echo "======================================="

./tests/complete_mqtt_test

# 5. 결과
test_result=$?
echo ""
if [ $test_result -eq 0 ]; then
    echo "🎉 MQTT Driver Test PASSED!"
    echo "✅ All functionality verified successfully"
else
    echo "❌ MQTT Driver Test FAILED!"
