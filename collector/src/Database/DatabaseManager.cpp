...
bool DatabaseManager::connectPostgres() {
    for (int i = 0; i < MAX_RETRIES; ++i) {
        try {
            ...
            if (pg_conn->is_open()) {
                LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_INFO, "PostgreSQL 연결 성공");
                return true;
            }
        } catch (const std::exception& e) {
            LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_ERROR, std::string("PostgreSQL 연결 실패: ") + e.what());
        }
...
bool DatabaseManager::connectSQLite() {
    ...
    if (rc != SQLITE_OK) {
        LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_ERROR, "SQLite 연결 실패: " + std::string(sqlite3_errmsg(sqlite_conn)));
        return false;
    }
    LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_INFO, "SQLite 연결 성공");
    return true;
}
...
pqxx::result DatabaseManager::executeQueryPostgres(const std::string& query) {
    try {
        ...
        txn.commit();
        LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_INFO, "PostgreSQL 쿼리 성공: " + query);
        return r;
    } catch (const std::exception& e) {
        LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_ERROR, std::string("PostgreSQL 쿼리 실패: ") + e.what());
        throw;
    }
}
...
bool DatabaseManager::executeNonQueryPostgres(const std::string& query) {
    try {
        ...
        txn.exec(query);
        txn.commit();
        LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_INFO, "PostgreSQL non-query 성공: " + query);
        return true;
    } catch (const std::exception& e) {
        LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_ERROR, std::string("PostgreSQL non-query 실패: ") + e.what());
        return false;
    }
}
...
bool DatabaseManager::executeQuerySQLite(const std::string& query,
    ...
    if (rc != SQLITE_OK) {
        LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_ERROR, "SQLite 쿼리 실패: " + std::string(errMsg));
        sqlite3_free(errMsg);
        return false;
    }
    LogManager::getInstance().log(LOG_MODULE_DATABASE, LOG_LEVEL_INFO, "SQLite 쿼리 성공: " + query);
    return true;
}
