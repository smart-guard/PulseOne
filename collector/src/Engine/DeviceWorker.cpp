    ...
    std::thread([this, promise]() {
        try {
            ...
            if (current_state_ == WorkerState::RUNNING) {
                logger_->Warning("Device already running: " + device_info_.device_name);
        ...
        try {
            ...
            if (!points_to_read.empty()) {
                ...
                if (driver_->ReadValues(points_to_read, values)) {
                    ...
                    logger_->Warning("Failed to read data from device: " + device_info_.device_name);
        ...
        try {
            ...
            if (has_request) {
                ...
                if (now > request.timeout) {
                    ...
                    logger_->Warning("Hardware control request timed out");
                    continue;
                }
...
bool DeviceWorker::AttemptReconnection() {
    try {
        ...
        logger_->Warning("Reconnection failed for device: " + device_info_.device_name);
        return false;
