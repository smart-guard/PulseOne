...
bool TcpBasedWorker::ValidateTcpConfig() const {
    ...
    if (connection_timeout_seconds_ <= 0) {
        LogMessage(LogLevel::LOG_ERROR, "Invalid timeout value: " + std::to_string(connection_timeout_seconds_));
        return false;
    }
...
bool TcpBasedWorker::CreateTcpSocket() {
    ...
    if (socket_fd_ < 0) {
        LogMessage(LogLevel::LOG_ERROR, "Failed to create TCP socket: " + std::string(strerror(errno)));
        return false;
    }
    ...
    if (inet_pton(AF_INET, ip_address_.c_str(), &server_addr.sin_addr) <= 0) {
        LogMessage(LogLevel::LOG_ERROR, "Invalid IP address: " + ip_address_);
        CloseTcpSocket();
        return false;
    }
    ...
    if (connect(socket_fd_, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
        LogMessage(LogLevel::LOG_ERROR, "TCP connection failed: " + std::string(strerror(errno)));
        CloseTcpSocket();
        return false;
    }
