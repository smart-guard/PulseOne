...
/**
 * @brief 드라이버 설정 구조체
 * @details JSON과 기존 ConfigManager와 호환되는 설정 관리
 */
struct DriverConfig {
    // 기본 식별 정보
    ...
    int reconnect_delay_ms;                   ///< 재연결 지연 시간
    int max_concurrent_requests;              ///< 최대 동시 요청 수
    
    // 고급 설정
    bool enable_diagnostics;                  ///< 진단 정보 수집 여부
    bool enable_packet_logging;               ///< 패킷 로깅 여부
    bool enable_statistics;                   ///< 통계 수집 여부
    LogLevel log_level;                       ///< 로그 레벨
    
    // 프로토콜별 특수 설정 (JSON 형태)
    std::map<std::string, std::string> protocol_specific_config;
