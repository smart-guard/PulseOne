...
class SqliteAdapter extends BaseAdapter {
    ...
    convertFunctions(query) {
        ...
        // PostgreSQL/MariaDB/MSSQL → SQLite 함수 변환
        // ✅ localtime 유지: datetime('now', 'localtime') → 그대로 (SQLite 로컬시간)
        // ✅ NOW() → datetime('now', 'localtime') (UTC 아닌 로컬시간으로)
        adaptedQuery = adaptedQuery.replace(/NOW\(\)/gi, "datetime('now', 'localtime')");
        adaptedQuery = adaptedQuery.replace(/GETDATE\(\)/gi, "datetime('now', 'localtime')");
        // datetime('now', 'localtime')은 이미 SQLite 호환이므로 그대로 유지
...
class PostgresAdapter extends BaseAdapter {
    ...
    convertFunctions(query) {
        ...
        // ✅ datetime('now') 및 datetime('now', 'localtime') → NOW() 모두 커버
        adaptedQuery = adaptedQuery.replace(/datetime\s*\('now'\s*,\s*'localtime'\)/gi, 'NOW()');
...
class MariadbAdapter extends BaseAdapter {
    ...
    convertFunctions(query) {
        ...
        // ✅ datetime('now', 'localtime') 및 datetime('now') → NOW() 모두 커버
        adaptedQuery = adaptedQuery.replace(/datetime\s*\('now'\s*,\s*'localtime'\)/gi, 'NOW()');
        adaptedQuery = adaptedQuery.replace(/datetime\('now'\)/gi, 'NOW()');
