...
public:
  ...
  ParseErrorString(const std::string &error_str) const {
    static const std::unordered_map<std::string, PulseOne::Enums::ErrorCode>
        stringToEnum = {
            // 연결 관련
            {"HARDWARE_CONNECTION_FAILED",
             PulseOne::Enums::ErrorCode::CONNECTION_FAILED},
            {"HARDWARE_TIMEOUT",
             PulseOne::Enums::ErrorCode::CONNECTION_TIMEOUT},
            {"CONNECTION_REFUSED",
             PulseOne::Enums::ErrorCode::CONNECTION_REFUSED},
            {"CONNECTION_LOST", PulseOne::Enums::ErrorCode::CONNECTION_LOST},

            // Modbus 관련
            {"MODBUS_SLAVE_NO_RESPONSE",
             PulseOne::Enums::ErrorCode::DEVICE_NOT_RESPONDING},
            {"MODBUS_CRC_ERROR", PulseOne::Enums::ErrorCode::CHECKSUM_ERROR},
            {"MODBUS_PROTOCOL_ERROR",
             PulseOne::Enums::ErrorCode::MODBUS_EXCEPTION},

            // MQTT 관련
            {"MQTT_BROKER_UNREACHABLE",
             PulseOne::Enums::ErrorCode::CONNECTION_FAILED},
            {"MQTT_AUTH_FAILED",
             PulseOne::Enums::ErrorCode::AUTHENTICATION_FAILED},
            {"MQTT_CONNECTION_ERROR",
             PulseOne::Enums::ErrorCode::MQTT_PUBLISH_FAILED},
            ...
            // 권한 관련
            {"PERMISSION_DENIED",
             PulseOne::Enums::ErrorCode::INSUFFICIENT_PERMISSION},
  ...
  ErrorDetail GetErrorDetail(PulseOne::Enums::ErrorCode error_code) const {
    ...
    case PulseOne::Enums::ErrorCode::DEVICE_OFFLINE:
      return {http_status,
              ...
              true,
              "디바이스 전원 확인",
              "디바이스 오프라인 - 전원 및 연결 상태 확인"};

    // 데이터 관련
    case PulseOne::Enums::ErrorCode::INVALID_DATA:
      return {http_status, "data", "medium",      "software",
              false,       true,   "입력값 확인", "잘못된 데이터 형식 또는 값"};
    ...
    case PulseOne::Enums::ErrorCode::DATA_OUT_OF_RANGE:
      return {http_status, "data", "medium",       "software",
              false,       true,   "값 범위 확인", "허용 범위를 벗어난 데이터"};
    ...
    case PulseOne::Enums::ErrorCode::MODBUS_EXCEPTION:
      return {http_status,
              ...
              false,
              "Modbus 설정 확인",
              "Modbus 예외 발생 - 슬레이브 ID 또는 레지스터 주소 확인"};
    ...
    case PulseOne::Enums::ErrorCode::CONFIGURATION_ERROR:
      return {http_status,   "configuration",
              "high",        "software",
              false,         true,
              "설정 재검토", "설정 로드 또는 적용 중 에러"};

    case PulseOne::Enums::ErrorCode::INVALID_PARAMETER:
      return {http_status,     "parameter",
              "medium",        "software",
              false,           true,
  ...
  std::string GetErrorCategoryByHttpStatus(int http_status) const {
    if (http_status >= 420 && http_status <= 429)
      return "connection";
    if (http_status >= 430 && http_status <= 449)
      return "protocol";
    if (http_status >= 450 && http_status <= 469)
      return "data";
    if (http_status >= 470 && http_status <= 489)
      return "device";
    if (http_status >= 490 && http_status <= 499)
      return "configuration";
    if (http_status >= 510 && http_status <= 519)
      return "maintenance";
    if (http_status >= 520 && http_status <= 529)
      return "system";
    if (http_status >= 530 && http_status <= 539)
      return "modbus";
    if (http_status >= 540 && http_status <= 549)
      return "mqtt";
    if (http_status >= 550 && http_status <= 559)
      return "bacnet";
    return "unknown";
  }
  ...
  std::string GetUserFriendlyMessage(PulseOne::Enums::ErrorCode error_code,
    ...
    std::string device_info =
        device_id.empty() ? "디바이스" : "디바이스 '" + device_id + "'";
    ...
    case PulseOne::Enums::ErrorCode::BACNET_SERVICE_ERROR:
      return device_info +
             "의 BACnet 서비스에서 에러가 발생했습니다. (코드: 550)";
   ...
   */
  int MapDeviceStatusToHttpStatus(
      PulseOne::Enums::DeviceStatus device_status) const {
   ...
   */
  int MapConnectionStatusToHttpStatus(
      PulseOne::Enums::ConnectionStatus connection_status) const {
  ...
  int DetermineHttpStatus(PulseOne::Enums::DeviceStatus device_status,
    ...
    int connection_status_code =
        MapConnectionStatusToHttpStatus(connection_status);
  ...
  std::string ErrorCodeToString(PulseOne::Enums::ErrorCode error_code) const {
    ...
    case PulseOne::Enums::ErrorCode::TIMEOUT:
      return "TIMEOUT";
    case PulseOne::Enums::ErrorCode::INSUFFICIENT_PERMISSION: // 올바른 enum 값
                                                              // 사용
