...
struct MQTTPublishTask {
  ...
  MQTTPublishTask()
      : qos(MqttQoS::AT_LEAST_ONCE), retained(false),
        scheduled_time(std::chrono::system_clock::now()), retry_count(0),
        priority(5) {}
};
...
struct OfflineMessage {
  ...
  OfflineMessage(const std::string &t, const std::string &p,
                 MqttQoS q = MqttQoS::AT_LEAST_ONCE, bool r = false,
                 int prio = 5)
      : topic(t), payload(p), qos(q), retain(r),
        timestamp(std::chrono::system_clock::now()), priority(prio) {}
};
 ...
 * @details 기본 기능부터 프로덕션 고급 기능까지 모든 MQTT 기능을 하나의
 * 클래스로 제공 ModbusTcpWorker와 동일한 파이프라인 연동 패턴 적용
 *
 * 사용 방법:
...
public:
  ...
  bool SendJsonValuesToPipeline(const nlohmann::json &json_data,
                                ...
                                uint32_t priority = 0);
#else
  // Fallback for no JSON
  bool SendJsonValuesToPipeline(const std::string &raw_json,
                                const std::string &topic_context,
                                uint32_t priority = 0);
#endif
   ...
   */
  bool SendMultipleTopicValuesToPipeline(
      const std::map<std::string, PulseOne::Structs::DataValue> &topic_values,
      const std::string &batch_context, uint32_t priority = 0);
   ...
   * @brief 우선순위 메시지 발행
   */
  bool PublishWithPriority(const std::string &topic, const std::string &payload,
                           int priority, MqttQoS qos = MqttQoS::AT_LEAST_ONCE);
   ...
   * @brief 자동 장애 조치 활성화
   */
  void EnableAutoFailover(const std::vector<std::string> &backup_brokers,
                          int max_failures = 5);
   ...
   */
  virtual bool
  WriteDataPoint(const std::string &point_id,
   ...
   * @return 성공 시 true
   */
  virtual bool WriteAnalogOutput(const std::string &output_id,
   ...
   * @return 성공 시 true
   */
  virtual bool WriteDigitalOutput(const std::string &output_id,
                                  bool value) override;
   ...
   * @return 성공 시 true
   */
  virtual bool WriteSetpoint(const std::string &setpoint_id,
                             double value) override;
   ...
   * @return 성공 시 true
   */
  virtual bool ControlDigitalDevice(const std::string &device_id,
   ...
   * @return 성공 시 true
   */
  virtual bool ControlAnalogDevice(const std::string &device_id,
                                   double value) override;
   ...
   * @return 성공 시 true
   */
  inline bool ConfigureIoTSensor(const std::string &sensor_id,
                                 double config_value) {
   ...
   * @return 성공 시 true
   */
  inline bool ControlSmartLight(const std::string &light_id,
                                double brightness) {
   ...
   * @return 성공 시 true
   */
  inline bool SendGatewayCommand(const std::string &gateway_id,
                                 const std::string &command) {
    return PublishMessage(gateway_id + "/cmd", command,
                          QosToInt(MqttQoS::AT_LEAST_ONCE));
  }
   ...
   * @return 성공 시 true
   */
  inline bool SetThermostatSetpoint(const std::string &thermostat_id,
                                    double temperature) {
  ...
  static int QosToInt(MqttQoS qos) { return static_cast<int>(qos); }
...
private:
  ...
  // 오프라인 메시지 큐 (프로덕션 모드)
  std::priority_queue<OfflineMessage, std::vector<OfflineMessage>,
                      OfflineMessageComparator>
      offline_messages_;
  ...
  bool ProcessReceivedMessage(const std::string &topic,
                              const std::string &payload);
  bool ValidateSubscription(const MQTTSubscription &subscription);

  // 🔥 파이프라인 연동 헬퍼 메서드들 (ModbusTcpWorker 패턴)
  const PulseOne::Structs::DataPoint *
  FindDataPointByTopic(const std::string &topic);
  std::optional<PulseOne::Structs::DataPoint>
  FindDataPointById(const std::string &point_id);
  ...
  double CalculateMessagePriority(const std::string &topic,
                                  const std::string &payload);

  // 콜백 및 드라이버 설정
  static void MessageCallback(MQTTWorker *worker, const std::string &topic,
                              const std::string &payload);
  void SetupMQTTDriverCallbacks();

  // 제어 인터페이스 내부 구현
  bool WriteDataPointValue(const std::string &point_id,
                           const PulseOne::Structs::DataValue &value);
  bool PublishControlMessage(const std::string &topic,
                             const PulseOne::Structs::DataValue &value,
                             int qos = 1);
  std::string BuildControlTopic(const std::string &device_id,
                                const std::string &control_type);
  std::string CreateJsonPayload(const PulseOne::Structs::DataValue &value);
  void LogWriteOperation(const std::string &topic,
                         const PulseOne::Structs::DataValue &value,
                         bool success);
