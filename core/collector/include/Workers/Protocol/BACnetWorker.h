...
using ValueChangedCallback =
    std::function<void(const std::string &object_id, const TimestampedValue &)>;
...
public:
   ...
   */
  bool SendBACnetDataToPipeline(
      const std::map<std::string, DataValue> &object_values,
      const std::string &context, uint32_t priority = 0);
   ...
   */
  bool
  SendCOVNotificationToPipeline(const std::string &object_id,
  ...
  FindDataPointByObjectId(const std::string &object_id);

  /**
   * @brief 최적화된 DataPoint 검색 (새로 추가)
   */
  PulseOne::Structs::DataPoint *
  FindDataPointByObjectIdOptimized(const std::string &object_id);
   ...
   */
  void
  LoadDataPointsFromConfiguration(const std::vector<DataPoint> &data_points);
  ...
  bool WriteObjectProperty(const std::string &object_id, const DataValue &value,
                           uint8_t priority = BACNET_NO_PRIORITY);

  bool WriteBACnetDataPoint(const std::string &point_id,
                            const DataValue &value);
   ...
   * @brief 냉각밸브 제어
   */
  inline bool ControlChilledWaterValve(const std::string &valve_id,
                                       double position) {
   ...
   * @brief 가열밸브 제어
   */
  inline bool ControlHeatingValve(const std::string &valve_id,
                                  double position) {
    return ControlAnalogDevice(valve_id, position);
  }
   ...
   * @brief 온도 설정점 변경
   */
  inline bool SetTemperatureSetpoint(const std::string &zone_id,
                                     double temperature) {
...
private:
  ...
  std::unordered_map<std::string, PulseOne::Structs::DataPoint *>
      datapoint_index_;
