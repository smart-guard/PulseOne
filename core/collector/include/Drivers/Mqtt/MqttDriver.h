...
public:
   ...
   * @return 성공 여부
   */
  bool EnableDiagnostics(bool enable = true, bool packet_logging = false,
   ...
   * @return 선택된 브로커 URL
   */
  std::string SelectOptimalBroker(const std::string &topic,
   ...
   * @return 성공 여부
   */
  bool SetClientCertificate(const std::string &cert_path,
   ...
   * @return 성공 여부
   */
  bool EnableBatchProcessing(bool enable = true, size_t batch_size = 100,
                             int batch_timeout_ms = 1000);
...
private:
   ...
   * @param data 이벤트 데이터
   */
  void NotifyAdvancedFeatures(const std::string &event_type,
   ...
   * @param details 추가 정보
   */
  void RecordDiagnosticEvent(const std::string &operation, bool success,
                             double duration_ms,
   ...
   * @param reason 원인
   */
  void NotifyConnectionChange(bool connected, const std::string &broker_url,
   ...
   */
  void NotifyMessageProcessing(const std::string &broker_url,
   ...
   * @brief 통계 업데이트
   */
  void UpdateStats(const std::string &operation, bool success,
  ...
  void LogMessage(const std::string &level, const std::string &message,
                  const std::string &category = "MQTT") const;
   ...
   * @brief 수신된 메시지 처리
   */
  void ProcessReceivedMessage(const std::string &topic,
