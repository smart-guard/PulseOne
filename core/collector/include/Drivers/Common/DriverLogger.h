...
private:
  ...
  mutable std::atomic<uint64_t> log_count_maintenance_{
      0}; // 🆕 MAINTENANCE 추가
  ...
  std::string
  FormatMessage(LogLevel level, DriverLogCategory category,
...
public:
  DriverLogger(const UniqueId &device_id = "",
    ...
    // 모든 카테고리 기본 활성화
    for (int i = 0; i <= static_cast<int>(DriverLogCategory::DIAGNOSTICS);
         ++i) {
  ...
  void LogPerformanceMetric(const std::string &metric_name, double value,
    ...
    std::ostringstream oss;
    oss << "Performance metric: " << metric_name << " = " << value << " "
        << unit;
  ...
  LogStatistics getStatistics() const {
    ...
    stats.total_logs = stats.debug_count + stats.info_count +
                       stats.warning_count + stats.error_count +
                       stats.trace_count + stats.maintenance_count;
    return stats;
  }
