...
// =============================================================================
// 매크로 충돌 방지 (Windows 호환성 및 표준 라이브러리 충돌 방지)
// =============================================================================
#ifdef max
#undef max
#endif
...
// BACnet 라이브러리가 min/max 매크로를 정의했을 가능성이 있으므로 다시 한번
// undef
#ifdef max
#undef max
#endif
...
#if !defined(HAS_BACNET) && !defined(HAS_BACNET) &&                          \
    !defined(HAS_BACNET_STACK)
...
#endif
#ifndef BACNET_NO_PRIORITY
constexpr uint32_t BACNET_NO_PRIORITY = 0;
#endif
#ifndef BACNET_MAX_INSTANCE
...
inline BACnetObjectType StringToObjectType(const std::string &type_str) {
  ...
  if (upper_str == "ANALOG_INPUT" || upper_str == "AI")
    return OBJECT_ANALOG_INPUT;
  if (upper_str == "ANALOG_OUTPUT" || upper_str == "AO")
    return OBJECT_ANALOG_OUTPUT;
  if (upper_str == "ANALOG_VALUE" || upper_str == "AV")
    return OBJECT_ANALOG_VALUE;
  if (upper_str == "BINARY_INPUT" || upper_str == "BI")
    return OBJECT_BINARY_INPUT;
  if (upper_str == "BINARY_OUTPUT" || upper_str == "BO")
    return OBJECT_BINARY_OUTPUT;
  if (upper_str == "BINARY_VALUE" || upper_str == "BV")
    return OBJECT_BINARY_VALUE;
  if (upper_str == "DEVICE")
    return OBJECT_DEVICE;
  if (upper_str == "MULTISTATE_INPUT" || upper_str == "MSI")
    return OBJECT_MULTI_STATE_INPUT;
  if (upper_str == "MULTISTATE_OUTPUT" || upper_str == "MSO")
    return OBJECT_MULTI_STATE_OUTPUT;
  if (upper_str == "MULTISTATE_VALUE" || upper_str == "MSV")
...
inline std::string PropertyIdToString(BACnetPropertyId prop_id) {
  ...
  default:
    return "Property " + std::to_string(static_cast<uint32_t>(prop_id));
  } // ✅ switch 괄호 닫힘
} // ✅ function 괄호 닫힘
...
inline BACnetPropertyId StringToPropertyId(const std::string &prop_str) {
  ...
  if (upper_str == "PRESENT_VALUE" || upper_str == "PV")
    return PROP_PRESENT_VALUE;
  if (upper_str == "DESCRIPTION")
    return PROP_DESCRIPTION;
  if (upper_str == "OBJECT_NAME")
    return PROP_OBJECT_NAME;
  if (upper_str == "OBJECT_TYPE")
    return PROP_OBJECT_TYPE;
  if (upper_str == "OBJECT_IDENTIFIER")
    return PROP_OBJECT_IDENTIFIER;
  if (upper_str == "STATUS_FLAGS")
    return PROP_STATUS_FLAGS;
  if (upper_str == "EVENT_STATE")
    return PROP_EVENT_STATE;
  if (upper_str == "OUT_OF_SERVICE")
    return PROP_OUT_OF_SERVICE;
  if (upper_str == "UNITS")
    return PROP_UNITS;
  if (upper_str == "HIGH_LIMIT")
    return PROP_HIGH_LIMIT;
  if (upper_str == "LOW_LIMIT")
    return PROP_LOW_LIMIT;
