...
void DeviceApiCallbacks::Setup(RestApiServer *server) {
  ...
  if (!server) {
    LogManager::getInstance().Error(
        "DeviceApiCallbacks::Setup - null server parameter");
    return;
  }
  ...
  server->SetDeviceListCallback([=]() -> json {
    try {
      ...
      try {
        ...
        if (device_repo) {
          ...
          for (const auto &device : devices) {
            ...
            if (status_json.contains("error")) {
              ...
              device_json["connection_status"] =
                  status_json.value("connected", false) ? "connected"
                                                        : "disconnected";
            }
          ...
          LogManager::getInstance().Warn(
              "DeviceRepository not available, using fallback");
          // 폴백으로 기본 디바이스 정보 제공
        ...
        LogManager::getInstance().Error("Error accessing device repository: " +
                                        std::string(e.what()));
        // 에러 발생시에도 빈 배열 반환
        return json::array();
      }
      ...
      LogManager::getInstance().Info(
          "API: Retrieved " + std::to_string(device_list.size()) + " devices");
      return device_list;

    } catch (const std::exception &e) {
      LogManager::getInstance().Error("API: Failed to get device list - " +
                                      std::string(e.what()));
      return json::array();
    }
  });

  server->SetWorkerStatusCallback([=]() -> json {
    try {
      LogManager::getInstance().Info("API: Retrieving workers status");

      json workers_data = json::object();
      ...
      if (device_repo) {
        ...
        for (const auto &device : devices) {
          ...
          if (status.contains("error")) {
            workers[device_id] = {{"status", "stopped"},
                                  {"error", status["error"]}};
          } else {
            workers[device_id] = status;
          }
        }
      }

      workers_data["workers"] = workers;
      workers_data["timestamp"] =
      ...
      LogManager::getInstance().Error("API: Failed to get workers status - " +
                                      std::string(e.what()));
      return {{"workers", json::object()}, {"error", e.what()}};
    }
  });

  server->SetDeviceStatusCallback([=](const std::string &device_id) -> json {
    try {
      LogManager::getInstance().Info("API: Getting status for device " +
                                     device_id);
      ...
      if (!status.contains("error")) {
        ...
        status["last_error"] = "";
        status["scan_rate"] =
            status.contains("metadata")
                ? status["metadata"].value("polling_interval_ms", 1000)
                : 1000;
      }
      ...
      LogManager::getInstance().Error("API: Failed to get device status for " +
                                      device_id + " - " +
                                      std::string(e.what()));
  ...
  server->SetDeviceStartCallback([=](const std::string &device_id) -> bool {
    try {
      LogManager::getInstance().Info("API: Starting device worker " +
                                     device_id);
      ...
      if (result) {
        LogManager::getInstance().Info("Device worker started successfully: " +
                                       device_id);
      } else {
        LogManager::getInstance().Error("Failed to start device worker: " +
                                        device_id);
      }
      ...
      LogManager::getInstance().Error("API: Exception starting device " +
                                      device_id + " - " +
                                      std::string(e.what()));
      return false;
    }
  ...
  server->SetDeviceStopCallback([=](const std::string &device_id) -> bool {
    try {
      LogManager::getInstance().Info("API: Stopping device worker " +
                                     device_id);
      ...
      if (result) {
        LogManager::getInstance().Info("Device worker stopped successfully: " +
                                       device_id);
      } else {
        LogManager::getInstance().Error("Failed to stop device worker: " +
                                        device_id);
      }
      ...
      LogManager::getInstance().Error("API: Exception stopping device " +
                                      device_id + " - " +
                                      std::string(e.what()));
      return false;
    }
  ...
  server->SetDevicePauseCallback([=](const std::string &device_id) -> bool {
    try {
      ...
      LogManager::getInstance().Error("API: Exception pausing device " +
                                      device_id + " - " +
                                      std::string(e.what()));
      return false;
    }
  ...
  server->SetDeviceResumeCallback([=](const std::string &device_id) -> bool {
    try {
      LogManager::getInstance().Info("API: Resuming device worker " +
                                     device_id);
      ...
      LogManager::getInstance().Error("API: Exception resuming device " +
                                      device_id + " - " +
                                      std::string(e.what()));
      return false;
    }
  ...
  server->SetDeviceRestartCallback([=](const std::string &device_id) -> bool {
    try {
      LogManager::getInstance().Info("API: Restarting device worker " +
                                     device_id);
      ...
      LogManager::getInstance().Error("API: Exception restarting device " +
                                      device_id + " - " +
                                      std::string(e.what()));
      return false;
    }
  ...
  server->SetDeviceReloadSettingsCallback(
      [=](const std::string &device_id) -> bool {
        try {
          LogManager::getInstance().Info("API: Reloading settings for device " +
                                         device_id);
          auto &worker_mgr = Workers::WorkerManager::getInstance();
          return worker_mgr.ReloadWorkerSettings(device_id);
        } catch (const std::exception &e) {
          LogManager::getInstance().Error(
              "API: Exception reloading settings for " + device_id + " - " +
              e.what());
          return false;
        }
      });

  server->SetDiscoveryStartCallback([=](const std::string &device_id,
                                        bool force) -> bool {
    try {
      LogManager::getInstance().Info("API: Starting discovery for device " +
                                     device_id + (force ? " (force)" : ""));
      auto &worker_mgr = Workers::WorkerManager::getInstance();

      // 디스커버리 시작 전 설정을 먼저 리로드하여 auto_registration_enabled가
      // 최신인지 확인
      worker_mgr.ReloadWorkerSettings(device_id);

      // 실제 디스커버리 로직 트리거 (현재는 설정 리로드만으로도 워커가 인지하게
      // 됨) 미래에는 워커에 직접 명시적인 Scan() 명령을 내릴 수도 있음
      return true;
    } catch (const std::exception &e) {
      LogManager::getInstance().Error("API: Exception starting discovery for " +
                                      device_id + " - " + e.what());
      return false;
    }
  });

  server->SetNetworkScanCallback([=](const std::string &protocol,
                                     const std::string &range,
                                     int timeout) -> bool {
    try {
      LogManager::getInstance().Info(
          "API: Starting network scan for protocol " + protocol);
      auto &worker_mgr = Workers::WorkerManager::getInstance();
      return worker_mgr.StartNetworkScan(protocol, range, timeout);
    } catch (const std::exception &e) {
      LogManager::getInstance().Error(
          "API: Exception starting network scan - " + std::string(e.what()));
      return false;
    }
  });
  ...
  server->SetReinitializeCallback([=]() -> bool {
    try {
      LogManager::getInstance().Info(
          "=== SYSTEM REINITIALIZATION REQUESTED ===");

      // 1단계: 모든 워커 안전 정지
      LogManager::getInstance().Info("Step 1: Stopping all workers...");
      if (!StopAllWorkersSafely()) {
        LogManager::getInstance().Warn(
            "Some workers didn't stop gracefully, continuing...");
      }
      ...
      // 3단계: 활성 디바이스들 재시작
      LogManager::getInstance().Info(
          "Step 3: Restarting configured devices...");
      ...
      LogManager::getInstance().Error("REINITIALIZATION FAILED: " +
                                      std::string(e.what()));
      return false;
    }
  });
  ...
  server->SetDigitalOutputCallback([=](const std::string &device_id,
                                       ...
                                       bool enable) -> bool {
    try {
      LogManager::getInstance().Info("API: Controlling digital output " +
                                     output_id + " on device " + device_id);
      auto &worker_mgr = Workers::WorkerManager::getInstance();
      return worker_mgr.ControlDigitalOutput(device_id, output_id, enable);
    } catch (const std::exception &e) {
      LogManager::getInstance().Error(
          "API: Exception controlling digital output - " +
          std::string(e.what()));
      return false;
    }
  });
  ...
  server->SetAnalogOutputCallback([=](const std::string &device_id,
                                      ...
                                      double value) -> bool {
    try {
      LogManager::getInstance().Info("API: Controlling analog output " +
                                     output_id + " on device " + device_id);
      auto &worker_mgr = Workers::WorkerManager::getInstance();
      return worker_mgr.ControlAnalogOutput(device_id, output_id, value);
    } catch (const std::exception &e) {
      LogManager::getInstance().Error(
          "API: Exception controlling analog output - " +
          std::string(e.what()));
      return false;
    }
  });
  ...
  LogManager::getInstance().Info(
      "Device control API callbacks setup completed");
}
...
bool DeviceApiCallbacks::StopAllWorkersSafely() {
  try {
    ...
    if (remaining > 0) {
      LogManager::getInstance().Warn("Could not stop " +
...
bool DeviceApiCallbacks::RestartConfiguredDevices() {
  try {
    ...
    try {
      ...
      if (device_repo) {
        ...
        std::string devices_config =
            ConfigManager::getInstance().get("devices.enabled");
      ...
      LogManager::getInstance().Error("Error reading device configuration: " +
                                      std::string(e.what()));
      // 폴백으로 기본 디바이스들 사용
      enabled_devices = {"1", "2", "3"};
    }

    LogManager::getInstance().Info("Found " +
                                   std::to_string(enabled_devices.size()) +
    ...
    for (const auto &device_id : enabled_devices) {
      LogManager::getInstance().Info("Starting worker for device: " +
                                     device_id);

      if (worker_mgr.StartWorker(device_id)) {
        restarted_count++;
        LogManager::getInstance().Info("Device " + device_id +
                                       " started successfully");
      } else {
        LogManager::getInstance().Error("Failed to start worker for device: " +
                                        device_id);
      }
    ...
    LogManager::getInstance().Info(
        "Restarted " + std::to_string(restarted_count) + "/" +
        std::to_string(enabled_devices.size()) + " workers");

    return restarted_count > 0;

  } catch (const std::exception &e) {
    LogManager::getInstance().Error("Error restarting workers: " +
                                    std::string(e.what()));
    return false;
  }
}
