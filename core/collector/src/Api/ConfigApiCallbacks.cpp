...
void ConfigApiCallbacks::Setup(RestApiServer *server) {
  ...
  if (!server) {
    LogManager::getInstance().Error(
        "ConfigApiCallbacks::Setup - null server parameter");
    return;
  }
  ...
  server->SetReloadConfigCallback([=]() -> bool {
    try {
      ...
      try {
        ...
        LogManager::getInstance().Error("Failed to reload config: " +
                                        std::string(e.what()));
        return false;
      }
      ...
      LogManager::getInstance().Info(
          "Step 3: Applying immediate config changes...");
      ApplyImmediateConfigChanges();

      // 4단계: 모든 워커에게 설정 변경 알림
      LogManager::getInstance().Info(
          "Step 4: Notifying workers of config changes...");
      ...
      LogManager::getInstance().Error("Config reload failed: " +
                                      std::string(e.what()));
      return false;
    }
  });
...
bool ConfigApiCallbacks::ValidateReloadedConfig() {
  try {
    ...
    std::vector<std::string> required_keys = {
        "database.host", "database.port", "database.name",     "redis.host",
        "redis.port",    "log.level",     "COLLECTOR_API_PORT"};

    for (const auto &key : required_keys) {
      // COLLECTOR_API_PORT는 환경변수이므로 별도 확인
      if (key == "COLLECTOR_API_PORT") {
        const char *env_val = std::getenv("COLLECTOR_API_PORT");
        if (!env_val || std::strlen(env_val) == 0) {
          LogManager::getInstance().Warn(
              "COLLECTOR_API_PORT env not set, using default 8501");
        }
        continue;
      }
    ...
    if (db_port <= 0 || db_port > 65535) {
      LogManager::getInstance().Error("Invalid database port: " +
    ...
    if (redis_port <= 0 || redis_port > 65535) {
      LogManager::getInstance().Error("Invalid redis port: " +
                                      std::to_string(redis_port));
      return false;
    }

    const char *env_port = std::getenv("COLLECTOR_API_PORT");
    int api_port = env_port ? std::atoi(env_port) : 8501;
    if (api_port <= 0 || api_port > 65535) {
      LogManager::getInstance().Error("Invalid API port: " +
    ...
    if (log_level.empty()) {
      LogManager::getInstance().Warn(
          "Log level not configured, will use default");
    ...
    LogManager::getInstance().Error("Configuration validation error: " +
                                    std::string(e.what()));
    return false;
  }
}
...
void ConfigApiCallbacks::ApplyImmediateConfigChanges() {
  try {
    ...
    logger.Info("New default scan interval: " + std::to_string(scan_interval) +
                "ms");

    bool debug_mode = config_mgr.getBool("debug.enabled", false);
    logger.Info("Debug mode: " +
                std::string(debug_mode ? "enabled" : "disabled"));

    logger.Info("Immediate configuration changes applied");

  } catch (const std::exception &e) {
    LogManager::getInstance().Error(
        "Error applying immediate config changes: " + std::string(e.what()));
  }
}
...
void ConfigApiCallbacks::NotifyWorkersConfigChanged() {
  try {
    ...
    logger.Info("Found " + std::to_string(active_count) +
                " active workers to notify");
    ...
    LogManager::getInstance().Error("Worker notification failed: " +
                                    std::string(e.what()));
  }
}