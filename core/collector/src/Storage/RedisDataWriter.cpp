...
void RedisDataWriter::SetRedisClient(
...
size_t
RedisDataWriter::SaveDeviceMessage(const Structs::DeviceDataMessage &message) {
  if (!IsConnected())
    return 0;
  ...
  try {
    ...
    // 1. 경량 모드 저장
    if (storage_mode_ == StorageMode::LIGHTWEIGHT ||
        storage_mode_ == StorageMode::HYBRID) {
      total_saved += SaveLightweightFormat(message);
    }

    // 2. 완전 데이터 저장
    if (storage_mode_ == StorageMode::FULL_DATA ||
        storage_mode_ == StorageMode::HYBRID) {
...
size_t RedisDataWriter::SaveLightweightFormat(
...
size_t
RedisDataWriter::SaveFullDataFormat(const Structs::DeviceDataMessage &message) {
  json full_data;
  ...
  std::string cv_key =
      "current_values:" + ExtractDeviceNumber(message.device_id);
  redis_client_->setex(cv_key, full_data.dump(), 3600);

  return 1;
}

size_t RedisDataWriter::SaveDevicePatternFormat(
    const Structs::DeviceDataMessage &message) {
  size_t saved = 0;
  ...
  for (const auto &point : message.points) {
    ...
    std::string device_key =
        "device:" + device_num + ":" + device_point.point_name;
...
size_t RedisDataWriter::SavePointLatestFormat(
    const Structs::DeviceDataMessage &message) {
  size_t saved = 0;
  std::string device_num = ExtractDeviceNumber(message.device_id);
  ...
  for (const auto &point : message.points) {
    ...
    std::string point_key =
        "point:" + std::to_string(point.point_id) + ":latest";
    redis_client_->setex(point_key, point_latest.toJson().dump(), 3600);

    // 🔧 ScheduledExporter 호환성: :current 키에도 저장
    std::string current_key =
        "point:" + std::to_string(point.point_id) + ":current";
    redis_client_->setex(current_key, point_latest.toJson().dump(), 3600);
...
bool RedisDataWriter::SaveSinglePoint(const Structs::TimestampedValue &point,
...
bool RedisDataWriter::PublishAlarmEvent(
    const BackendFormat::AlarmEventData &alarm_data) {
  if (!IsConnected()) {
    return false;
  }
  ...
  try {
    ...
    if (alarm_data.state == "active" || alarm_data.state == "ACTIVE") {
      std::string active_key =
          "alarm:active:" + std::to_string(alarm_data.rule_id);
    ...
    // redis_client_->ltrim("alarm:history", 0, 999); // Not supported in
    // interface yet
  ...
  try {
    ...
    for (const auto &value : current_values) {
      try {
        ...
        std::string point_key =
            "point:" + std::to_string(value.point_id) + ":latest";
        redis_client_->setex(point_key, point_latest.toJson().dump(), 7200);

        // 🔧 ScheduledExporter 호환성: :current 키에도 저장
        std::string current_key =
            "point:" + std::to_string(value.point_id) + ":current";
        redis_client_->setex(current_key, point_latest.toJson().dump(), 7200);
...
size_t RedisDataWriter::SyncWorkerDataPoints(
  ...
  try {
    ...
    for (const auto &point : points) {
      ...
      if (point_name.empty())
        point_name = "point_" + point.id; // 안전장치
      ...
      if (!device_key_exists) {
        ...
        // 🔧 ScheduledExporter 호환성: :current 키에도 저장
        std::string current_key = "point:" + point.id + ":current";
        redis_client_->setex(current_key, latest_data.toJson().dump(), 7200);

        LogManager::getInstance().log("redis_writer", LogLevel::DEBUG_LEVEL,
                                      "새 포인트 Redis 키 생성: " + device_key);
      } else {
        // 이미 존재하면 구조(메타데이터)만 업데이트하고 값은 유지할 수도 있음.
        // 하지만 현재 구조상 JSON 전체를 덮어쓰지 않고 일부만 수정하기는 어려울
        // 수 있음. 값은 유지하되, 이름이나 단위 등이 바뀌었을 수 있으므로
        // 읽어서 업데이트하는 것이 이상적. 성능상 여기서는 '존재하면 건너뛰기'
        // 전략을 사용하거나, 필요하다면 GET -> Update -> SET 과정을 거쳐야 함.
...
bool RedisDataWriter::SaveWorkerStatus(const std::string &device_id,
...
std::string
RedisDataWriter::ConvertValueToString(const Structs::DataValue &value) const {
  return std::visit(
...
std::string RedisDataWriter::ConvertQualityToString(
    const Structs::DataQuality &quality) const {
  switch (quality) {
  case Structs::DataQuality::GOOD:
    return "good";
  case Structs::DataQuality::BAD:
    return "bad";
  case Structs::DataQuality::UNCERTAIN:
    return "uncertain";
  case Structs::DataQuality::NOT_CONNECTED:
    return "comm_failure";
  case Structs::DataQuality::TIMEOUT:
    return "timeout";
  default:
    return "unknown";
  }
...
std::string
RedisDataWriter::GetDataTypeString(const Structs::DataValue &value) const {
  return std::visit(
      [](const auto &v) -> std::string {
        using T = std::decay_t<decltype(v)>;
        if constexpr (std::is_same_v<T, bool>)
...
std::string
RedisDataWriter::ExtractDeviceNumber(const std::string &device_id) const {
  // "device_001" -> "1"
...
void RedisDataWriter::HandleError(const std::string &context,
                                  const std::string &error_message) {
  stats_.total_writes.fetch_add(1);
  stats_.failed_writes.fetch_add(1);

  std::string full_message = context + ": " + error_message;
  LogManager::getInstance().log("redis_writer", LogLevel::LOG_ERROR,
                                full_message);
}
...
json RedisDataWriter::GetStatus() const {
  ...
  status["cache_size"] = {{"point_names", point_name_cache_.size()},
                          {"units", unit_cache_.size()}};

  return status;
}

bool RedisDataWriter::StoreVirtualPointToRedis(
    const Structs::TimestampedValue &virtual_point_data) {
  if (!IsConnected()) {
    return false;
  }
  ...
  try {
    ...
    // 가상포인트 전용 키 패턴
    std::string vp_key =
        "virtualpoint:" + std::to_string(virtual_point_data.point_id);
    ...
    vp_json["timestamp"] =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            virtual_point_data.timestamp.time_since_epoch())
            .count();
