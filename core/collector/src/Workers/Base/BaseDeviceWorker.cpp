...
BaseDeviceWorker::BaseDeviceWorker(
  ...
  LogMessage(LogLevel::INFO,
             "BaseDeviceWorker 생성됨 (Worker ID: " + worker_id_ + ")");
  ...
  // 🔥 재연결 설정 초기화 (DeviceInfo 및 properties 맵 기반)
  {
    ...
    reconnection_settings_.wait_time_after_max_retries_ms =
        device_info_.backoff_time_ms;
    reconnection_settings_.keep_alive_enabled =
        device_info_.is_keep_alive_enabled;
    reconnection_settings_.keep_alive_interval_seconds =
        device_info_.keep_alive_interval_s;

    if (device_info_.connection_timeout_ms.has_value()) {
      reconnection_settings_.connection_timeout_seconds =
          device_info_.connection_timeout_ms.value() / 1000;
    }

    // 2. properties 맵(config JSON)에서 추가/덮어쓰기 지원 (MasterModelModal
    // 호환)
    if (HasProperty("retry_interval_ms"))
      reconnection_settings_.retry_interval_ms =
          std::stoi(GetProperty("retry_interval_ms"));
    if (HasProperty("max_retry_count"))
      reconnection_settings_.max_retries_per_cycle =
          std::stoi(GetProperty("max_retry_count"));
    if (HasProperty("backoff_time_ms"))
      reconnection_settings_.wait_time_after_max_retries_ms =
          std::stoi(GetProperty("backoff_time_ms"));
    if (HasProperty("is_keep_alive_enabled"))
      reconnection_settings_.keep_alive_enabled =
          (GetProperty("is_keep_alive_enabled") == "1" ||
           GetProperty("is_keep_alive_enabled") == "true");
    if (HasProperty("keep_alive_interval_s"))
      reconnection_settings_.keep_alive_interval_seconds =
          std::stoi(GetProperty("keep_alive_interval_s"));

    LogMessage(
        LogLevel::DEBUG_LEVEL,
        "Initial Reconnection Settings: Interval=" +
  ...
  LogMessage(LogLevel::INFO,
             "BaseDeviceWorker created for device: " + device_info_.name);
...
BaseDeviceWorker::~BaseDeviceWorker() {
  ...
  StopAllThreads();
  LogMessage(LogLevel::INFO,
             "BaseDeviceWorker destroyed for device: " + device_info_.name);
}
  ...
  if (!thread_running_.exchange(true)) {
    try {
      ...
      LogMessage(LogLevel::DEBUG_LEVEL,
                 "Reconnection thread started successfully");
    } catch (const std::exception &e) {
      thread_running_ = false;
      LogMessage(LogLevel::LOG_ERROR, "Failed to start reconnection thread: " +
                                          std::string(e.what()));
    }
  }
}
  ...
  if (reconnection_thread_) {
    if (reconnection_thread_->joinable()) {
      try {
        ...
        LogMessage(LogLevel::DEBUG_LEVEL,
                   "Reconnection thread joined successfully");
      } catch (const std::exception &e) {
        LogMessage(LogLevel::LOG_ERROR, "Error joining reconnection thread: " +
                                            std::string(e.what()));
      }
    }
...
void BaseDeviceWorker::ReloadSettings(
  ...
  LogMessage(LogLevel::INFO,
             "설정 동기화 완료: (auto_reg=" +
                 std::string(device_info_.is_auto_registration_enabled
                                 ? "true"
                                 : "false") +
                 ")");
}
...
void BaseDeviceWorker::ReloadDataPoints(
  ...
  LogMessage(LogLevel::INFO,
             "데이터 포인트 리로드 완료: " + std::to_string(old_size) + " -> " +
...
bool BaseDeviceWorker::UpdateReconnectionSettings(
    const ReconnectionSettings &settings) {
  std::lock_guard<std::mutex> lock(settings_mutex_);
...
std::string BaseDeviceWorker::GetReconnectionStats() const {
  ...
  ss << "    \"total_connections\": "
     << reconnection_stats_.total_connections.load() << ",\n";
  ss << "    \"successful_connections\": "
     << reconnection_stats_.successful_connections.load() << ",\n";
  ss << "    \"failed_connections\": "
     << reconnection_stats_.failed_connections.load() << ",\n";
  ss << "    \"reconnection_cycles\": "
     << reconnection_stats_.reconnection_cycles.load() << ",\n";
  ss << "    \"wait_cycles\": " << reconnection_stats_.wait_cycles.load()
     << ",\n";
  ss << "    \"keep_alive_sent\": "
     << reconnection_stats_.keep_alive_sent.load() << ",\n";
  ss << "    \"keep_alive_failed\": "
     << reconnection_stats_.keep_alive_failed.load() << ",\n";
  ss << "    \"avg_connection_duration_seconds\": "
     << reconnection_stats_.avg_connection_duration_seconds.load() << "\n";
  ss << "  },\n";
  ss << "  \"current_state\": {\n";
...
void BaseDeviceWorker::ChangeState(WorkerState new_state) {
  ...
  if (old_state != new_state) {
    LogMessage(LogLevel::INFO, "상태 변경: " + WorkerStateToString(old_state) +
...
void BaseDeviceWorker::SetConnectionState(bool connected) {
  ...
  if (old_connected != connected) {
    LogMessage(LogLevel::INFO,
               connected ? "디바이스 연결됨" : "디바이스 연결 해제됨");
...
void BaseDeviceWorker::HandleConnectionError(const std::string &error_message) {
  ...
  if (current == WorkerState::RUNNING ||
      current == WorkerState::DEVICE_OFFLINE) {
        ...
        if ((current_state == WorkerState::RUNNING ||
          ...
          if (AttemptReconnection()) {
            ...
            if (current_state == WorkerState::RECONNECTING ||
                current_state == WorkerState::WAITING_RETRY) {
              ChangeState(WorkerState::RUNNING);
            }
          } else {
...
void BaseDeviceWorker::HandleKeepAlive() {
  ...
  auto now = system_clock::now();
  auto time_since_last_keepalive =
      duration_cast<seconds>(now - last_keep_alive_time_).count();

  if (time_since_last_keepalive >=
      reconnection_settings_.keep_alive_interval_seconds) {
    try {
      ...
      if (keepalive_success) {
        ...
        if (!CheckConnection()) {
          LogMessage(LogLevel::WARN,
                     "Keep-alive sent but connection check failed");
      ...
      LogMessage(LogLevel::LOG_ERROR,
                 "Keep-alive exception: " + std::string(e.what()));
...
bool BaseDeviceWorker::IsActiveState(WorkerState state) {
  return state == WorkerState::RUNNING || state == WorkerState::SIMULATION ||
...
// =============================================================================
bool BaseDeviceWorker::SendDataToPipeline(
  ...
  try {
    ...
    message.device_type =
        device_info_.device_type; // 🔥 Hybrid Data Strategy: 디바이스 타입 전달
    ...
    if (device_info_.GetProtocolName() == "MODBUS_TCP" ||
        device_info_.GetProtocolName() == "MODBUS_RTU") {
      message.high_priority = (priority > 3);
    } else if (device_info_.GetProtocolName() == "MQTT") {
      message.high_priority = (priority > 0); // MQTT는 실시간성 중요
    } else if (device_info_.GetProtocolName() == "BACNET_IP" ||
               device_info_.GetProtocolName() == "BACNET_MSTP") {
    ...
    message.device_status =
        ConvertWorkerStateToDeviceStatus(current_state_.load());
    ...
    if (success) {
      LogMessage(
          LogLevel::DEBUG_LEVEL,
          "파이프라인 전송 성공: " + std::to_string(values.size()) +
              "개 포인트 " + "(상태: " +
              PulseOne::Enums::DeviceStatusToString(message.device_status) +
...
bool BaseDeviceWorker::SendValuesToPipelineWithLogging(
    const std::vector<PulseOne::Structs::TimestampedValue> &values,
    const std::string &data_type, uint32_t priority) {
  ...
  try {
    ...
    LogMessage(LogLevel::LOG_ERROR, "파이프라인 전송 예외 (" + data_type +
                                        "): " + std::string(e.what()));
    return false;
  }
}

uint32_t BaseDeviceWorker::RegisterNewDataPoint(
    const std::string &name, const std::string &address_string,
    const std::string &mapping_key, const std::string &data_type) {
  if (!device_info_.is_auto_registration_enabled) {
    return 0;
  }

  try {
    LogMessage(LogLevel::INFO, "새로운 데이터포인트 자동 등록 시도: " + name +
                                   " (" + address_string + ")");
    ...
    if (!repo) {
      LogMessage(LogLevel::LOG_ERROR,
                 "DataPointRepository를 가져올 수 없습니다.");
      return 0;
    }
    ...
    try {
      ...
      next_address = max_addr + 1;
    } catch (...) {
      LogMessage(LogLevel::WARN, "기존 Address 조회 실패, 임시 주소 사용: " +
                                     std::to_string(std::time(nullptr)));
      next_address = static_cast<int>(std::time(nullptr)); // Fallback
    }
    ...
    entity.setAddress(next_address); // 🔥 고유 주소 설정
    entity.setMappingKey(mapping_key);
    entity.setEnabled(true);
    entity.setDataType(data_type);   // 🔥 전달된 데이터 타입 사용
    entity.setPollingInterval(1000); // 기본값 1초

    // 🔥 개선: 자동 등록되는 모든 데이터 포인트의 범위값을 0으로 초기화
    // (UI에서 비정상적으로 큰 과학적 표기법 값이 표시되는 것을 방지)
    ...
    if (repo->save(entity)) {
      ...
      LogMessage(LogLevel::INFO,
                 "새로운 데이터포인트 등록 성공: ID=" + std::to_string(new_id));
      ...
      // 🔥 개선: data_type을 "number"가 아닌 실제 타입 기반으로 설정
      if (data_type == "STRING")
        dp.data_type = "string";
      else if (data_type == "BOOL")
        dp.data_type = "boolean";
      else if (data_type == "DATETIME")
        dp.data_type = "datetime";
      else
        dp.data_type = "number";
    ...
    LogMessage(LogLevel::LOG_ERROR,
               "RegisterNewDataPoint 예외: " + std::string(e.what()));
    return 0;
  }
}
...
// =============================================================================
std::vector<PulseOne::Structs::DataPoint>
BaseDeviceWorker::DiscoverDataPoints() {
  LogMessage(LogLevel::DEBUG_LEVEL,
             "DiscoverDataPoints not implemented for this protocol type: " +
                 GetProtocolType());
  return {};
}
...
void BaseDeviceWorker::LogMessage(LogLevel level,
...
std::string BaseDeviceWorker::GetWorkerIdString() const {
  std::stringstream ss;
  ss << device_info_.GetProtocolName() << "_" << device_info_.name << "_"
     << device_info_.id;
  return ss.str();
}

void BaseDeviceWorker::UpdateCommunicationResult(
    bool success, const std::string &error_msg, int error_code,
...
void BaseDeviceWorker::OnStateChanged(WorkerState old_state,
...
BaseDeviceWorker::ConvertWorkerStateToDeviceStatus(WorkerState state) const {
  switch (state) {
...
std::string BaseDeviceWorker::GenerateCorrelationId() const {
  ...
  auto timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
                       ...
                       .count();
  return device_info_.id + "_" + GetWorkerIdString() + "_" +
