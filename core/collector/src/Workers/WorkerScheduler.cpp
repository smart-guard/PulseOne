...
WorkerScheduler::WorkerScheduler(
    ...
    : registry_(std::move(registry)), redis_writer_(std::move(redis_writer)) {}
...
bool WorkerScheduler::StartWorker(const std::string &device_id) {
  ...
  if (worker) {
    ...
    if (state == WorkerState::RUNNING || state == WorkerState::RECONNECTING) {
      LogManager::getInstance().Info(
          "WorkerScheduler - Worker already active: " + device_id);
      return true;
    }
    ...
    if (state == WorkerState::STOPPED || state == WorkerState::WORKER_ERROR) {
      ...
      try {
        ...
        LogManager::getInstance().Error(
            "WorkerScheduler - Exception restarting worker " + device_id +
            ": " + e.what());
        return false;
      }
    }
  ...
  if (!worker) {
    ...
    if (!worker) {
      LogManager::getInstance().Error(
          "WorkerScheduler - Failed to create worker: " + device_id);
      return false;
    }
  }

  try {
    LogManager::getInstance().Info("WorkerScheduler - Starting worker: " +
                                   device_id);
    ...
    LogManager::getInstance().Error(
        "WorkerScheduler - Exception starting worker " + device_id + ": " +
        e.what());
    return false;
  }
}
...
bool WorkerScheduler::StopWorker(const std::string &device_id) {
  ...
  auto worker = registry_->GetWorker(device_id);
  if (!worker) {
    LogManager::getInstance().Info("WorkerScheduler - No worker to stop: " +
                                   device_id);
    return true;
  }

  try {
    LogManager::getInstance().Info("WorkerScheduler - Stopping worker: " +
                                   device_id);
    ...
    LogManager::getInstance().Error(
        "WorkerScheduler - Exception stopping worker " + device_id + ": " +
        e.what());
...
bool WorkerScheduler::RestartWorker(const std::string &device_id) {
  LogManager::getInstance().Info("WorkerScheduler - Restart requested: " +
                                 device_id);
  ...
  try {
    ...
    if (redis_writer_) {
      try {
        ...
        restart_metadata["restart_initiated_at"] =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now().time_since_epoch())
                .count();
        restart_metadata["action"] = "worker_restart";
        redis_writer_->SaveWorkerStatus(device_id, "restarting",
                                        restart_metadata);
      } catch (...) {
      }
    }
    ...
    LogManager::getInstance().Error(
        "WorkerScheduler - Exception during restart " + device_id + ": " +
        e.what());
...
bool WorkerScheduler::ReloadWorkerSettings(const std::string &device_id) {
  if (!registry_->HasWorker(device_id))
    return false;
...
bool WorkerScheduler::PauseWorker(const std::string &device_id) {
  ...
  // assuming it might be added or we just log/noop for now based on original
  // Manager. Original Manager didn't implement these in the snippet provided,
  // but header had signatures. We'll leave as placeholders or impl if
  // available.
...
int WorkerScheduler::StartAllActiveWorkers() {
  LogManager::getInstance().Info(
      "WorkerScheduler - Starting ALL active workers...");
  ...
  try {
    ...
    // 🔥 컬렉터 ID (에지 서버 ID) 가져오기
    int collector_id = Config().getCollectorId();
    LogManager::getInstance().Info(
        "WorkerScheduler - Fetching devices for Collector ID: " +
        std::to_string(collector_id));

    // 🔥 해당 컬렉터에 할당된 디바이스만 조회
    auto all_devices = device_repo->findByEdgeServer(collector_id);
    ...
    if (active_device_ids.empty()) {
      LogManager::getInstance().Warn(
          "WorkerScheduler - No active devices found for Collector ID " +
          std::to_string(collector_id) +
          ". Partitioning check: are devices assigned to this collector in the "
          "DB?");
      return 0;
    }
    ...
    for (const auto &id : active_device_ids) {
      if (StartWorker(id)) {
        ...
        LogManager::getInstance().Error(
            "WorkerScheduler - Failed to start worker: " + id);
      }
    ...
    LogManager::getInstance().Error("WorkerScheduler - Bulk start failed: " +
                                    std::string(e.what()));
    return 0;
  }
}
...
void WorkerScheduler::StopAllWorkers() {
  ...
  for (auto &worker : workers) {
    // We iterate registry copies, but we need ID to call StopWorker (which
    // unregisters) BaseDeviceWorker doesn't expose ID getter in interface
    // usually? We'll iterate the registry map keys using GetAllWorkers
    // implementation details or friend access? WorkerRegistry::ForEachWorker is
    // better.
  }

  registry_->ForEachWorker(
      [this](const std::string &id, std::shared_ptr<BaseDeviceWorker>) {
...
bool WorkerScheduler::WriteDataPoint(const std::string &device_id,
...
bool WorkerScheduler::ControlDigitalOutput(const std::string &device_id,
...
bool WorkerScheduler::ControlAnalogOutput(const std::string &device_id,
                                          const std::string &output_id,
                                          double value) {
  auto worker = registry_->GetWorker(device_id);
  if (!worker)
...
bool WorkerScheduler::ControlDigitalDevice(const std::string &device_id,
                                           const std::string &device_id_target,
                                           bool enable) {
  auto worker = registry_->GetWorker(device_id);
  if (!worker)
    return false;
...
int WorkerScheduler::BatchInitializeRedisData(
...
WorkerScheduler::LoadCurrentValuesFromDB(const std::string &device_id) {
    ...
    for (const auto &ent : entities) {
      ...
      try {
        ...
        if (j.contains("value")) {
          auto jv = j["value"];
          if (jv.is_boolean())
            tv.value = jv.get<bool>();
          else if (jv.is_number_integer())
            tv.value = jv.get<int>();
          else if (jv.is_number_float())
            tv.value = jv.get<double>();
          else if (jv.is_string())
            tv.value = jv.get<std::string>();
...
bool WorkerScheduler::ConvertStringToDataValue(const std::string &str,
