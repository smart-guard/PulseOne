...
HttpRestWorker::HttpRestWorker(const DeviceInfo& device_info)
    ...
    , http_driver_(nullptr)
    , polling_thread_running_(false)
{
    LogMessage(LogLevel::INFO, "HttpRestWorker created for device: " + device_info.name);
...
std::future<bool> HttpRestWorker::Start() {
    return std::async(std::launch::async, [this]() -> bool {
        ...
        polling_thread_ = std::make_unique<std::thread>(&HttpRestWorker::PollingThreadFunction, this);
...
bool HttpRestWorker::WriteDataPoint(const std::string& point_id, const DataValue& value) {
    if (!http_driver_) return false;
...
bool HttpRestWorker::ParseHttpRestConfig() {
    ...
    http_config_.protocol = PulseOne::Enums::ProtocolType::HTTP_REST;
...
bool HttpRestWorker::InitializeHttpRestDriver() {
    try {
        ...
        http_driver_ = PulseOne::Drivers::DriverFactory::GetInstance().CreateDriver("HTTP_REST");
        
        if (!http_driver_) {
            LogMessage(LogLevel::LOG_ERROR, "Failed to create HTTP/REST driver from factory");
            return false;
        }
        ...
        LogMessage(LogLevel::LOG_ERROR, "Exception during HTTP/REST driver initialization: " + std::string(e.what()));
        return false;
    }
}
...
void HttpRestWorker::PollingThreadFunction() {
    ...
    while (polling_thread_running_) {
        if (GetState() != WorkerState::RUNNING || !http_driver_ || !http_driver_->IsConnected()) {
        ...
        uint32_t interval = device_info_.polling_interval_ms > 0 ? device_info_.polling_interval_ms : 5000;
