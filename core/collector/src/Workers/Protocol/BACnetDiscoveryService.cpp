...
int ResolveProtocolTypeToId(const std::string &protocol_type) {
  try {
    ...
    std::string actual_protocol_type =
        protocol_type.empty() ? "BACNET_IP" : protocol_type;
    auto protocol_opt = protocol_repo->findByType(actual_protocol_type);

    if (protocol_opt.has_value()) {
      LogManager::getInstance().Debug("Resolved protocol '" +
                                      actual_protocol_type + "' to ID " +
                                      std::to_string(protocol_opt->getId()));
      return protocol_opt->getId();
    } else {
      LogManager::getInstance().Warn(
          "Protocol type not found in database: " + actual_protocol_type +
          ". Attempting to seed...");
      return 0; // 프로토콜이 DB에 없으면 0 반환
    }

  } catch (const std::exception &e) {
    LogManager::getInstance().Error("Failed to resolve protocol type '" +
                                    protocol_type +
                                    "': " + std::string(e.what()));
    return 0;
  }
  ...
  try {
    ...
    if (!protocol_opt) {
      ...
      protocol.setDescription(
          "Building Automation and Control Network over IP");
      ...
      if (protocol_repo->save(protocol)) {
        LogManager::getInstance().Info(
            "Successfully seeded BACNET_IP protocol into database");
      }
    }
  } catch (const std::exception &e) {
    LogManager::getInstance().Error("Error seeding BACnet protocol: " +
                                    std::string(e.what()));
  }
}
...
void ConvertDeviceInfoToEntity(const DeviceInfo &device_info,
                               Database::Entities::DeviceEntity &entity);
Database::Entities::DeviceEntity
ConvertDeviceInfoToEntity(const DeviceInfo &device_info);

// =============================================================================
// 🔥 DeviceInfo ↔ DeviceEntity 변환 구현
// =============================================================================

void BACnetDiscoveryService::ConvertDeviceInfoToEntity(
    const DeviceInfo &device_info, Database::Entities::DeviceEntity &entity) {
  try {
    // ID 변환 - 안전성 강화
    int device_id = 0;
    try {
      ...
      device_id =
          static_cast<int>(std::hash<std::string>{}(device_info.id) % 1000000);
    }
    ...
    // Default values for optional fields to avoid database compatibility issues
    // with NULL
    ...
    if (protocol_id > 0) {
      ...
      throw std::runtime_error("Cannot resolve protocol type: " +
                               device_info.protocol_type);
    }
    ...
    try {
      ...
      auto &logger = LogManager::getInstance();
      logger.Warn("Failed to serialize properties for device " +
                  device_info.id +
    ...
    auto &logger = LogManager::getInstance();
    logger.Error("Failed to convert DeviceInfo to DeviceEntity: " +
                 std::string(e.what()));
    throw;
  }
}
...
Database::Entities::DeviceEntity
BACnetDiscoveryService::ConvertDeviceInfoToEntity(
...
void BACnetDiscoveryService::SetDeviceScheduleRepository(
    std::shared_ptr<Database::Repositories::DeviceScheduleRepository>
        device_schedule_repo) {
...
BACnetDiscoveryService::BACnetDiscoveryService(
    ...
    std::shared_ptr<Database::Repositories::CurrentValueRepository>
        current_value_repo,
    std::shared_ptr<Database::Repositories::DeviceSettingsRepository>
        device_settings_repo,
    std::shared_ptr<WorkerFactory> worker_factory)
    : device_repository_(device_repo), datapoint_repository_(datapoint_repo),
      current_value_repository_(current_value_repo),
      device_settings_repository_(device_settings_repo),
      worker_factory_(worker_factory), is_active_(false),
      is_discovery_active_(false), is_network_scan_active_(false),
      network_scan_running_(false) {

  // 필수 repository 검증
  if (!device_repository_ || !datapoint_repository_) {
    throw std::invalid_argument(
        "DeviceRepository or DataPointRepository is null");
  }
  ...
  logger.Info("  - DeviceRepository: " +
              std::string(device_repository_ ? "OK" : "FAIL"));
  logger.Info("  - DataPointRepository: " +
              std::string(datapoint_repository_ ? "OK" : "FAIL"));
  logger.Info("  - CurrentValueRepository: " +
              std::string(current_value_repository_ ? "OK" : "OPTIONAL"));
  logger.Info("  - DeviceSettingsRepository: " +
              std::string(device_settings_repository_ ? "OK" : "OPTIONAL"));
  logger.Info("  - WorkerFactory: " +
              std::string(worker_factory_ ? "OK" : "OPTIONAL"));
...
BACnetDiscoveryService::~BACnetDiscoveryService() {
  try {
    ...
    try {
      auto &logger = LogManager::getInstance();
      logger.Error("Exception in BACnetDiscoveryService destructor: " +
                   std::string(e.what()));
    } catch (...) {
      // 로깅조차 실패하면 무시
    }
...
void BACnetDiscoveryService::SafeCleanupAllWorkers() {
  ...
  for (const auto &device_id : device_ids) {
    try {
      ...
      try {
        auto &logger = LogManager::getInstance();
        logger.Error("Failed to cleanup worker " + device_id + ": " +
                     std::string(e.what()));
      } catch (...) {
        // 로깅 실패 시 무시
      }
...
bool BACnetDiscoveryService::StopWorkerForDeviceSafe(
  ...
  if (managed_worker_to_stop && managed_worker_to_stop->worker &&
      managed_worker_to_stop->is_running) {
    try {
      auto &logger = LogManager::getInstance();
      ...
      if (status == std::future_status::ready) {
        ...
        logger.Info("Worker stopped " +
                    std::string(success ? "successfully" : "with errors") +
      ...
      auto &logger = LogManager::getInstance();
      logger.Error("Exception stopping worker for device " + device_id + ": " +
                   std::string(e.what()));
      return false;
    }
  }
...
void BACnetDiscoveryService::SyncDeviceSchedules(uint32_t device_id) {
  ...
  if (!base_worker) {
    LogManager::getInstance().Warn(
        "SyncDeviceSchedules: Worker not found for device " + s_device_id);
    return;
  }
  ...
  if (!bacnet_worker) {
    LogManager::getInstance().Warn(
        "SyncDeviceSchedules: Worker is not BACnetWorker");
    return;
  }
  ...
  if (managed_info) {
    if (managed_info->device_info.properties.count("instance_number")) {
      try {
        bacnet_device_id = std::stoul(
            managed_info->device_info.properties.at("instance_number"));
      } catch (...) {
      }
    } else if (managed_info->device_info.properties.count("device_id")) {
      try {
        bacnet_device_id =
            std::stoul(managed_info->device_info.properties.at("device_id"));
      } catch (...) {
      }
    }
  }
  ...
  if (bacnet_device_id == 0) {
    ...
    LogManager::getInstance().Debug(
        "SyncDeviceSchedules: BACnet Device ID not found for " + s_device_id);
  }
  ...
  for (const auto &point : points) {
    try {
      ...
      if (obj_type_str == "17" || obj_type_str == "SCHEDULE") {
        std::string obj_instance_str =
            point.getProtocolParam("object_instance", "0");
        ...
        if (bacnet_driver->ReadSingleProperty(dp, result)) {
          ...
          auto existing =
              device_schedule_repository_->findByDeviceId(device_id);
          bool found = false;
          ...
          LogManager::getInstance().Info("Synced schedule for device " +
                                         s_device_id + " point " +
                                         std::to_string(point.getId()));
        } else {
          LogManager::getInstance().Warn("Failed to read schedule for device " +
                                         s_device_id);
        }
      }
    } catch (const std::exception &e) {
      LogManager::getInstance().Error("Error syncing schedule point: " +
                                      std::string(e.what()));
      continue;
    }
  }
...
void BACnetDiscoveryService::StopDynamicDiscovery() {
  ...
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.Error("Exception during dynamic discovery stop: " +
                 std::string(e.what()));
  }
}
...
std::shared_ptr<BaseDeviceWorker>
BACnetDiscoveryService::CreateWorkerForDevice(const DeviceInfo &device_info) {
  if (!worker_factory_) {
    auto &logger = LogManager::getInstance();
    logger.Error("WorkerFactory is null - cannot create worker for device: " +
                 device_info.id);
    return nullptr;
  }
  ...
  try {
    auto &logger = LogManager::getInstance();
    logger.Info("Creating worker for device: " + device_info.name +
                " (ID: " + device_info.id + ")");
    ...
    try {
      shared_worker =
          std::shared_ptr<BaseDeviceWorker>(unique_worker.release());
    } catch (const std::exception &e) {
    ...
    // ManagedWorker로 감싸서 안전하게 관리
    {
      std::lock_guard<std::mutex> lock(managed_workers_mutex_);
      auto managed_worker =
          std::make_unique<ManagedWorker>(shared_worker, device_info);
    ...
    auto &logger = LogManager::getInstance();
    logger.Error("Exception creating worker for device " + device_info.id +
                 ": " + std::string(e.what()));
    UpdateStatistics("workers_failed", false);
    return nullptr;
  }
...
bool BACnetDiscoveryService::StartWorkerForDevice(
    const std::string &device_id) {
  if (device_id.empty()) {
    auto &logger = LogManager::getInstance();
  ...
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.Error("Exception starting worker for device " + device_id + ": " +
                 std::string(e.what()));
    UpdateStatistics("workers_failed", false);
    return false;
  }
...
bool BACnetDiscoveryService::RemoveWorkerForDevice(
    const std::string &device_id) {
  // 먼저 중지
...
void BACnetDiscoveryService::UpdateStatistics(const std::string &operation,
                                              bool success) {
  try {
...
std::shared_ptr<BaseDeviceWorker>
BACnetDiscoveryService::GetWorkerForDevice(const std::string &device_id) const {
  std::lock_guard<std::mutex> lock(managed_workers_mutex_);
  auto it = managed_workers_.find(device_id);
  if (it != managed_workers_.end()) {
...
BACnetDiscoveryService::ManagedWorker *
BACnetDiscoveryService::GetManagedWorkerInfo(
...
void BACnetDiscoveryService::OnDeviceDiscovered(const DeviceInfo &device) {
  try {
    ...
    try {
      ...
      logger.Error("Exception saving device to database: " +
                   std::string(e.what()));
...
void BACnetDiscoveryService::OnObjectDiscovered(const DataPoint &object) {
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.Debug("BACnet object discovered: " + object.name +
                 " (ID: " + object.id + ")");
    ...
    try {
      ...
      logger.Warn("Invalid device_id in DataPoint, using default: " +
                  object.device_id);
    }
    ...
    auto &logger = LogManager::getInstance();
    logger.Error("Exception in OnObjectDiscovered (single): " +
                 std::string(e.what()));
  }
}

void BACnetDiscoveryService::OnObjectDiscovered(
    uint32_t device_id, const std::vector<DataPoint> &objects) {
  try {
    UpdateStatistics("objects_processed", objects.size());

    auto &logger = LogManager::getInstance();
    logger.Debug("BACnet objects discovered for device " +
                 std::to_string(device_id) + ": " +
                 std::to_string(objects.size()) + " objects");

    if (SaveDiscoveredObjectsToDatabase(device_id, objects)) {
      UpdateStatistics("objects_saved", objects.size());
      logger.Info("Objects saved to database for device " +
                  std::to_string(device_id));
    } else {
      UpdateStatistics("database_errors", false);
      logger.Error("Failed to save objects to database for device " +
                   std::to_string(device_id));
    }
    ...
    auto &logger = LogManager::getInstance();
    logger.Error("Exception in OnObjectDiscovered (multiple): " +
                 std::string(e.what()));
  }
}
...
bool BACnetDiscoveryService::CreateAndStartWorkerForNewDevice(
    const DeviceInfo &device_info) {
  try {
    auto &logger = LogManager::getInstance();
    logger.Info("Creating and starting worker for new device: " +
                device_info.name);
    ...
    logger.Info("Successfully created and started worker for device: " +
                device_info.name);
    return true;

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.Error("Exception in CreateAndStartWorkerForNewDevice: " +
                 std::string(e.what()));
    return false;
  }
}

bool BACnetDiscoveryService::IsDeviceAlreadyManaged(
    const std::string &device_id) const {
  std::lock_guard<std::mutex> lock(managed_workers_mutex_);
...
void BACnetDiscoveryService::UpdateWorkerActivity(
    const std::string &device_id) {
  std::lock_guard<std::mutex> lock(managed_workers_mutex_);
  auto it = managed_workers_.find(device_id);
  if (it != managed_workers_.end()) {
...
bool BACnetDiscoveryService::RegisterToWorker(
    std::shared_ptr<BACnetWorker> worker) {
  try {
    if (!worker) {
      auto &logger = LogManager::getInstance();
...
BACnetDiscoveryService::Statistics
BACnetDiscoveryService::GetStatistics() const {
  std::lock_guard<std::mutex> lock(stats_mutex_);
  return statistics_;
}

bool BACnetDiscoveryService::IsActive() const { return is_active_.load(); }
...
void BACnetDiscoveryService::SetDeviceSettingsRepository(
  ...
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.Info(
        "DeviceSettingsRepository injected into BACnetDiscoveryService");

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.Error("Exception in SetDeviceSettingsRepository: " +
                 std::string(e.what()));
  }
}
...
bool BACnetDiscoveryService::StartNetworkScan(
    const std::string &network_range) {
  try {
    auto &logger = LogManager::getInstance();
    ...
    // 네트워크 스캔 스레드 시작
    network_scan_thread_ = std::make_unique<std::thread>([this,
                                                          network_range]() {
      try {
        auto &logger = LogManager::getInstance();
        ...
        while (network_scan_running_.load()) {
          try {
            ...
            logger.Error("Exception in network scan loop: " +
                         std::string(e.what()));
        ...
        try {
          auto &logger = LogManager::getInstance();
          logger.Error("Exception in network scan thread: " +
                       std::string(e.what()));
        } catch (...) {
          // 로깅 실패 시 무시
        }
...
void BACnetDiscoveryService::StopNetworkScan() {
  try {
    auto &logger = LogManager::getInstance();
    if (!is_network_scan_active_.load()) {
      logger.Info("StopNetworkScan: Scan is not active, nothing to stop");
      return;
    }

    logger.Info("Stopping Network Scan... Clearing flags.");
    network_scan_running_ = false;
    is_network_scan_active_ = false;
...
bool BACnetDiscoveryService::SaveDiscoveredDeviceToDatabase(
  ...
  try {
    ...
    try {
      ...
      auto &logger = LogManager::getInstance();
      logger.Error("Failed to convert device info to entity: " +
                   std::string(e.what()));
      return false;
    }
  ...
  try {
    ...
    for (const auto &object : objects) {
      try {
        ...
        auto &logger = LogManager::getInstance();
        logger.Error("Failed to save individual object " + object.id + ": " +
                     std::string(e.what()));
        all_success = false;
      }
    }
...
bool BACnetDiscoveryService::UpdateCurrentValueInDatabase(
    const std::string &object_id, const TimestampedValue &value) {
  try {
    ...
    auto existing_values =
        current_value_repository_->findByConditions(conditions);
...
int BACnetDiscoveryService::FindDeviceIdInDatabase(uint32_t bacnet_device_id) {
  try {
    ...
    std::vector<QueryCondition> conditions = {QueryCondition(
        "config", "LIKE",
        "%\"bacnet_device_id\":\"" + std::to_string(bacnet_device_id) + "\"%")};
...
std::string
BACnetDiscoveryService::GenerateDataPointId(uint32_t device_id,
  ...
  try {
    ...
    if (obj_type_it != object.protocol_params.end() &&
      ...
      return std::to_string(device_id) + ":" + obj_type_it->second + ":" +
             obj_inst_it->second;
    }
...
std::string
BACnetDiscoveryService::DataTypeToString(PulseOne::Enums::DataType type) {
  switch (type) {
  case DataType::BOOL:
    return "bool";
  case DataType::INT32:
...
std::string
BACnetDiscoveryService::ConvertDataValueToString(const DataValue &value) {
  try {
...
double
BACnetDiscoveryService::ConvertDataValueToDouble(const DataValue &value) {
  try {
    if (std::holds_alternative<bool>(value)) {
...
void BACnetDiscoveryService::HandleError(const std::string &context,
...
int BACnetDiscoveryService::GuessObjectTypeFromName(
  ...
  try {
    ...
    if (std::regex_search(object_name, ai_pattern))
      return 0;
    if (std::regex_search(object_name, ao_pattern))
      return 1;
    if (std::regex_search(object_name, av_pattern))
      return 2;
    if (std::regex_search(object_name, bi_pattern))
      return 3;
    if (std::regex_search(object_name, bo_pattern))
      return 4;
    if (std::regex_search(object_name, bv_pattern))
      return 5;
    if (std::regex_search(object_name, mi_pattern))
      return 13;
    if (std::regex_search(object_name, mo_pattern))
      return 14;
    if (std::regex_search(object_name, mv_pattern))
      return 19;
...
std::string
BACnetDiscoveryService::BACnetAddressToString(const BACNET_ADDRESS &address) {
  try {
    std::stringstream ss;
