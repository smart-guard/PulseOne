...
ROSWorker::ROSWorker(const DeviceInfo& device_info)
    ...
    // Initialize Driver
    driver_ = std::make_unique<ROSDriver>();
    ...
    if (driver_->Initialize(config)) {
         driver_->SetMessageCallback([this](const std::string& topic, const nlohmann::json& msg) {
             this->HandleRosMessage(topic, msg);
         });
    }
...
bool ROSWorker::CheckConnection() {
    ...
    if (!driver_connected && is_connected_.load()) {
        ...
        if (last_success_time_ != std::chrono::system_clock::time_point()) {
            auto diff = std::chrono::duration_cast<std::chrono::seconds>(now - last_success_time_).count();
            if (diff > 60) {
                LogMessage(LogLevel::WARN, "Data Watchdog: No data received from ROS Bridge for " + std::to_string(diff) + " seconds");
            }
        }
    }
...
void ROSWorker::HandleRosMessage(const std::string& topic, const nlohmann::json& msg) {
    std::lock_guard<std::mutex> lock(data_points_mutex_);
    
    auto it = topic_to_points_.find(topic);
    if (it == topic_to_points_.end()) return;
    ...
    for (const auto& point_id_str : it->second) {
        ...
        for (const auto& dp : data_points_) {
            if (dp.id == point_id_str) { target_dp = &dp; break; }
        }
        if (!target_dp) continue;
        ...
        if (!target_dp->mapping_key.empty()) {
            try {
                std::string path = target_dp->mapping_key;
                ...
                if (msg.contains(ptr)) {
                    ...
                    if (json_val.is_number()) val.value = json_val.get<double>();
                    else if (json_val.is_boolean()) val.value = json_val.get<bool>() ? 1.0 : 0.0;
                    else if (json_val.is_string()) val.value = json_val.get<std::string>();
                    else val.value = json_val.dump();
                    value_found = true;
                }
            } catch (...) {}
        }
        ...
        if (!value_found) {
            if (topic.find("battery") != std::string::npos) {
                if (msg.contains("percentage")) { val.value = msg["percentage"].get<double>(); value_found = true; }
                else if (msg.contains("charge")) { val.value = msg["charge"].get<double>(); value_found = true; }
            } else if (topic.find("odom") != std::string::npos) {
                if (msg.contains("pose") && msg["pose"].contains("pose") && msg["pose"]["pose"].contains("position")) {
                    auto pos = msg["pose"]["pose"]["position"];
                    if (target_dp->name.find("X") != std::string::npos) { val.value = pos["x"].get<double>(); value_found = true; }
                    else if (target_dp->name.find("Y") != std::string::npos) { val.value = pos["y"].get<double>(); value_found = true; }
                }
            } else {
                if (msg.is_number()) { val.value = msg.get<double>(); value_found = true; }
                else if (msg.is_boolean()) { val.value = msg.get<bool>() ? 1.0 : 0.0; value_found = true; }
            }
        }
    ...
    if (msg.is_object()) {
        for (auto& [key, value] : msg.items()) {
            ...
            if (!already_mapped && device_info_.is_auto_registration_enabled && (value.is_number() || value.is_boolean())) {
                ...
                if (new_id > 0) {
                    ...
                    val.quality = PulseOne::Enums::DataQuality::GOOD;
                    val.value = value.is_number() ? value.get<double>() : (value.get<bool>() ? 1.0 : 0.0);
                    points_to_send.push_back(val);
                }
            }
        }
