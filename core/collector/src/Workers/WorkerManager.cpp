...
#if defined(HAS_BACNET) && !defined(_WIN32) && !defined(__linux__)
...
WorkerManager::WorkerManager() {
  try {
...
#if defined(HAS_BACNET) && !defined(_WIN32) && !defined(__linux__)
    // Initialize BACnet Discovery Service (Only if not Windows/Linux modular
    // build)
    ...
    LogManager::getInstance().Error("WorkerManager initialization failed: " +
                                    std::string(e.what()));
  }
}
...
bool WorkerManager::WriteDataPoint(const std::string &device_id,
  ...
  if (scheduler_)
    return scheduler_->WriteDataPoint(device_id, point_id, value);
  return false;
}
...
bool WorkerManager::ControlDigitalOutput(const std::string &device_id,
                                         ...
                                         bool enable) {
  if (scheduler_)
    return scheduler_->ControlDigitalOutput(device_id, output_id, enable);
  return false;
}
...
bool WorkerManager::ControlAnalogOutput(const std::string &device_id,
                                        ...
                                        double value) {
  if (scheduler_)
    return scheduler_->ControlAnalogOutput(device_id, output_id, value);
  return false;
}

bool WorkerManager::ControlOutput(const std::string &device_id,
                                  const std::string &output_id, bool enable) {
  return ControlDigitalOutput(device_id, output_id, enable);
}

bool WorkerManager::ControlDigitalDevice(const std::string &device_id,
                                         const std::string &device_id_target,
                                         bool enable) {
  if (scheduler_)
    return scheduler_->ControlDigitalDevice(device_id, device_id_target,
                                            enable);
  return false;
}
...
WorkerManager::DiscoverDevicePoints(const std::string &device_id) {
  if (!registry_)
    return {};
...
bool WorkerManager::StartNetworkScan(const std::string &protocol,
  ...
  if (protocol == "BACNET" || protocol == "BACnet") {
    ...
    LogManager::getInstance().Warn("WorkerManager: BACnet Discovery Service "
                                   "not available in this build (or modular)");
    return false;
#endif
    ...
    LogManager::getInstance().Info(
        "WorkerManager: Starting MQTT Smart Discovery for broker: " + range);
    // MQTT Discovery is primarily passive through MQTTWorker auto-registration
    // logic. Returning true allows the frontend to proceed with result polling.
    return true;
  }

  LogManager::getInstance().Warn(
      "WorkerManager: Unsupported partial scan protocol: " + protocol);
  return false;
}
...
nlohmann::json
WorkerManager::GetWorkerStatus(const std::string &device_id) const {
  if (monitor_)
    return monitor_->GetWorkerStatus(device_id);
  return {{"error", "Monitor not initialized"}};
}

nlohmann::json WorkerManager::GetWorkerList() const {
  if (monitor_)
    return monitor_->GetWorkerList();
  return json::array();
}

nlohmann::json WorkerManager::GetManagerStats() const {
  if (monitor_)
    return monitor_->GetDetailedStatistics();
  return json::object();
}

size_t WorkerManager::GetActiveWorkerCount() const {
  if (registry_)
    return registry_->GetWorkerCount();
  return 0;
}

bool WorkerManager::HasWorker(const std::string &device_id) const {
  if (registry_)
    return registry_->HasWorker(device_id);
  return false;
}
