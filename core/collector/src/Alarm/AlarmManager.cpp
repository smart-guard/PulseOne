...
AlarmManager::AlarmManager()
    : initialized_(false), total_evaluations_(0), alarms_raised_(0),
      alarms_cleared_(0), next_occurrence_id_(1), js_runtime_(nullptr),
      js_context_(nullptr) {}
...
bool AlarmManager::initialize() {
  ...
  try {
    ...
    logger.log("alarm", LogLevel::DEBUG,
               "🎯 AlarmManager 초기화 시작 (순수 알람 모드)");

    // ❌ 제거: Redis 클라이언트 초기화
    // initializeClients();

    if (!initScriptEngine()) {
      logger.log("alarm", LogLevel::WARN,
                 "JavaScript 엔진 초기화 실패 (스크립트 알람 비활성화)");
    }

    initializeData();
    initialized_ = true;

    logger.log("alarm", LogLevel::INFO,
               "✅ AlarmManager 초기화 완료 (외부 의존성 없음)");
    return true;

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ AlarmManager 초기화 실패: " + std::string(e.what()));
    initialized_ = false;
    return false;
  }
}

AlarmManager::~AlarmManager() { shutdown(); }
...
void AlarmManager::initializeData() {
  try {
    ...
    // 🎯 Repository에서 알람 규칙 로드
    auto repo =
        Database::RepositoryFactory::getInstance().getAlarmRuleRepository();
    if (!repo) {
      LogManager::getInstance().log(
          "alarm", LogLevel::LOG_ERROR,
          "❌ AlarmRuleRepository를 가져올 수 없습니다.");
      return;
    }

    auto entities = repo->findAllEnabled();
    ...
    for (const auto &entity : entities) {
      ...
      rule.tenant_id = entity.getTenantId();
      rule.name = entity.getName();
      rule.description = entity.getDescription();

      // Target Type 변환
      auto targetType = entity.getTargetType();
      if (targetType ==
          Database::Entities::AlarmRuleEntity::TargetType::DATA_POINT) {
        rule.target_type = "data_point";
      } else if (targetType == Database::Entities::AlarmRuleEntity::TargetType::
                                   VIRTUAL_POINT) {
        rule.target_type = "virtual_point";
      } else if (targetType ==
                 Database::Entities::AlarmRuleEntity::TargetType::GROUP) {
        rule.target_type = "group";
      }

      rule.target_id = entity.getTargetId().value_or(0);

      // Alarm Type 변환
      auto alarmType = entity.getAlarmType();
      if (alarmType == Database::Entities::AlarmRuleEntity::AlarmType::ANALOG) {
      ...
      // Analog Limits
      rule.analog_limits.high_high = entity.getHighHighLimit();
      rule.analog_limits.high = entity.getHighLimit();
      rule.analog_limits.low = entity.getLowLimit();
      rule.analog_limits.low_low = entity.getLowLowLimit();
      rule.analog_limits.deadband = entity.getDeadband();

      // 기타 속성
      rule.is_enabled = entity.isEnabled();
      // rule.auto_clear = entity.isAutoClear(); // AlarmRule struct에
      // auto_clear가 있는지 확인 필요, 없으면 생략
      rule.condition_script = entity.getConditionScript();
      rule.message_template = entity.getMessageTemplate();
      rule.severity = entity.getSeverity(); // 문자열 그대로 복사
    ...
    logger.log("alarm", LogLevel::INFO,
               "✅ 알람 초기 데이터 로드 완료: " +
                   std::to_string(alarm_rules_.size()) + "개 규칙");

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ 초기 데이터 로드 실패: " + std::string(e.what()));
  }
}
...
bool AlarmManager::initScriptEngine() {
  ...
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ JavaScript 엔진 초기화 실패: " + std::string(e.what()));
    return false;
  }
#else
  return false;
...
AlarmManager::evaluateForMessage(const DeviceDataMessage &msg) {
  ...
  try {
    ...
    LogManager::getInstance().log(
        ...
        "AlarmManager: Delegating to AlarmEngine for " +
            std::to_string(msg.points.size()) + " points");
    events = alarm_engine.evaluateForMessage(msg);
    LogManager::getInstance().log("alarm", PulseOne::Enums::LogLevel::DEBUG,
                                  "AlarmManager: AlarmEngine returned " +
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::INFO,
               "🎯 알람 평가 완료: " + std::to_string(events.size()) +
                   "개 이벤트 생성 (외부 발송은 호출자 담당)");

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ 메시지 평가 실패: " + std::string(e.what()));
  }
...
void AlarmManager::enhanceAlarmEventWithBusinessLogic(
    AlarmEvent &event, const DeviceDataMessage &msg) {
  try {
    // 기본 메타데이터 설정
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::WARN,
               "⚠️ 알람 이벤트 강화 실패: " + std::string(e.what()));
  }
}
...
void AlarmManager::adjustSeverityByBusinessRules(AlarmEvent &event) {
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ 심각도 조정 실패: " + std::string(e.what()));
  }
}

void AlarmManager::addLocationAndContext(AlarmEvent &event,
                                         const DeviceDataMessage &msg) {
  try {
    // 🎯 위치 정보 상세화
    ...
    std::string context =
        " [위치: " + event.location + ", 시간: " + std::string(time_str) + "]";
    event.message += context;

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::WARN,
               "⚠️ 위치/컨텍스트 추가 실패: " + std::string(e.what()));
  }
}
...
void AlarmManager::generateLocalizedMessage(AlarmEvent &event) {
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::WARN,
               "⚠️ 지역화 메시지 생성 실패: " + std::string(e.what()));
  }
}
...
void AlarmManager::analyzeContinuousAlarmPattern(AlarmEvent &event) {
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::DEBUG,
               "🔍 연속 알람 패턴 분석 완료: rule_id=" +
                   std::to_string(event.rule_id));

  } catch (const std::exception &e) {
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ 연속 알람 패턴 분석 실패: " + std::string(e.what()));
  }
}
...
void AlarmManager::applyCategorySpecificRules(AlarmEvent &event) {
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ 카테고리별 규칙 적용 실패: " + std::string(e.what()));
  }
}
...
AlarmEvaluation AlarmManager::evaluateRule(const AlarmRule &rule,
...
AlarmEvaluation AlarmManager::evaluateAnalogAlarm(const AlarmRule &rule,
                                                  double value) {
  if (!initialized_.load()) {
    return {};
  }
...
AlarmEvaluation AlarmManager::evaluateDigitalAlarm(const AlarmRule &rule,
                                                   bool state) {
  if (!initialized_.load()) {
    return {};
  }
...
AlarmEvaluation AlarmManager::evaluateScriptAlarm(const AlarmRule &rule,
                                                  const json &context) {
  if (!initialized_.load()) {
    return {};
  }
...
std::optional<int64_t> AlarmManager::raiseAlarm(const AlarmRule &rule,
...
bool AlarmManager::clearAlarm(int64_t occurrence_id,
                              const DataValue &clear_value) {
  if (!initialized_.load()) {
    return false;
  }
...
bool AlarmManager::acknowledgeAlarm(int64_t occurrence_id, int user_id,
  ...
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::INFO,
               "✅ 알람 확인: " + std::to_string(occurrence_id) + " by user " +
...
std::string AlarmManager::generateMessage(const AlarmRule &rule,
                                          const DataValue &value,
...
std::string AlarmManager::generateAdvancedMessage(const AlarmRule &rule,
                                                  const AlarmEvent &event) {
  return event.message;
}

std::string AlarmManager::generateCustomMessage(const AlarmRule &rule,
                                                const DataValue &value) {
...
Database::Entities::AlarmRuleEntity
AlarmManager::convertToEntity(const AlarmRule &rule) {
  ...
  if (rule.target_type == "data_point") {
    ...
    entity.setTargetType(
        Database::Entities::AlarmRuleEntity::TargetType::VIRTUAL_POINT);
...
std::vector<AlarmRule>
AlarmManager::getAlarmRulesForPoint(int point_id,
                                    const std::string &point_type) const {
  // TODO: 실제 포인트별 알람 규칙 검색 구현
  return {};
}

std::string AlarmManager::interpolateTemplate(
  ...
  try {
    ...
    auto &logger = LogManager::getInstance();
    logger.log("alarm", LogLevel::LOG_ERROR,
               "❌ 템플릿 변수 치환 실패: " + std::string(e.what()));
  }
