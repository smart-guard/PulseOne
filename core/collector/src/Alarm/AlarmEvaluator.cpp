...
AlarmEvaluator::AlarmEvaluator(PulseOne::Scripting::ScriptExecutor& executor, AlarmStateCache& state_cache)
    : executor_(executor), state_cache_(state_cache) {}

AlarmEvaluation AlarmEvaluator::evaluate(const Database::Entities::AlarmRuleEntity& rule, 
                                        const PulseOne::Structs::DataValue& value) {
    // DataValue(variant)에서 double 값 추출
    double dbl_value = std::visit([](auto&& arg) -> double {
    ...
    if (rule.getAlarmType() == Database::Entities::AlarmRuleEntity::AlarmType::ANALOG) {
        return evaluateAnalog(rule, dbl_value);
    } else if (rule.getAlarmType() == Database::Entities::AlarmRuleEntity::AlarmType::DIGITAL) {
        bool bool_value = (dbl_value != 0.0);
        return evaluateDigital(rule, bool_value);
    } else if (rule.getAlarmType() == Database::Entities::AlarmRuleEntity::AlarmType::SCRIPT) {
...
AlarmEvaluation AlarmEvaluator::evaluateAnalog(const Database::Entities::AlarmRuleEntity& rule, double value) {
    AlarmEvaluation eval;
    eval.rule_id = rule.getId();
    ...
    double high_limit = rule.getHighLimit().value_or(1e18);
    double low_limit = rule.getLowLimit().value_or(-1e18);
...
AlarmEvaluation AlarmEvaluator::evaluateDigital(const Database::Entities::AlarmRuleEntity& rule, bool value) {
    AlarmEvaluation eval;
    eval.rule_id = rule.getId();
    eval.tenant_id = rule.getTenantId();
    eval.timestamp = std::chrono::system_clock::now();
...
AlarmEvaluation AlarmEvaluator::evaluateScript(const Database::Entities::AlarmRuleEntity& rule, const nlohmann::json& context) {
    AlarmEvaluation eval;
    eval.rule_id = rule.getId();
    eval.tenant_id = rule.getTenantId();
    eval.timestamp = std::chrono::system_clock::now();
