...
VirtualPointEngine::~VirtualPointEngine() { shutdown(); }
...
VirtualPointEngine::calculateForMessage(
  ...
  for (int vp_id : affected_vps) {
    ...
    if (calc_result.success) {
      ...
      // [BUG #23 FIX] pass the real tenant_id from the VP definition
      triggerAlarmEvaluation(vp_id, tv.value, vp_opt->tenant_id);
    }
  }
  return results;
}

CalculationResult VirtualPointEngine::calculate(int vp_id,
  ...
  try {
    ...
    LogManager::getInstance().log(
        "VirtualPointEngine", LogLevel::LOG_ERROR,
        "VP " + std::to_string(vp_id) +
            " Calculation Error: " + std::string(e.what()));
...
CalculationResult
VirtualPointEngine::calculateWithFormula(const std::string &formula,
                                         const nlohmann::json &input_values) {
  CalculationResult result;
  try {
    result.value =
        dataValueToJson(executor_.evaluateRaw(formula, input_values));
    result.success = true;
  } catch (const std::exception &e) {
    result.success = false;
    result.error_message = e.what();
...
PulseOne::Structs::DataValue
VirtualPointEngine::jsonToDataValue(const nlohmann::json &j) {
  if (j.is_null())
...
nlohmann::json VirtualPointEngine::collectInputValues(
    const VirtualPointDef &vp,
    const PulseOne::Structs::DeviceDataMessage &msg) {
  nlohmann::json inputs;
  if (!vp.input_points.contains("inputs") ||
      !vp.input_points["inputs"].is_array())
    return inputs;
...
std::optional<double> VirtualPointEngine::getPointValue(
    const std::string &point_id,
    const PulseOne::Structs::DeviceDataMessage &msg) {
  int id = std::stoi(point_id);
  for (const auto &dp : msg.points) {
    if (dp.point_id == id) {
      // [BUG #23b FIX] All numeric / bool variants properly converted
      if (std::holds_alternative<double>(dp.value))
        return std::get<double>(dp.value);
      if (std::holds_alternative<int>(dp.value))
        return static_cast<double>(std::get<int>(dp.value));
      if (std::holds_alternative<bool>(dp.value))
        return std::get<bool>(dp.value) ? 1.0 : 0.0;
      if (std::holds_alternative<std::string>(dp.value)) {
        try {
          return std::stod(std::get<std::string>(dp.value));
        } catch (...) {
          return std::nullopt; // non-numeric string: skip, don't feed 0.0
        }
      }
    }
  }
...
void VirtualPointEngine::updateVirtualPointStats(
    int vp_id, const CalculationResult &result) {
  std::lock_guard<std::mutex> lock(stats_mutex_);
...
void VirtualPointEngine::triggerAlarmEvaluation(
    int vp_id, const PulseOne::Structs::DataValue &value, int tenant_id) {
  // VP 계산 결과를 AlarmManager에 실제로 전달.
  // VirtualPoint는 device_id가 없으므로 synthetic DeviceDataMessage를 구성.
  // tenant_id는 호출자(calculateForMessage)에서 vp_opt->tenant_id로 전달받음.
  try {
    ...
    PulseOne::Structs::DeviceDataMessage synthetic_msg;
    synthetic_msg.device_id = "virtual";
    synthetic_msg.tenant_id = tenant_id; // 실제 테넌트 ID 사용
    synthetic_msg.timestamp = now;
    ...
    tv.quality = PulseOne::Enums::DataQuality::GOOD;
    tv.is_virtual_point = true;
    synthetic_msg.points.push_back(tv);

    alarm_manager.evaluateForMessage(synthetic_msg);

    LogManager::getInstance().log(
        "VirtualPointEngine", LogLevel::DEBUG,
        "VP " + std::to_string(vp_id) +
            " 알람 평가 완료 (tenant=" + std::to_string(tenant_id) + ")");
  } catch (const std::exception &e) {
    LogManager::getInstance().log(
        "VirtualPointEngine", LogLevel::WARN,
        "VP " + std::to_string(vp_id) +
