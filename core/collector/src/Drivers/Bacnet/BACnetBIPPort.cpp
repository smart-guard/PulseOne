    ...
    unsigned pdu_len) 
{
    ...
    if (!g_pPulseOneBACnetServiceManager) return -1;
    ...
    if (dest->len == 6) {
        ...
        LogManager::getInstance().Debug(
            "[BIP] Sending PDU to " + std::string(ip_str) + ":" + std::to_string(port) + 
            " (len: " + std::to_string(pdu_len) + ")"
        );
...
uint16_t bip_receive(
    ...
    unsigned timeout) 
{
    if (!g_pPulseOneBACnetServiceManager) return 0;
    ...
    if (result <= 0) return 0; // Timeout or error
    ...
    if (received > 0 && src) {
        ...
        LogManager::getInstance().Debug(
            "[BIP] Received PDU from " + std::string(src_ip) + ":" + std::to_string(src_port) + 
            " (len: " + std::to_string(received) + ")"
        );
    }
