...
struct HttpRestDriver::CurlContext {
    ...
    CurlContext() : curl_handle(nullptr), headers_list(nullptr) {}
    ~CurlContext() {
        if (headers_list) {
...
HttpRestDriver::HttpRestDriver()
    ...
    , http_method_("GET")
    , headers_()
    , request_body_template_()
    , timeout_ms_(5000)
    , retry_count_(3)
    , is_connected_(false)
    , status_(DriverStatus::UNINITIALIZED)
    , driver_statistics_("HTTP_REST")
    , curl_context_(nullptr)
{
...
bool HttpRestDriver::Initialize(const DriverConfig& config) {
    try {
        ...
        LogManager::getInstance().Info("[HTTP/REST] Driver initialized successfully");
        return true;
...
bool HttpRestDriver::Connect() {
    try {
        ...
        LogManager::getInstance().Info("[HTTP/REST] Testing connection to: " + endpoint_url_);
        ...
        if (!response.empty() || curl_context_->curl_handle) {
            is_connected_ = true;
...
bool HttpRestDriver::IsConnected() const {
...
DriverStatus HttpRestDriver::GetStatus() const {
    return status_.load();
}
...
bool HttpRestDriver::WriteValue(const DataPoint& point, const DataValue& value) {
    if (!IsConnected()) {
        SetError("Driver not connected");
        return false;
    }
...
bool HttpRestDriver::ParseDriverConfig(const DriverConfig& config) {
    ...
    if (config.properties.count("headers")) {
        try {
            ...
            LogManager::getInstance().Warn("[HTTP/REST] Failed to parse headers JSON");
        }
    }
...
bool HttpRestDriver::InitializeHttpClient() {
    ...
    // Set write callback
    curl_easy_setopt(curl_context_->curl_handle, CURLOPT_WRITEFUNCTION, WriteCallback);
    curl_easy_setopt(curl_context_->curl_handle, CURLOPT_WRITEDATA, &curl_context_->response_buffer);
    
    // Set headers
    for (const auto& [key, value] : headers_) {
        std::string header = key + ": " + value;
        curl_context_->headers_list = curl_slist_append(curl_context_->headers_list, header.c_str());
    }
    
    if (curl_context_->headers_list) {
        curl_easy_setopt(curl_context_->curl_handle, CURLOPT_HTTPHEADER, curl_context_->headers_list);
    }
    
    return true;
}

void HttpRestDriver::CleanupHttpClient() {
    curl_context_.reset();
}
...
std::string HttpRestDriver::ExecuteHttpRequest(const std::string& url,
    ...
    if (method == "POST") {
        ...
        curl_easy_setopt(curl_context_->curl_handle, CURLOPT_POSTFIELDS, body.c_str());
    } else if (method == "PUT") {
        curl_easy_setopt(curl_context_->curl_handle, CURLOPT_CUSTOMREQUEST, "PUT");
        curl_easy_setopt(curl_context_->curl_handle, CURLOPT_POSTFIELDS, body.c_str());
    } else if (method == "DELETE") {
        curl_easy_setopt(curl_context_->curl_handle, CURLOPT_CUSTOMREQUEST, "DELETE");
    } else if (method == "HEAD") {
...
DataValue HttpRestDriver::ExtractValueFromJson(const std::string& json_response,
    ...
    try {
        ...
        // Simple JSONPath implementation (supports basic paths like $.field or $.nested.field)
        json value = j;
        ...
        LogManager::getInstance().Error("[HTTP/REST] JSON extraction failed: " + std::string(e.what()));
        return 0.0;  // Default value
    }
}

std::string HttpRestDriver::BuildRequestBody(const DataPoint& point, const DataValue& value) {
...
void HttpRestDriver::SetError(const std::string& message, Structs::ErrorCode code) {
    std::lock_guard<std::mutex> lock(error_mutex_);
...
void HttpRestDriver::UpdateStatistics(const std::string& operation, bool success, double duration_ms) {
    if (operation == "read") {
...
size_t HttpRestDriver::WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) {
