...
bool OPCUADriver::Initialize(const DriverConfig& config) {
    ...
    LogManager::getInstance().Info("OPC-UA Driver initialized. Target: " + endpoint_url_);
    UpdateStatus(DriverStatus::STOPPED);
    return true;
}

bool OPCUADriver::WriteValue(const PulseOne::Structs::DataPoint& point, const PulseOne::Structs::DataValue& value) {
    if (!IsConnected()) return false;
    std::lock_guard<std::mutex> lock(client_mutex_);
    
    std::string nodeIdStr = point.address_string;
    if (nodeIdStr.empty()) nodeIdStr = point.name;
    ...
    if (retval == UA_STATUSCODE_GOOD) {
        ...
        LogManager::getInstance().Error("Write failed: " + std::string(UA_StatusCode_name(retval)));
        return false;
    }
}
...
std::vector<PulseOne::Structs::DataPoint> OPCUADriver::DiscoverPoints() {
    ...
    if (!IsConnected()) {
        LogManager::getInstance().Error("[OPC-UA] Cannot discover points: Not connected");
        return points;
    }
    
    std::lock_guard<std::mutex> lock(client_mutex_);
    LogManager::getInstance().Info("[OPC-UA] Starting Node Discovery (Root=Objects)");
    
    // Browse Objects Folder
    BrowseRecursive(UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER), points, 0);
    
    LogManager::getInstance().Info("[OPC-UA] Discovery Completed. Found " + std::to_string(points.size()) + " variables.");
    return points;
}

void OPCUADriver::BrowseRecursive(UA_NodeId nodeId, std::vector<PulseOne::Structs::DataPoint>& points, int depth) {
    if (depth > MAX_BROWSE_DEPTH) {
        // LogManager::getInstance().Warn("[OPC-UA] Max browse depth reached, stopping recursion on this branch.");
        return;
    }
            ...
            for (size_t j = 0; j < bResp.results[i].referencesSize; ++j) {
                ...
                if (ref->nodeClass == UA_NODECLASS_VARIABLE) {
                    ...
                    if(targetNode.identifierType == UA_NODEIDTYPE_STRING) {
                        nodeIdStr = "ns=" + std::to_string(targetNode.namespaceIndex) + ";s=" + 
                                    std::string((char*)targetNode.identifier.string.data, targetNode.identifier.string.length);
                    } else if(targetNode.identifierType == UA_NODEIDTYPE_NUMERIC) {
                        nodeIdStr = "ns=" + std::to_string(targetNode.namespaceIndex) + ";i=" + 
                                    std::to_string(targetNode.identifier.numeric);
                    }
                    
                    if(!nodeIdStr.empty()) {
                        p.name = std::string((char*)ref->displayName.text.data, ref->displayName.text.length);
...
bool OPCUADriver::Connect() {
    ...
    // Log connecting
    LogManager::getInstance().Info("Connecting to OPC UA Server: " + endpoint_url_);
    
    UA_StatusCode retval;
    // Authenticate
    if (!username_.empty()) {
        LogManager::getInstance().Info("Authenticating with User: " + username_);
        retval = UA_Client_connectUsername(client_, endpoint_url_.c_str(), username_.c_str(), password_.c_str());
    } else {
        retval = UA_Client_connect(client_, endpoint_url_.c_str());
    }
    
    if(retval != UA_STATUSCODE_GOOD) {
        SetError("Connection failed: " + std::string(UA_StatusCode_name(retval)), PulseOne::Structs::ErrorCode::CONNECTION_FAILED);
        UA_Client_delete(client_);
        client_ = nullptr;
        return false;
    }
    ...
    // 1. Identify new items to monitor
    {
        ...
        for (const auto& point : points) {
            ...
            if (monitored_nodes_.find(nodeIdStr) == monitored_nodes_.end()) {
                ...
                // NOTE: We could check if it's already in pending list to avoid duplicates,
                // but SyncMonitoredItems deals with unique set anyway or we optimize here.
                bool already_pending = false;
                for(const auto& p : pending_monitored_items_) {
                    if(p == nodeIdStr) { already_pending = true; break; }
                }
                if(!already_pending) {
...
void OPCUADriver::CreateSubscription() {
    ...
    UA_CreateSubscriptionResponse response = UA_Client_Subscriptions_create(client_, request, this, NULL, NULL);

    if (response.responseHeader.serviceResult == UA_STATUSCODE_GOOD) {
        subscription_id_ = response.subscriptionId;
        LogManager::getInstance().Info("[OPC-UA] Subscription created. ID: " + std::to_string(subscription_id_));
    } else {
...
void OPCUADriver::SyncMonitoredItems() {
    ...
    for (const auto& nodeIdStr : new_items) {
        ...
        // Here we pass nodeIdStr as context for this specific item so we know which one updated
        // Note: we must allocate string memory that persists.
        // For simplicity, let's look up NodeID in cache or map key if possible.
        // Better: Pass a heap-allocated string pointer.
        std::string* contextStr = new std::string(nodeIdStr);

        UA_MonitoredItemCreateResult monResult = UA_Client_MonitoredItems_createDataChange(
            client_, subscription_id_, UA_TIMESTAMPSTORETURN_BOTH, 
            monRequest, contextStr, DataChangeNotificationCallback, NULL);

        if (monResult.statusCode == UA_STATUSCODE_GOOD) {
            monitored_nodes_.insert(nodeIdStr);
            // LogManager::getInstance().Info("[OPC-UA] Monitored Item created: " + nodeIdStr);
        } else {
            LogManager::getInstance().Error("[OPC-UA] Failed to monitor: " + nodeIdStr);
...
// Static Callback
void OPCUADriver::DataChangeNotificationCallback(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    // Ideally, we should have passed 'this' to CreateSubscription or set user context.
    // open62541 allows getting client context.
    
    // Workaround: We need access to 'value_cache_'. 
    // Since this is a plugin, we might need a way to link back.
    // For now, let's assume we can get the driver instance via a static pointer if we were a singleton, 
    // but we are an instance.
    
    // Wait, monContext IS the string pointer we passed!
    std::string* nodeIdStr = (std::string*)monContext;
    if (!nodeIdStr || !value) return;
    
    // We also need access to the driver instance to lock the mutex and update cache.
    // This is tricky in C callbacks without a "User Data" passed to Validating.
    // Ah, 'subContext' is passed! We passed NULL in CreateSubscription.
    // We should fix CreateSubscription to pass 'this'.
    ...
    if(!driver) return; // Can't update if we don't know the driver
...
bool dummy_prevent_compile_error_helper = false; // Placeholder to ensure valid C++ syntax block ending before SetError implementation

void OPCUADriver::SetError(const std::string& message, PulseOne::Structs::ErrorCode code) {
