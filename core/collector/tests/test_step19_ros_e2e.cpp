...
public:
  MockRosBridgeServer(int port)
      : port_(port), running_(false), server_fd_(-1), client_fd_(-1) {}

  ~MockRosBridgeServer() { Stop(); }
...
protected:
  void SetUp() override {
    ...
    DbLib::DatabaseManager::getInstance().initialize(dbConfig); // Mock DB
...
TEST_F(RosE2ETest, ConnectionAndDataFlow) {
  ...
  // Since we can't easily peek into the pipeline in this strict test env
  // without mocking everything, we rely on the fact that no crash occurred and
  // logs would show "Processing" Also, InfluxClient would log specific
  // messages.
