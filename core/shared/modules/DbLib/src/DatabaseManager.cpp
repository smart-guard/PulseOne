...
bool DatabaseManager::initialize(const DatabaseConfig& config, IDbLogger* logger) {
...
bool DatabaseManager::doInitialize() {
    ...
    if (enabled_databases_[DatabaseType::POSTGRESQL]) {
        if (!connectPostgres()) log(2, "Failed to connect to PostgreSQL");
    }
#endif

#ifdef HAS_MYSQL
    if (enabled_databases_[DatabaseType::MYSQL]) {
        if (!connectMySQL()) log(2, "Failed to connect to MySQL");
    }
#endif

#if HAS_MSSQL
    if (enabled_databases_[DatabaseType::MSSQL]) {
        if (!connectMSSQL()) log(2, "Failed to connect to MSSQL");
    }
#endif
...
bool DatabaseManager::connectMSSQL() {
    if (SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &mssql_env_) != SQL_SUCCESS) return false;
...
bool DatabaseManager::executeQueryMSSQL(const std::string& query, 
    ...
    SQLHSTMT stmt;
    if (SQLAllocHandle(SQL_HANDLE_STMT, mssql_conn_, &stmt) != SQL_SUCCESS) return false;
    ...
    while (SQLFetch(stmt) == SQL_SUCCESS) {
        ...
        for (int i = 1; i <= col_count; ++i) {
            ...
            if (SQLGetData(stmt, i, SQL_C_CHAR, buffer, sizeof(buffer), &indicator) == SQL_SUCCESS) {
                row.push_back(indicator == SQL_NULL_DATA ? "NULL" : reinterpret_cast<char*>(buffer));
            } else row.push_back("");
        }
...
bool DatabaseManager::executeQueryMSSQL(const std::string&, std::vector<std::vector<std::string>>&) { return false; }
bool DatabaseManager::executeNonQueryMSSQL(const std::string&) { return false; }
...
bool DatabaseManager::connectRedis() {
    try {
        ...
        return redis_client_->connect(current_config_.redis_host, current_config_.redis_port, current_config_.redis_pass);
    } catch (...) { return false; }
}
...
std::map<std::string, std::string> DatabaseManager::getRedisInfo() { return {}; }
        ...
        case DatabaseType::POSTGRESQL: {
            try {
                ...
                for (const auto& row : pg_result) {
                    std::vector<std::string> row_data;
                    for (const auto& field : row) row_data.push_back(field.c_str());
                    results.push_back(row_data);
                }
                result = true;
            } catch (...) { result = false; }
...
#endif
#ifdef HAS_MYSQL
        case DatabaseType::MYSQL: result = executeQueryMySQL(query, results); break;
#endif
        case DatabaseType::MSSQL: result = executeQueryMSSQL(query, results); break;
...
bool DatabaseManager::executeNonQuery(const std::string& query) {
    switch (primary_rdb_) {
        case DatabaseType::SQLITE: return executeNonQuerySQLite(query);
#ifdef HAS_POSTGRESQL
        case DatabaseType::POSTGRESQL: return executeNonQueryPostgres(query);
#endif
#ifdef HAS_MYSQL
        case DatabaseType::MYSQL: return executeNonQueryMySQL(query);
#endif
        case DatabaseType::MSSQL: return executeNonQueryMSSQL(query);
...
void DatabaseManager::disconnectAll() {
    if (sqlite_conn_) { sqlite3_close(sqlite_conn_); sqlite_conn_ = nullptr; }
#ifdef HAS_POSTGRESQL
    pg_conn_.reset();
#endif
#ifdef HAS_MYSQL
    if (mysql_conn_) { mysql_close(mysql_conn_); mysql_conn_ = nullptr; }
#endif
#if HAS_MSSQL
    if (mssql_conn_) { SQLDisconnect(mssql_conn_); SQLFreeHandle(SQL_HANDLE_DBC, mssql_conn_); mssql_conn_ = nullptr; }
    if (mssql_env_) { SQLFreeHandle(SQL_HANDLE_ENV, mssql_env_); mssql_env_ = nullptr; }
#endif
    disconnectRedis();
}
...
std::string DatabaseManager::buildConnectionString(DatabaseType type) {
    switch (type) {
        case DatabaseType::POSTGRESQL:
            return "host=" + current_config_.pg_host + " port=" + std::to_string(current_config_.pg_port) + " dbname=" + current_config_.pg_db;
        case DatabaseType::MSSQL:
            return "Driver={ODBC Driver 17 for SQL Server};Server=" + current_config_.mysql_host + ";"; // Reusing mysql_host as generic server host
        default: return "";
    }
}
...
void DatabaseManager::log(int level, const std::string& message) {
    if (logger_) logger_->log("DbLib", level, message);
}
