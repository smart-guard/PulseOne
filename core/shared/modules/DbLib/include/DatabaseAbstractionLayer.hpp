...
public:
  ...
  virtual std::string
  buildUpsertQuery(const std::string &table_name,
                   const std::vector<std::string> &columns,
                   const std::vector<std::string> &primary_keys) const = 0;

  // CREATE TABLE specific adaptation
  virtual std::string
  adaptCreateTableQuery(const std::string &base_query) const = 0;
...
public:
  ...
  std::string
  adaptCreateTableQuery(const std::string &base_query) const override;
...
private:
  ...
  std::vector<std::map<std::string, std::string>>
  convertToMapResults(const std::vector<std::vector<std::string>> &raw_results,
  ...
  getTableColumnsFromSchema(const std::string &table_name);
