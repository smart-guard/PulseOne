 ...
 * - DataVariant 타입 지원 (bool, int16, uint16, int32, uint32, float, double,
 * string)
 ...
 *     value_type VARCHAR(10),      -- bool, int16, uint16, int32, uint32,
 * float, double, string quality_code INTEGER, quality VARCHAR(20),
...
public:
  ...
  CurrentValueEntity(int point_id,
                     const PulseOne::BasicTypes::DataVariant &value);
  ...
  json toJson() const override {
    ...
    try {
      ...
      // 타임스탬프들 (ISO 8601 형식)
      j["value_timestamp"] =
          PulseOne::Utils::TimestampToDBString(value_timestamp_);
      j["quality_timestamp"] =
          PulseOne::Utils::TimestampToDBString(quality_timestamp_);
      j["last_log_time"] = PulseOne::Utils::TimestampToDBString(last_log_time_);
      j["last_read_time"] =
          PulseOne::Utils::TimestampToDBString(last_read_time_);
      j["last_write_time"] =
          PulseOne::Utils::TimestampToDBString(last_write_time_);
      ...
      if (logger_)
        logger_->Error("CurrentValueEntity::toJson failed: " +
                       std::string(e.what()));
    }
    return j;
  }
  ...
  bool fromJson(const json &data) override {
    try {
      if (data.contains("point_id"))
        point_id_ = data["point_id"];
      if (data.contains("current_value"))
        current_value_ = data["current_value"];
      if (data.contains("raw_value"))
        raw_value_ = data["raw_value"];
      if (data.contains("value_type"))
        value_type_ = data["value_type"];
      if (data.contains("quality_code"))
        quality_code_ =
            static_cast<PulseOne::Enums::DataQuality>(data["quality_code"]);
      ...
      if (data.contains("value_timestamp")) {
        value_timestamp_ =
            PulseOne::Utils::ParseTimestampFromString(data["value_timestamp"]);
      }
      if (data.contains("quality_timestamp")) {
        quality_timestamp_ = PulseOne::Utils::ParseTimestampFromString(
            data["quality_timestamp"]);
      }
      if (data.contains("last_log_time")) {
        last_log_time_ =
            PulseOne::Utils::ParseTimestampFromString(data["last_log_time"]);
      }
      if (data.contains("last_read_time")) {
        last_read_time_ =
            PulseOne::Utils::ParseTimestampFromString(data["last_read_time"]);
      }
      if (data.contains("last_write_time")) {
        last_write_time_ =
            PulseOne::Utils::ParseTimestampFromString(data["last_write_time"]);
      }
      if (data.contains("updated_at")) {
        updated_at_ =
            PulseOne::Utils::ParseTimestampFromString(data["updated_at"]);
      }

      // 통계 카운터들
      if (data.contains("read_count"))
        read_count_ = data["read_count"];
      if (data.contains("write_count"))
        write_count_ = data["write_count"];
      if (data.contains("error_count"))
        error_count_ = data["error_count"];

      markModified();
      return true;
    } catch (const std::exception &e) {
      if (logger_)
        logger_->Error("CurrentValueEntity::fromJson failed: " +
                       std::string(e.what()));
      return false;
    }
  }
  ...
  // 타임스탬프 Getter들
  const std::chrono::system_clock::time_point &getValueTimestamp() const {
    return value_timestamp_;
  }
  const std::chrono::system_clock::time_point &getQualityTimestamp() const {
    return quality_timestamp_;
  }
  const std::chrono::system_clock::time_point &getLastLogTime() const {
    return last_log_time_;
  }
  const std::chrono::system_clock::time_point &getLastReadTime() const {
    return last_read_time_;
  }
  const std::chrono::system_clock::time_point &getLastWriteTime() const {
  ...
  void setRawValue(const std::string &raw_value) {
    raw_value_ = raw_value;
    markModified();
  }
  void setValueType(const std::string &value_type) {
  ...
  void setQuality(PulseOne::Enums::DataQuality quality_code) {
    ...
    quality_ =
        PulseOne::Utils::DataQualityToString(quality_code, true); // lowercase
    markModified();
  }
  ...
  // 타임스탬프 Setter들
  void
  setValueTimestamp(const std::chrono::system_clock::time_point &timestamp) {
    value_timestamp_ = timestamp;
    markModified();
  }
  void
  setQualityTimestamp(const std::chrono::system_clock::time_point &timestamp) {
    quality_timestamp_ = timestamp;
    markModified();
  }
  ...
  void
  setLastWriteTime(const std::chrono::system_clock::time_point &timestamp) {
  ...
  // 통계 카운터 Setter들
  void setReadCount(uint64_t count) {
    read_count_ = count;
    markModified();
  }
  void setWriteCount(uint64_t count) {
    write_count_ = count;
    markModified();
  }
  void setErrorCount(uint64_t count) {
    error_count_ = count;
    markModified();
  }
   ...
   */
  void
  setCurrentValueFromVariant(const PulseOne::BasicTypes::DataVariant &value) {
    json j;
  ...
  PulseOne::BasicTypes::DataVariant getCurrentValueAsVariant() const {
    try {
      ...
      if (logger_)
        logger_->Error("CurrentValueEntity::getCurrentValueAsVariant failed: " +
                       std::string(e.what()));
      return 0.0;
    }
  }
  ...
  std::string toString() const override {
    ...
    oss << "CurrentValue{point_id=" << point_id_ << ", value=" << current_value_
        << ", type=" << value_type_ << ", quality=" << quality_
        << ", timestamp="
        << PulseOne::Utils::TimestampToDBString(value_timestamp_)
        << ", reads=" << read_count_ << ", writes=" << write_count_
        << ", errors=" << error_count_ << "}";
    return oss.str();
  }
  ...
  json getStatistics() const {
    ...
    if (total_operations > 0) {
      stats["error_rate"] =
          static_cast<double>(error_count_) / total_operations * 100.0;
    } else {
  ...
  // CPP에서 구현될 메서드들 (선언만)
  // =======================================================================
  void updateValue(const PulseOne::BasicTypes::DataVariant &new_value,
...
private:
  ...
  std::string current_value_ =
      R"({"value": 0.0})"; // JSON: 현재값 (스케일링 적용됨)
  std::string raw_value_ = R"({"value": 0.0})"; // JSON: 원시값 (스케일링 전)
  std::string value_type_ = "double"; // 값 타입 (bool, int16, uint16, etc.)

  // 🔥 품질 정보
  PulseOne::Enums::DataQuality quality_code_ =
      PulseOne::Enums::DataQuality::GOOD; // 품질 코드
  std::string quality_ = "good";          // 품질 텍스트

  // 🔥 다양한 타임스탬프들
  std::chrono::system_clock::time_point value_timestamp_ =
      std::chrono::system_clock::now(); // 값 변경 시간
  std::chrono::system_clock::time_point quality_timestamp_ =
      std::chrono::system_clock::now(); // 품질 변경 시간
  std::chrono::system_clock::time_point last_log_time_ =
      std::chrono::system_clock::now(); // 마지막 로깅 시간
  std::chrono::system_clock::time_point last_read_time_ =
      std::chrono::system_clock::now(); // 마지막 읽기 시간
  std::chrono::system_clock::time_point last_write_time_ =
      std::chrono::system_clock::now(); // 마지막 쓰기 시간
  std::chrono::system_clock::time_point updated_at_ =
      std::chrono::system_clock::now(); // 업데이트 시간
