 ...
 * @brief PulseOne DeviceScheduleEntity - 디바이스용 스케줄 엔티티 (BACnet Weekly_Schedule 등)
 */
...
public:
    ...
    json toJson() const override {
        ...
        j["last_sync_time"] = last_sync_time_.has_value() ? timestampToString(last_sync_time_.value()) : "";
        return j;
    }

    bool fromJson(const json& data) override {
        try {
            if (data.contains("id")) setId(data["id"].get<int>());
            if (data.contains("device_id")) setDeviceId(data["device_id"].get<int>());
            if (data.contains("point_id")) {
                int pid = data["point_id"].get<int>();
                if (pid != -1) point_id_ = pid;
                else point_id_ = std::nullopt;
            }
            if (data.contains("schedule_type")) setScheduleType(data["schedule_type"].get<std::string>());
            if (data.contains("schedule_data")) setScheduleData(data["schedule_data"].get<std::string>());
            if (data.contains("is_synced")) setSynced(data["is_synced"].get<bool>());
            return true;
    ...
    virtual std::string getEntityTypeName() const override { return "DeviceScheduleEntity"; }
    ...
    std::optional<std::chrono::system_clock::time_point> getLastSyncTime() const { return last_sync_time_; }

    // Setters
    void setDeviceId(int device_id) { device_id_ = device_id; markModified(); }
    void setPointId(std::optional<int> point_id) { point_id_ = point_id; markModified(); }
    void setScheduleType(const std::string& type) { schedule_type_ = type; markModified(); }
    void setScheduleData(const std::string& data) { schedule_data_ = data; markModified(); }
    void setSynced(bool synced) { is_synced_ = synced; markModified(); }
    void setLastSyncTime(const std::chrono::system_clock::time_point& time) { last_sync_time_ = time; markModified(); }
