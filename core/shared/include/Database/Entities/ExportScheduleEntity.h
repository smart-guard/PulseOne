...
public:
    ...
    void setScheduleName(const std::string& name) { schedule_name_ = name; markModified(); }
    ...
    void setTimezone(const std::string& tz) { timezone_ = tz; markModified(); }
    void setDataRange(const std::string& range) { data_range_ = range; markModified(); }
    void setLookbackPeriods(int periods) { lookback_periods_ = periods; markModified(); }
    void setEnabled(bool enabled) { is_enabled_ = enabled; markModified(); }
    void setLastStatus(const std::string& status) { last_status_ = status; markModified(); }
    void setTotalRuns(int count) { total_runs_ = count; markModified(); }
    void setSuccessfulRuns(int count) { successful_runs_ = count; markModified(); }
    void setFailedRuns(int count) { failed_runs_ = count; markModified(); }
    
    void setLastRunAt(const std::chrono::system_clock::time_point& time) { 
        last_run_at_ = time; markModified(); 
    }
    void setNextRunAt(const std::chrono::system_clock::time_point& time) { 
        next_run_at_ = time; markModified(); 
    }
    ...
    void recordRun(bool success, const std::chrono::system_clock::time_point& next_run);
...
} // namespace PulseOne::Database::Entities
