...
public:
  ...
  findByIds(const std::vector<int> &ids) override;
   ...
   */
  std::vector<CurrentValueEntity>
  findByQuality(PulseOne::Enums::DataQuality quality);
  ...
  /**
   * @brief 다수 엔티티를 1 트랜잭션으로 배치 저장 (30K 포인트 성능 최적화)
   * @return 성공한 항목 수
   */
  size_t saveBatch(const std::vector<CurrentValueEntity> &entities);
...
protected:
  ...
  entityToParams(const CurrentValueEntity &entity);
