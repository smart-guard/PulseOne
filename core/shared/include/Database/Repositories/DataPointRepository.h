...
public:
  ...
  DataPointRepository() : IRepository<DataPointEntity>("DataPointRepository") {
    ...
    if (logger_) {
      logger_->Info(
          "DataPointRepository initialized with quality/alarm support");
      logger_->Info("Cache enabled: " +
                    std::string(isCacheEnabled() ? "true" : "false"));
    }
  }
   ...
   * @return 데이터포인트 벡터
   */
  std::vector<DataPointEntity> findByDeviceId(int device_id,
  ...
  findByDeviceIds(const std::vector<int> &device_ids,
                  bool enabled_only = false);
   ...
   * @return 데이터포인트 (없으면 nullopt)
   */
  std::optional<DataPointEntity> findByDeviceAndAddress(int device_id,
                                                        int address);
  ...
  getDataPointsWithCurrentValues(int device_id, bool enabled_only = true);

  /**
   * @brief CurrentValueRepository 의존성 주입
   * @param current_value_repo CurrentValueRepository 인스턴스
   */
  void setCurrentValueRepository(
...
private:
  ...
  entityToParams(const DataPointEntity &entity);
   ...
   */
  std::map<std::string, std::string>
  parseJsonToStringMap(const std::string &json_str,
   ...
   */
  std::vector<DataPointEntity>
  findDataPointsForWorkers(const std::vector<int> &device_ids = {});
