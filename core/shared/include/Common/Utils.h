...
inline std::string DriverLogCategoryToString(DriverLogCategory category) {
  switch (category) {
  case DriverLogCategory::GENERAL:
    return "GENERAL";
  case DriverLogCategory::CONNECTION:
    return "CONNECTION";
  case DriverLogCategory::COMMUNICATION:
    return "COMMUNICATION";
  case DriverLogCategory::DATA_PROCESSING:
    return "DATA_PROCESSING";
  case DriverLogCategory::ERROR_HANDLING:
    return "ERROR_HANDLING";
  case DriverLogCategory::PERFORMANCE:
    return "PERFORMANCE";
  case DriverLogCategory::SECURITY:
    return "SECURITY";
  case DriverLogCategory::PROTOCOL_SPECIFIC:
    return "PROTOCOL_SPECIFIC";
  case DriverLogCategory::DIAGNOSTICS:
    return "DIAGNOSTICS";
  default:
    return "UNKNOWN";
  }
...
inline DriverLogCategory StringToDriverLogCategory(const std::string &str) {
  ...
  if (upper == "GENERAL")
    return DriverLogCategory::GENERAL;
  if (upper == "CONNECTION")
    return DriverLogCategory::CONNECTION;
  if (upper == "COMMUNICATION")
    return DriverLogCategory::COMMUNICATION;
  if (upper == "DATA_PROCESSING")
    return DriverLogCategory::DATA_PROCESSING;
  if (upper == "ERROR_HANDLING")
    return DriverLogCategory::ERROR_HANDLING;
  if (upper == "PERFORMANCE")
    return DriverLogCategory::PERFORMANCE;
  if (upper == "SECURITY")
    return DriverLogCategory::SECURITY;
  if (upper == "PROTOCOL_SPECIFIC")
    return DriverLogCategory::PROTOCOL_SPECIFIC;
  if (upper == "DIAGNOSTICS")
    return DriverLogCategory::DIAGNOSTICS;
  if (upper == "MAINTENANCE")
    return DriverLogCategory::MAINTENANCE;
...
inline std::string LogLevelToString(LogLevel level) {
  ...
  case LogLevel::TRACE:
    return "TRACE";
  case LogLevel::DEBUG:
    return "DEBUG"; // 🔥 DEBUG_LEVEL → DEBUG 수정
  case LogLevel::INFO:
    return "INFO";
  case LogLevel::WARN:
    return "WARN";
...
inline LogLevel StringToLogLevel(const std::string &level) {
  ...
  if (upper == "DEBUG")
    return LogLevel::DEBUG; // 🔥 DEBUG_LEVEL → DEBUG 수정
  if (upper == "INFO")
...
inline std::string DataQualityToString(DataQuality quality,
  ...
  case DataQuality::STALE:
    result = "STALE";
    break;
  case DataQuality::MAINTENANCE:
    result = "MAINTENANCE";
    break;
  case DataQuality::SIMULATED:
    result = "SIMULATED";
    break;
  ...
  case DataQuality::NOT_CONNECTED:
    ...
    break;
  case DataQuality::TIMEOUT:
    result = "TIMEOUT";
    break;
  case DataQuality::SCAN_DELAYED:
    result = "SCAN_DELAYED";
    break;
  case DataQuality::UNDER_MAINTENANCE:
    result = "UNDER_MAINTENANCE";
    break;
  case DataQuality::STALE_DATA:
    result = "STALE_DATA";
    break;
  case DataQuality::VERY_STALE_DATA:
    result = "VERY_STALE_DATA";
    break;
  case DataQuality::MAINTENANCE_BLOCKED:
    result = "MAINTENANCE_BLOCKED";
    break;
  case DataQuality::ENGINEER_OVERRIDE:
    result = "ENGINEER_OVERRIDE";
    break;
...
inline DataQuality StringToDataQuality(const std::string &str) {
  ...
  if (upper == "MAINTENANCE")
    return DataQuality::MAINTENANCE;
  if (upper == "SIMULATED")
  ...
  // 확장 품질 상태
  if (upper == "NOT_CONNECTED")
    return DataQuality::NOT_CONNECTED;
  if (upper == "TIMEOUT")
    return DataQuality::TIMEOUT;
  if (upper == "SCAN_DELAYED")
    return DataQuality::SCAN_DELAYED;
  if (upper == "UNDER_MAINTENANCE")
    return DataQuality::UNDER_MAINTENANCE;
  if (upper == "STALE_DATA")
    return DataQuality::STALE_DATA;
  if (upper == "VERY_STALE_DATA")
    return DataQuality::VERY_STALE_DATA;
  if (upper == "MAINTENANCE_BLOCKED")
    return DataQuality::MAINTENANCE_BLOCKED;
  if (upper == "ENGINEER_OVERRIDE")
    return DataQuality::ENGINEER_OVERRIDE;
...
inline std::string MaintenanceStatusToString(MaintenanceStatus status) {
  ...
  case MaintenanceStatus::CANCELLED:
    return "CANCELLED"; // 추가
  case MaintenanceStatus::EMERGENCY:
    return "EMERGENCY"; // 변경
  default:
    return "UNKNOWN";
  }
...
inline std::string ErrorCodeToString(ErrorCode code) {
  ...
  case ErrorCode::FRAME_ERROR:
    return "FRAME_ERROR";

  // 데이터 관련 에러 (200-204)
  case ErrorCode::INVALID_DATA:
    return "INVALID_DATA";
  case ErrorCode::DATA_TYPE_MISMATCH:
  ...
  case ErrorCode::MISSING_CONFIGURATION:
  ...
  case ErrorCode::MAINTENANCE_PERMISSION_DENIED:
    return "MAINTENANCE_PERMISSION_DENIED";
  case ErrorCode::MAINTENANCE_TIMEOUT:
...
inline UniqueId GenerateDeviceID(ProtocolType protocol,
  ...
  std::stringstream ss;
  ss << prefix << "-" << std::hex << hash << "-" << std::hex
     << std::time(nullptr);
  return ss.str();
}
 ...
 */
inline Timestamp CurrentTimestamp() { return GetCurrentTimestamp(); }
...
inline Duration GetDurationSince(const Timestamp &start) {
  return std::chrono::duration_cast<Duration>(GetCurrentTimestamp() - start);
}
...
inline std::string ExtractIPFromEndpoint(const std::string &endpoint) {
  ...
  return (colon_pos != std::string::npos) ? endpoint.substr(0, colon_pos)
                                          : endpoint;
}
 ...
 */
inline uint16_t ExtractPortFromEndpoint(const std::string &endpoint,
 ...
 */
inline bool CanSafelyReleaseMaintenance(
 ...
 */
inline DataVariant StringToDataVariant(const std::string &str,
...
inline bool IsValidEngineerID(const EngineerID &engineer_id) {
  return !engineer_id.empty() && engineer_id.length() <= 64 &&
         engineer_id != "UNKNOWN" && engineer_id != "SYSTEM";
}
 ...
 */
inline std::string
TimestampToString(const PulseOne::BasicTypes::Timestamp &timestamp) {
  try {
 ...
 * @return Timestamp 객체
 */
inline PulseOne::BasicTypes::Timestamp
StringToTimestamp(const std::string &timestamp_str) {
  (void)timestamp_str;
...
inline std::string AccessModeToString(PulseOne::Enums::AccessMode mode) {
  switch (mode) {
  case PulseOne::Enums::AccessMode::read:
    return "read";
  case PulseOne::Enums::AccessMode::write:
    return "write";
  case PulseOne::Enums::AccessMode::read_write:
    return "read_write";
  default:
    return "read";
  }
...
inline PulseOne::Enums::AccessMode StringToAccessMode(const std::string &str) {
  if (str == "write")
    return PulseOne::Enums::AccessMode::write;
  if (str == "read_write")
    return PulseOne::Enums::AccessMode::read_write;
...
inline std::string DataTypeToString(PulseOne::Enums::DataType type) {
  switch (type) {
  case PulseOne::Enums::DataType::BOOL:
    return "BOOLEAN";
  case PulseOne::Enums::DataType::INT8:
    return "INT8";
  case PulseOne::Enums::DataType::UINT8:
    return "UINT8";
  case PulseOne::Enums::DataType::INT16:
    return "INT16";
  case PulseOne::Enums::DataType::UINT16:
    return "UINT16";
  case PulseOne::Enums::DataType::INT32:
    return "INT32";
  case PulseOne::Enums::DataType::UINT32:
    return "UINT32";
  case PulseOne::Enums::DataType::INT64:
    return "INT64";
  case PulseOne::Enums::DataType::UINT64:
    return "UINT64";
  case PulseOne::Enums::DataType::FLOAT32:
    return "FLOAT";
  case PulseOne::Enums::DataType::FLOAT64:
    return "DOUBLE";
  case PulseOne::Enums::DataType::STRING:
    return "STRING";
  case PulseOne::Enums::DataType::BINARY:
    return "BINARY";
  case PulseOne::Enums::DataType::DATETIME:
    return "DATETIME";
  case PulseOne::Enums::DataType::JSON:
    return "JSON";
  case PulseOne::Enums::DataType::ARRAY:
    return "ARRAY";
  case PulseOne::Enums::DataType::OBJECT:
    return "OBJECT";
  default:
    return "UNKNOWN";
  }
...
inline PulseOne::Enums::DataType StringToDataType(const std::string &str) {
  ...
  if (upper == "BOOLEAN" || upper == "BOOL")
    return PulseOne::Enums::DataType::BOOL;
  if (upper == "INT8")
    return PulseOne::Enums::DataType::INT8;
  if (upper == "UINT8")
    return PulseOne::Enums::DataType::UINT8;
  if (upper == "INT16")
    return PulseOne::Enums::DataType::INT16;
  if (upper == "UINT16")
    return PulseOne::Enums::DataType::UINT16;
  if (upper == "INT32")
    return PulseOne::Enums::DataType::INT32;
  if (upper == "UINT32")
    return PulseOne::Enums::DataType::UINT32;
  if (upper == "INT64")
    return PulseOne::Enums::DataType::INT64;
  if (upper == "UINT64")
    return PulseOne::Enums::DataType::UINT64;
  if (upper == "FLOAT" || upper == "FLOAT32")
    return PulseOne::Enums::DataType::FLOAT32;
  if (upper == "DOUBLE" || upper == "FLOAT64")
    return PulseOne::Enums::DataType::FLOAT64;
  if (upper == "STRING")
    return PulseOne::Enums::DataType::STRING;
  if (upper == "BINARY")
    return PulseOne::Enums::DataType::BINARY;
  if (upper == "DATETIME")
    return PulseOne::Enums::DataType::DATETIME;
  if (upper == "JSON")
    return PulseOne::Enums::DataType::JSON;
  if (upper == "ARRAY")
    return PulseOne::Enums::DataType::ARRAY;
  if (upper == "OBJECT")
...
inline std::string NormalizeDataType(const std::string &data_type) {
  ...
  if (upper == "FLOAT" || upper == "FLOAT32")
    return "FLOAT32"; // 32비트 → "FLOAT"
  if (upper == "DOUBLE" || upper == "FLOAT64")
    return "FLOAT64"; // 64비트 → "DOUBLE"
...
inline bool IsValidDataType(const std::string &data_type) {
  ...
  static const std::set<std::string> valid_normalized_types = {
      ...
      "INT64",  "UINT64",   "FLOAT", "DOUBLE", "FLOAT32", "FLOAT64", "STRING",
      "BINARY", "DATETIME", "JSON",  "ARRAY",  "OBJECT"};

  return valid_normalized_types.find(normalized) !=
         valid_normalized_types.end();
}
