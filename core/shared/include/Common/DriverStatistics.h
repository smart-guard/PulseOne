...
struct DriverStatistics {
  ...
  explicit DriverStatistics(const std::string &protocol_type)
      : DriverStatistics() {
  ...
  void UpdateTotalOperations() {
    ...
    successful_operations.store(successful_reads.load() +
                                successful_writes.load());
  ...
  // ===== 새로운 확장 메서드들 =====
  void IncrementProtocolCounter(const std::string &counter_name,
  ...
  void SetProtocolStatus(const std::string &status_name,
   ...
   * @return 평균 응답 시간 (밀리초)
   */
  double GetAverageResponseTime() const { return avg_response_time_ms.load(); }

  /**
   * @brief 최소 응답 시간 반환
   * @return 최소 응답 시간 (밀리초)
   */
  double GetMinResponseTime() const { return min_response_time_ms.load(); }

  /**
   * @brief 최대 응답 시간 반환
   * @return 최대 응답 시간 (밀리초)
   */
  double GetMaxResponseTime() const { return max_response_time_ms.load(); }
  ...
  double GetConnectionSuccessRate() const {
    uint64_t total_conn =
        successful_connections.load() + failed_connections.load();
    if (total_conn == 0)
      return 100.0;

    return (static_cast<double>(successful_connections.load()) / total_conn) *
           100.0;
  }
...
struct DriverStatisticsSnapshot {
  ...
  operator=(const DriverStatisticsSnapshot &) = default;
   ...
   * @brief 평균 응답 시간 반환
   */
  double GetAverageResponseTime() const { return avg_response_time_ms; }
};
...
inline DriverStatisticsSnapshot DriverStatistics::CreateSnapshot() const {
  ...
  snapshot.min_response_time_ms =
      static_cast<uint32_t>(min_response_time_ms.load());
  snapshot.max_response_time_ms =
      static_cast<uint32_t>(max_response_time_ms.load());
  ...
  snapshot.last_connection_time =
      Utils::TimestampToString(last_connection_time);
