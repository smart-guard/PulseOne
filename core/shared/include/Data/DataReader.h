...
public:
  ...
  explicit DataReader(std::shared_ptr<PulseOne::RedisClient> redis,
  ...
  ReadVirtualPointsBatch(const std::vector<int> &point_ids);
   ...
   */
  bool
  SubscribeToAlarms(std::function<void(const ActiveAlarm &)> callback,
...
private:
  ...
  parseVirtualPoint(const std::string &json_str);
