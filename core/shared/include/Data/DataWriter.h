...
public:
    ...
    explicit DataWriter(std::shared_ptr<RedisClient> redis,
     ...
     * @brief 서비스 시작 기록
     */
    bool RecordServiceStart(int service_id, 
     ...
     * @brief 서비스 요청 카운트 증가
     */
    bool IncrementServiceRequest(int service_id, bool success, int response_time_ms);
     ...
     * @brief 키-값 쓰기 (TTL 포함)
     */
    bool Write(const std::string& key, const std::string& value, int ttl_seconds = 0);
    
    /**
     * @brief JSON 쓰기
     */
    bool WriteJson(const std::string& key, const nlohmann::json& json, int ttl_seconds = 0);
...
private:
    ...
    bool writeWithRetry(const std::string& key, const std::string& value, int ttl_seconds);
    bool enqueueWrite(const std::string& key, const std::string& value, int ttl_seconds);
    void asyncWriteThreadFunc();
    void updateStats(bool success, double elapsed_ms, const std::string& operation_type);
