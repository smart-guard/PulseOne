 ...
 * @details 
 * 이 클래스는 기존 PulseOne 프로젝트와의 호환성을 유지하기 위한 Wrapper 클래스입니다.
 * 실제 모든 로깅 로직(파일 관리, 로테이션 등)은 독립 라이브러리인 LogLib::LoggerEngine이 수행합니다.
 * 이를 통해 핵심 로그 엔진을 다른 프로젝트에서 재사용(DLL/SO)할 수 있습니다.
 */
...
public:
    ...
    void Info(const std::string& format, Args&&... args) { Info(formatString(format, std::forward<Args>(args)...)); }
    template<typename... Args>
    void Warn(const std::string& format, Args&&... args) { Warn(formatString(format, std::forward<Args>(args)...)); }
    template<typename... Args>
    void Error(const std::string& format, Args&&... args) { Error(formatString(format, std::forward<Args>(args)...)); }
    template<typename... Args>
    void Fatal(const std::string& format, Args&&... args) { Fatal(formatString(format, std::forward<Args>(args)...)); }
    template<typename... Args>
    void Debug(const std::string& format, Args&&... args) { Debug(formatString(format, std::forward<Args>(args)...)); }
    template<typename... Args>
    void Trace(const std::string& format, Args&&... args) { Trace(formatString(format, std::forward<Args>(args)...)); }
    ...
    void logMaintenance(const UniqueId& device_id, const EngineerID& engineer_id, const std::string& message);
    ...
    void logDriver(const UniqueId& device_id, DriverLogCategory category, LogLevel level, const std::string& message);
    void logDataQuality(const UniqueId& device_id, const UniqueId& point_id, DataQuality quality, const std::string& reason = "");

    void logError(const std::string& message);
    void logPacket(const std::string& driver, const std::string& device, const std::string& rawPacket, const std::string& decoded);
...
private:
    ...
    void formatRecursive(std::stringstream& ss, const std::string& format, size_t& pos) { ss << format.substr(pos); }
...
inline void LogMaintenance(const UniqueId& device_id, const EngineerID& engineer_id, const std::string& message) { LogManager::getInstance().logMaintenance(device_id, engineer_id, message); }
inline void LogDriver(const UniqueId& device_id, DriverLogCategory category, LogLevel level, const std::string& message) { LogManager::getInstance().logDriver(device_id, category, level, message); }
inline void ReloadLogSettings() { LogManager::getInstance().reloadSettings(); }
inline void SetConsoleLogging(bool enabled) { LogManager::getInstance().setConsoleOutput(enabled); }
inline void SetFileLogging(bool enabled) { LogManager::getInstance().setFileOutput(enabled); }
inline void SetLogPath(const std::string& path) { LogManager::getInstance().setLogBasePath(path); }
