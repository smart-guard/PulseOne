...
ConfigManager::ConfigManager() 
    : initialized_(false)
    , secret_manager_initialized_(false) {
}
...
void ConfigManager::initializeSecretManager() {
    ...
    try {
        ...
        LogManager::getInstance().log("config", LogLevel::INFO, "SecretManager 연동 완료");
        
    } catch (const std::exception& e) {
        LogManager::getInstance().log("config", LogLevel::LOG_ERROR, 
            "SecretManager 초기화 실패: " + std::string(e.what()));
    }
}
...
std::string ConfigManager::findDataDirectory() {
    ...
    if (env_data && FileSystem::DirectoryExists(env_data)) {
        return std::string(env_data);
    }
    
    std::string data_dir_from_config = getOrDefault("DATA_DIR", "./data");
    
    if (data_dir_from_config[0] != '/' && data_dir_from_config.find(":\\") == std::string::npos) {
        std::string exe_dir = Path::GetExecutableDirectory();
...
void ConfigManager::printConfigSearchLog() const {
    LogManager::getInstance().log("config", LogLevel::INFO, "설정 디렉토리 탐색 로그:");
...
SecretManager가 자동으로 감지하여 처리합니다.
        ...
        LogManager::getInstance().log("config", LogLevel::INFO, "secrets/ 디렉토리 생성 (평문/암호화 호환)");
    }
}
...
void ConfigManager::ensureCSPDirectories() {
    ...
    std::vector<std::string> csp_dirs = {
        "failed_alarms", "csp_logs", "csp_backup", "csp_cache"
    };
...
bool ConfigManager::updateConfig(const std::string& key, const std::string& value, const std::string& config_file) {
    // 메모리에 설정값 업데이트
    set(key, value);
    
    // 파일 결정
    std::string target_file = config_file.empty() ? determineConfigFile(key) : config_file;
...
bool ConfigManager::updateMultipleConfigs(const std::map<std::string, std::string>& updates, const std::string& config_file) {
    // 메모리에 모든 설정값 업데이트
...
bool ConfigManager::writeConfigToFile(const std::string& filepath, const std::map<std::string, std::string>& config_data) {
    try {
        std::string dir = Path::GetDirectory(filepath);
        if (!FileSystem::DirectoryExists(dir)) {
            FileSystem::CreateDirectoryRecursive(dir);
        ...
        file << "# 자동 생성됨: " << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S") << "\n\n";
        ...
        LogManager::getInstance().log("config", LogLevel::LOG_ERROR, 
            "설정 파일 쓰기 실패: " + filepath + " - " + e.what());
        return false;
    }
}

std::map<std::string, std::string> ConfigManager::parseConfigFile(const std::string& filepath) const {
...
bool ConfigManager::setSecret(const std::string& config_key, const std::string& value, bool encrypt) {
...
ConfigManager::CSPAPIConfig ConfigManager::getCSPAPIConfig() const {
    return {
        .enabled = getBool("CSP_API_ENABLED", false),
...
ConfigManager::CSPS3Config ConfigManager::getCSPS3Config() const {
    return {
        ...
        .object_prefix = expandVariables(getOrDefault("CSP_S3_OBJECT_PREFIX", "alarms/{building_id}/")),
        .file_name_pattern = expandVariables(getOrDefault("CSP_S3_FILE_NAME_PATTERN", "{building_id}_{timestamp}_alarm.json")),
...
ConfigManager::CSPInsiteConfig ConfigManager::getCSPInsiteConfig() const {
    return {
        .enabled = getBool("CSP_INSITE_ENABLED", false),
        ...
        .control_clear_interval_hours = getInt("CSP_INSITE_CONTROL_CLEAR_INTERVAL_HOURS", 24)
    };
}

ConfigManager::CSPInbaseConfig ConfigManager::getCSPInbaseConfig() const {
    return {
        .enabled = getBool("CSP_INBASE_ENABLED", false),
...
ConfigManager::CSPBEMSConfig ConfigManager::getCSPBEMSConfig() const {
    return {
        .enabled = getBool("CSP_BEMS_ENABLED", false),
        .endpoint = getOrDefault("CSP_BEMS_ENDPOINT", ""),
        .api_key = getSecret("CSP_BEMS_API_KEY_FILE"),
        .building_id = getInt("CSP_BEMS_BUILDING_ID", getInt("CSP_GATEWAY_BUILDING_ID", 1001)),
...
ConfigManager::CSPAlarmFilter ConfigManager::getCSPAlarmFilter() const {
    return {
        .min_priority = getInt("CSP_ALARM_MIN_PRIORITY", 1),
        .max_priority = getInt("CSP_ALARM_MAX_PRIORITY", 4),
        .tag_include_pattern = getOrDefault("CSP_ALARM_TAG_INCLUDE_PATTERN", ".*"),
        .tag_exclude_pattern = getOrDefault("CSP_ALARM_TAG_EXCLUDE_PATTERN", "^TEST_.*"),
...
ConfigManager::CSPModbusConfig ConfigManager::getCSPModbusConfig() const {
    return {
        .alarm_timezone = getOrDefault("CSP_MODBUS_ALARM_TIMEZONE", "LOCAL_TIME"),
        .update_time_source = getOrDefault("CSP_MODBUS_UPDATE_TIME_SOURCE", "DATA_TIME"),
        .invalid_value_threshold = static_cast<long>(getInt("CSP_MODBUS_INVALID_VALUE_THRESHOLD", -2147483638)),
        .use_previous_on_invalid = getBool("CSP_MODBUS_USE_PREVIOUS_ON_INVALID", true)
    };
}

ConfigManager::CSPBatchConfig ConfigManager::getCSPBatchConfig() const {
    return {
        .enabled = getBool("CSP_BATCH_PROCESSING_ENABLED", true),
...
ConfigManager::CSPFailureConfig ConfigManager::getCSPFailureConfig() const {
    return {
        .save_enabled = getBool("CSP_FAILED_ALARM_SAVE_ENABLED", true),
        .save_path = expandVariables(getOrDefault("CSP_FAILED_ALARM_SAVE_PATH", "${DATA_DIR}/failed_alarms/")),
        .file_pattern = getOrDefault("CSP_FAILED_ALARM_FILE_PATTERN", "failed_{timestamp}_alarm.json"),
        .retry_enabled = getBool("CSP_FAILED_ALARM_RETRY_ENABLED", true),
        .retry_interval_minutes = getInt("CSP_FAILED_ALARM_RETRY_INTERVAL_MINUTES", 30),
...
ConfigManager::CSPLoggingConfig ConfigManager::getCSPLoggingConfig() const {
    return {
        .detailed_logging = getBool("CSP_DETAILED_LOGGING", true),
        ...
        .stats_update_interval_ms = getInt("CSP_STATS_UPDATE_INTERVAL_MS", 30000),
        .health_check_interval_ms = getInt("CSP_HEALTH_CHECK_INTERVAL_MS", 60000)
    };
}

ConfigManager::CSPNetworkConfig ConfigManager::getCSPNetworkConfig() const {
    return {
        .reconnect_enabled = getBool("CSP_NETWORK_RECONNECT_ENABLED", true),
        .reconnect_timeout_ms = getInt("CSP_NETWORK_RECONNECT_TIMEOUT_MS", 30000),
        .service_restart_on_reconnect = getBool("CSP_NETWORK_SERVICE_RESTART_ON_RECONNECT", true)
    };
}

ConfigManager::CSPSSLConfig ConfigManager::getCSPSSLConfig() const {
    return {
        .enabled = getBool("CSP_SSL_ENABLED", true),
...
bool ConfigManager::setAWSCredentials(const std::string& access_key, const std::string& secret_key) {
    bool success1 = setSecret("CSP_S3_ACCESS_KEY_FILE", access_key);
    bool success2 = setSecret("CSP_S3_SECRET_KEY_FILE", secret_key);
    return success1 && success2;
}

bool ConfigManager::setSSLCertificates(const std::string& cert_content, const std::string& key_content, const std::string& ca_content) {
    bool success1 = setSecret("CSP_SSL_CERT_FILE", cert_content);
    bool success2 = setSecret("CSP_SSL_KEY_FILE", key_content);
    bool success3 = ca_content.empty() || setSecret("CSP_SSL_CA_FILE", ca_content);
    return success1 && success2 && success3;
}

bool ConfigManager::setDatabasePassword(const std::string& db_type, const std::string& password) {
    std::string key;
...
bool ConfigManager::setAllCSPSecrets(const std::map<std::string, std::string>& csp_secrets) {
    bool all_success = true;
