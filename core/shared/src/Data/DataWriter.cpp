...
DataWriter::DataWriter(std::shared_ptr<PulseOne::RedisClient> redis,
  ...
  if (options_.enable_async_write) {
    async_thread_ =
        std::make_unique<std::thread>(&DataWriter::asyncWriteThreadFunc, this);
...
bool DataWriter::RecordServiceStart(int service_id,
  ...
  if (success) {
    LogManager::getInstance().Info("DataWriter: 서비스 시작 기록 - " +
                                   service_name);
  }
  ...
  try {
    ...
    if (success) {
      LogManager::getInstance().Info(
          "DataWriter: 서비스 중지 기록 - service_id=" +
...
bool DataWriter::IncrementServiceRequest(int service_id, bool success,
  ...
  try {
    ...
    bool write_success =
        writeWithRetry(key, j.dump(), options_.default_ttl_seconds);
    ...
    LogManager::getInstance().Error(
        "DataWriter::IncrementServiceRequest failed: " + std::string(e.what()));
    auto elapsed =
        duration_cast<microseconds>(high_resolution_clock::now() - start)
            .count() /
        1000.0;
...
int DataWriter::IncrementCounter(const std::string &counter_name,
                                 int increment) {
  auto start = high_resolution_clock::now();
...
bool DataWriter::Publish(const std::string &channel,
  ...
  while (true) {
    ...
    if (elapsed >= timeout_ms) {
      LogManager::getInstance().Warn(
          "DataWriter::WaitForPendingWrites timeout: " +
          std::to_string(GetPendingQueueSize()) + " items remaining");
      return false;
    }
...
bool DataWriter::IsConnected() const { return redis_ && redis_->isConnected(); }
...
bool DataWriter::writeWithRetry(const std::string &key,
  ...
  LogManager::getInstance().Error("DataWriter::writeWithRetry final failure: " +
                                  key);
  return false;
}
...
bool DataWriter::enqueueWrite(const std::string &key, const std::string &value,
  ...
  if (async_queue_.size() >= static_cast<size_t>(options_.async_queue_size)) {
    LogManager::getInstance().Warn(
        "DataWriter: 비동기 큐 가득참, 동기 쓰기로 전환");
...
void DataWriter::asyncWriteThreadFunc() {
  ...
  while (running_) {
    ...
    async_cv_.wait(lock, [this] { return !async_queue_.empty() || !running_; });
    ...
    int batch_size =
        std::min(static_cast<int>(async_queue_.size()), options_.batch_size);
