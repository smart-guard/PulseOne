...
DataReader::DataReader(std::shared_ptr<PulseOne::RedisClient> redis,
                       const DataReaderOptions &options)
    : redis_(redis), options_(options), stats_{} {
...
std::optional<CurrentValue>
DataReader::ReadDevicePoint(int device_num, const std::string &point_name) {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::ReadDevicePoint failed: " +
                                    std::string(e.what()));
...
std::vector<CurrentValue> DataReader::ReadAllDevicePoints(int device_num) {
  ...
  try {
    ...
    LogManager::getInstance().Info("DataReader::ReadAllDevicePoints: Found " +
    ...
    LogManager::getInstance().Error("DataReader::ReadAllDevicePoints failed: " +
                                    std::string(e.what()));
...
std::optional<CurrentValue> DataReader::ReadPointLatest(int point_id) {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::ReadPointLatest failed: " +
                                    std::string(e.what()));
...
BatchReadResult DataReader::ReadPointsBatch(const std::vector<int> &point_ids) {
  ...
  try {
    ...
    LogManager::getInstance().Info(
        ...
        "/" + std::to_string(result.total_requested) + " in " +
        std::to_string(result.elapsed_ms) + "ms");

    updateStats(true, result.elapsed_ms, true);

  } catch (const std::exception &e) {
    LogManager::getInstance().Error("DataReader::ReadPointsBatch failed: " +
                                    std::string(e.what()));
...
std::optional<VirtualPointValue> DataReader::ReadVirtualPoint(int point_id) {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::ReadVirtualPoint failed: " +
                                    std::string(e.what()));
...
std::vector<VirtualPointValue>
DataReader::ReadVirtualPointsBatch(const std::vector<int> &point_ids) {
  auto start = std::chrono::steady_clock::now();
...
std::optional<ActiveAlarm> DataReader::ReadActiveAlarm(int rule_id) {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::ReadActiveAlarm failed: " +
                                    std::string(e.what()));
...
std::vector<ActiveAlarm> DataReader::ReadAllActiveAlarms() {
  ...
  try {
    ...
    LogManager::getInstance().Info("DataReader::ReadAllActiveAlarms: Found " +
                                   std::to_string(results.size()) + " alarms");

    updateStats(!results.empty(), elapsed);

  } catch (const std::exception &e) {
    LogManager::getInstance().Error("DataReader::ReadAllActiveAlarms failed: " +
                                    std::string(e.what()));
...
int DataReader::ReadAlarmCount() {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::ReadAlarmCount failed: " +
                                    std::string(e.what()));
...
bool DataReader::SubscribeToAlarms(
  ...
  try {
    ...
    auto thread = std::make_unique<std::thread>(
        &DataReader::subscriptionThreadFunc, this, channel,
    ...
    LogManager::getInstance().Error("DataReader::SubscribeToAlarms failed: " +
                                    std::string(e.what()));
    return false;
  }
}
...
bool DataReader::IsSubscribed() const { return is_subscribed_.load(); }
...
DataReader::CacheStats DataReader::GetCacheStats() const {
  ...
  stats.hit_rate =
      total > 0 ? static_cast<double>(stats.hit_count) / total : 0.0;
...
bool DataReader::KeyExists(const std::string &key) {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::KeyExists failed: " +
                                    std::string(e.what()));
...
std::vector<std::string> DataReader::FindKeys(const std::string &pattern) {
  ...
  try {
    ...
    LogManager::getInstance().Error("DataReader::FindKeys failed: " +
                                    std::string(e.what()));
...
bool DataReader::IsConnected() const { return redis_ && redis_->isConnected(); }
...
std::optional<CurrentValue>
DataReader::parseCurrentValue(const std::string &json_str) {
  try {
    auto j = nlohmann::json::parse(json_str);
    return CurrentValue::fromJson(j);
  } catch (const std::exception &e) {
    LogManager::getInstance().Error("DataReader::parseCurrentValue failed: " +
                                    std::string(e.what()));
    return std::nullopt;
  }
}

std::optional<VirtualPointValue>
DataReader::parseVirtualPoint(const std::string &json_str) {
  try {
    auto j = nlohmann::json::parse(json_str);
    return VirtualPointValue::fromJson(j);
  } catch (const std::exception &e) {
    LogManager::getInstance().Error("DataReader::parseVirtualPoint failed: " +
                                    std::string(e.what()));
    return std::nullopt;
  }
}

std::optional<ActiveAlarm>
DataReader::parseActiveAlarm(const std::string &json_str) {
  try {
    auto j = nlohmann::json::parse(json_str);
    return ActiveAlarm::fromJson(j);
  } catch (const std::exception &e) {
    LogManager::getInstance().Error("DataReader::parseActiveAlarm failed: " +
                                    std::string(e.what()));
    return std::nullopt;
  }
}
    ...
    try {
      ...
      if (retry_count >= options_.retry_count) {
        LogManager::getInstance().Error(
            ...
            std::to_string(retry_count) + " retries: " + std::string(e.what()));
        throw;
      }
...
void DataReader::subscriptionThreadFunc(
  ...
  try {
    LogManager::getInstance().Info(
        "DataReader: Subscription thread started for " + channel);
    ...
    LogManager::getInstance().Info(
        "DataReader: Subscription thread ended for " + channel);
