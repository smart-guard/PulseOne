...
DeviceEntity::DeviceEntity()
    : BaseEntity<DeviceEntity>(), tenant_id_(0), site_id_(0),
      device_group_id_(std::nullopt), edge_server_id_(std::nullopt),
      protocol_instance_id_(std::nullopt), name_(""), description_(""),
      device_type_(""), manufacturer_(""), model_(""), serial_number_(""),
      ...
      ,
      is_enabled_(true), installation_date_(std::nullopt),
      last_maintenance_(std::nullopt), created_by_(std::nullopt),
      created_at_(std::chrono::system_clock::now()),
      updated_at_(std::chrono::system_clock::now()) {}

DeviceEntity::DeviceEntity(int device_id) : DeviceEntity() { // 위임 생성자 사용
  setId(device_id);
}
...
bool DeviceEntity::loadFromDatabase() {
  if (getId() <= 0) {
    if (logger_) {
      logger_->Error("DeviceEntity::loadFromDatabase - Invalid device ID: " +
                     std::to_string(getId()));
    }
    markError();
    return false;
  ...
  try {
        ...
        *this = loaded.value();
        ...
        if (logger_) {
          logger_->Info("DeviceEntity - Loaded device: " + name_ +
                        ...
                        ", polling: " + std::to_string(polling_interval_) +
                        "ms)");
        }
        return true;
      }
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::loadFromDatabase failed: " +
                     std::string(e.what()));
    }
    markError();
    return false;
...
bool DeviceEntity::saveToDatabase() {
  if (!isValid()) {
    if (logger_) {
      logger_->Error(
          "DeviceEntity::saveToDatabase - Invalid device data. "
          "Required fields: tenant_id=" +
          std::to_string(tenant_id_) + ", site_id=" + std::to_string(site_id_) +
          ", name=" + name_ + ", protocol_id=" + std::to_string(protocol_id_) +
          ", endpoint=" + endpoint_);
    }
    return false;
  }
  ...
  try {
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::saveToDatabase failed: " +
                     std::string(e.what()));
    }
    markError();
    return false;
...
bool DeviceEntity::deleteFromDatabase() {
  ...
  try {
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::deleteFromDatabase failed: " +
                     std::string(e.what()));
    }
    markError();
    return false;
...
bool DeviceEntity::updateToDatabase() {
  if (getId() <= 0 || !isValid()) {
    if (logger_) {
      logger_->Error(
          "DeviceEntity::updateToDatabase - Invalid device data or ID. "
          "ID=" +
          std::to_string(getId()) +
  ...
  try {
      ...
      if (success) {
        ...
        if (logger_) {
          logger_->Info("DeviceEntity - Updated device: " + name_ +
                        " (polling: " + std::to_string(polling_interval_) +
                        "ms, " + "timeout: " + std::to_string(timeout_) +
                        "ms)");
        }
      }
      return success;
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::updateToDatabase failed: " +
                     std::string(e.what()));
    }
    markError();
    return false;
...
std::string DeviceEntity::getProtocolDisplayName() const {
  try {
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::getProtocolDisplayName failed: " +
                     std::string(e.what()));
    }
    return "Unknown Protocol";
  }
...
int DeviceEntity::getProtocolDefaultPort() const {
  try {
    ...
    if (protocol_repo && protocol_id_ > 0) {
      auto protocol_opt = protocol_repo->findById(protocol_id_);
      if (protocol_opt.has_value() &&
          protocol_opt->getDefaultPort().has_value()) {
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::getProtocolDefaultPort failed: " +
                     std::string(e.what()));
    }
    return 0;
  }
...
bool DeviceEntity::isProtocolSerial() const {
  try {
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::isProtocolSerial failed: " +
                     std::string(e.what()));
    }
    return false;
  }
...
bool DeviceEntity::requiresBroker() const {
  try {
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::requiresBroker failed: " +
                     std::string(e.what()));
    }
    return false;
  }
...
std::string DeviceEntity::getProtocolCategory() const {
  try {
    ...
    case 3:
    case 4:
      return "industrial"; // Modbus, BACnet, OPC-UA
    case 2:
      return "iot"; // MQTT
    default:
      return "unknown";
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::getProtocolCategory failed: " +
                     std::string(e.what()));
    }
    return "unknown";
  }
  ...
  try {
    ...
    if (protocol_repo && protocol_id_ > 0) {
      ...
      if (protocol_opt.has_value()) {
        ...
        if (logger_) {
          logger_->Info(
              "DeviceEntity - Set optimal polling from protocol DB: " +
    ...
    if (logger_) {
      logger_->Info(
          "DeviceEntity - Set optimal polling (fallback) for protocol_id " +
    ...
    if (logger_) {
      logger_->Error("DeviceEntity::setOptimalPollingForProtocol failed: " +
                     std::string(e.what()));
    }
    // 실패 시 기본값 사용
    polling_interval_ = 1000;
...
void DeviceEntity::setOptimalTimeoutForEndpoint() {
  try {
    ...
    if (logger_) {
      logger_->Info("DeviceEntity - Set optimal timeout for endpoint " +
                    endpoint_ + ": " + std::to_string(timeout_) + "ms");
    }

  } catch (const std::exception &e) {
    if (logger_) {
      logger_->Error("DeviceEntity::setOptimalTimeoutForEndpoint failed: " +
                     std::string(e.what()));
    }
    // 실패 시 기본값 사용
    timeout_ = 3000;
...
void DeviceEntity::applyProtocolDefaults() {
  ...
  try {
      ...
      if (protocol_opt.has_value()) {
        ...
        if (!config.contains("supported_operations")) {
          config["supported_operations"] =
              protocol_opt->getSupportedOperationsAsJson();
        }

        if (!config.contains("supported_data_types")) {
          config["supported_data_types"] =
              protocol_opt->getSupportedDataTypesAsJson();
        }
      }
    }
  } catch (const std::exception &e) {
    if (logger_) {
      logger_->Error(
          "DeviceEntity::applyProtocolDefaults - ProtocolRepository error: " +
          std::string(e.what()));
    }
  }
...
std::string DeviceEntity::dateToString(
...
std::string DeviceEntity::timestampToString(
...
std::shared_ptr<Repositories::DeviceRepository>
DeviceEntity::getRepository() const {
  auto &factory = RepositoryFactory::getInstance();
...
void DeviceEntity::applyDeviceTypeDefaults() {
  ...
  if (type_lower == "plc") {
    if (manufacturer_.empty())
      manufacturer_ = "Generic";
    if (polling_interval_ == 1000)
      polling_interval_ = 500; // PLC는 빠른 폴링
    if (retry_count_ == 3)
      retry_count_ = 5; // PLC는 재시도 많이
  } else if (type_lower == "sensor") {
    if (manufacturer_.empty())
      manufacturer_ = "Generic";
    if (polling_interval_ == 1000)
      polling_interval_ = 2000; // 센서는 느린 폴링
    if (timeout_ == 3000)
      timeout_ = 5000; // 센서는 긴 타임아웃
  } else if (type_lower == "gateway") {
    if (manufacturer_.empty())
      manufacturer_ = "Generic";
    if (polling_interval_ == 1000)
      polling_interval_ = 1000; // 게이트웨이는 표준 폴링
    if (retry_count_ == 3)
      retry_count_ = 2; // 게이트웨이는 재시도 적게
  } else if (type_lower == "hmi") {
    if (manufacturer_.empty())
      manufacturer_ = "Generic";
    if (polling_interval_ == 1000)
      polling_interval_ = 1500; // HMI는 중간 폴링
    if (timeout_ == 3000)
      timeout_ = 2000; // HMI는 빠른 응답 기대
  } else if (type_lower == "meter") {
    if (manufacturer_.empty())
      manufacturer_ = "Generic";
    if (polling_interval_ == 1000)
      polling_interval_ = 5000; // 미터는 매우 느린 폴링
    if (timeout_ == 3000)
      timeout_ = 10000; // 미터는 긴 타임아웃
  }
  ...
  if (logger_) {
    logger_->Info(
        "DeviceEntity - Applied defaults for device type: " + device_type_ +
