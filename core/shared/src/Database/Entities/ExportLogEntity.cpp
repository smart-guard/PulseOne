...
ExportLogEntity::ExportLogEntity()
    : BaseEntity(), service_id_(0), target_id_(0), mapping_id_(0), point_id_(0),
      http_status_code_(0), processing_time_ms_(0),
      timestamp_(std::chrono::system_clock::now()), gateway_id_(0),
      sent_payload_("") {}

ExportLogEntity::ExportLogEntity(int id)
    : BaseEntity(id), service_id_(0), target_id_(0), mapping_id_(0),
      point_id_(0), http_status_code_(0), processing_time_ms_(0),
      timestamp_(std::chrono::system_clock::now()), gateway_id_(0),
      sent_payload_("") {}
...
bool ExportLogEntity::validate() const {
  ...
  if (status_ != "success" && status_ != "failed" && status_ != "timeout" &&
      status_ != "pending") {
    return false;
  }
...
bool ExportLogEntity::isSuccess() const { return status_ == "success"; }
...
bool ExportLogEntity::updateToDatabase() { return saveToDatabase(); }
...
json ExportLogEntity::toJson() const {
  ...
  try {
    ...
    j["gateway_id"] = gateway_id_;
    j["sent_payload"] = sent_payload_;
...
bool ExportLogEntity::fromJson(const json &data) {
  try {
    ...
    if (data.contains("gateway_id")) {
      gateway_id_ = data["gateway_id"].get<int>();
    }

    if (data.contains("sent_payload")) {
      sent_payload_ = data["sent_payload"].get<std::string>();
    }
