...
AlarmOccurrenceEntity::AlarmOccurrenceEntity()
    : BaseEntity<AlarmOccurrenceEntity>(), rule_id_(0), tenant_id_(0),
      occurrence_time_(std::chrono::system_clock::now()), trigger_value_(""),
      trigger_condition_(""), alarm_message_(""),
      severity_(AlarmSeverity::MEDIUM), state_(AlarmState::ACTIVE),
      acknowledged_time_(std::nullopt), acknowledged_by_(std::nullopt),
      acknowledge_comment_(""), cleared_time_(std::nullopt), cleared_value_(""),
      clear_comment_(""), cleared_by_(std::nullopt), notification_sent_(false),
      notification_time_(std::nullopt), notification_count_(0),
      notification_result_(""), context_data_("{}"), source_name_(""),
      location_(""), created_at_(std::chrono::system_clock::now()),
      updated_at_(std::chrono::system_clock::now()),
      device_id_(std::nullopt) // 🔧 수정: optional<int>로 초기화
      ,
      point_id_(std::nullopt), category_(std::nullopt), tags_() {}
...
bool AlarmOccurrenceEntity::loadFromDatabase() {
  if (getId() <= 0) {
    LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::LOG_ERROR,
                                  "loadFromDatabase - Invalid occurrence ID: " +
                                      std::to_string(getId()));
    markError();
    return false;
  }
  ...
  try {
      ...
      if (loaded.has_value()) {
        *this = loaded.value();
        ...
        LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::INFO,
                                      "loadFromDatabase - Loaded occurrence: " +
                                          std::to_string(getId()));
        return true;
      }
    }
    ...
    LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::WARN,
                                  "loadFromDatabase - Occurrence not found: " +
                                      std::to_string(getId()));
    return false;

  } catch (const std::exception &e) {
    LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::LOG_ERROR,
                                  "loadFromDatabase failed: " +
                                      std::string(e.what()));
    markError();
    return false;
  }
  ...
  try {
    ...
    if (repo) {
      ...
      if (success) {
        markSaved();
        LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::INFO,
                                      "saveToDatabase - Saved occurrence: " +
                                          std::to_string(getId()));
        return true;
      }
    }
    ...
    LogManager::getInstance().log(
        "AlarmOccurrenceEntity", LogLevel::LOG_ERROR,
        "saveToDatabase - Repository operation failed");
    return false;

  } catch (const std::exception &e) {
    LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::LOG_ERROR,
                                  "saveToDatabase failed: " +
                                      std::string(e.what()));
    markError();
    return false;
  }
...
bool AlarmOccurrenceEntity::deleteFromDatabase() {
  if (getId() <= 0) {
    LogManager::getInstance().log(
        "AlarmOccurrenceEntity", LogLevel::LOG_ERROR,
        "deleteFromDatabase - Invalid occurrence ID: " +
            std::to_string(getId()));
    return false;
  }
  ...
  try {
    ...
    if (repo) {
      ...
      if (success) {
        markDeleted();
        LogManager::getInstance().log(
            "AlarmOccurrenceEntity", LogLevel::INFO,
            "deleteFromDatabase - Deleted occurrence: " +
                std::to_string(getId()));
        return true;
      }
    }
...
bool AlarmOccurrenceEntity::updateToDatabase() {
  if (getId() <= 0) {
    LogManager::getInstance().log("AlarmOccurrenceEntity", LogLevel::LOG_ERROR,
                                  "updateToDatabase - Invalid occurrence ID: " +
                                      std::to_string(getId()));
    return false;
  }
  ...
  try {
    ...
    if (repo) {
      ...
      if (success) {
        ...
        LogManager::getInstance().log(
            "AlarmOccurrenceEntity", LogLevel::INFO,
            "updateToDatabase - Updated occurrence: " +
                std::to_string(getId()));
        return true;
      }
    }
...
json AlarmOccurrenceEntity::toJson() const {
  ...
  try {
    ...
    j["acknowledged_time"] = acknowledged_time_.has_value()
                                 ...
                                 : "";
    j["acknowledged_by"] =
        acknowledged_by_.has_value() ? acknowledged_by_.value() : -1;
...
bool AlarmOccurrenceEntity::fromJson(const json &j) {
  try {
    ...
    if (j.contains("trigger_value"))
      trigger_value_ = j["trigger_value"].get<std::string>();
    if (j.contains("trigger_condition"))
      trigger_condition_ = j["trigger_condition"].get<std::string>();
    if (j.contains("alarm_message"))
      alarm_message_ = j["alarm_message"].get<std::string>();
    if (j.contains("severity")) {
      severity_ =
          PulseOne::Alarm::stringToSeverity(j["severity"].get<std::string>());
    }
    if (j.contains("state")) {
    ...
    if (j.contains("acknowledge_comment"))
      acknowledge_comment_ = j["acknowledge_comment"].get<std::string>();
    ...
    if (j.contains("cleared_value"))
      cleared_value_ = j["cleared_value"].get<std::string>();
    if (j.contains("clear_comment"))
      clear_comment_ = j["clear_comment"].get<std::string>();
    ...
    // 알림 정보
    if (j.contains("notification_sent"))
      notification_sent_ = j["notification_sent"].get<bool>();
    ...
    if (j.contains("notification_count"))
      notification_count_ = j["notification_count"].get<int>();
    if (j.contains("notification_result"))
      notification_result_ = j["notification_result"].get<std::string>();

    // 컨텍스트 정보
    if (j.contains("context_data"))
      context_data_ = j["context_data"].get<std::string>();
    if (j.contains("source_name"))
      source_name_ = j["source_name"].get<std::string>();
    if (j.contains("location"))
      location_ = j["location"].get<std::string>();
...
bool AlarmOccurrenceEntity::isValid() const {
  return rule_id_ > 0 && tenant_id_ > 0 && !alarm_message_.empty();
}
...
bool AlarmOccurrenceEntity::acknowledge(int user_id,
...
bool AlarmOccurrenceEntity::clear(int user_id, const std::string &cleared_value,
                                  const std::string &comment) {
  try {
...
std::string AlarmOccurrenceEntity::toString() const {
  ...
  ss << "AlarmOccurrence[id=" << getId() << ", rule_id=" << rule_id_
     << ", tenant_id=" << tenant_id_ << ", device_id="
     << (device_id_.has_value() ? std::to_string(device_id_.value()) : "null")
     << ", point_id="
     << (point_id_.has_value() ? std::to_string(point_id_.value()) : "null")
     << ", severity=" << PulseOne::Alarm::severityToString(severity_)
     << ", state=" << PulseOne::Alarm::stateToString(state_) << ", message=\""
     << alarm_message_ << "\""
     << ", category=" << (category_.has_value() ? category_.value() : "null")
     << ", tags_count=" << tags_.size() << ", elapsed=" << getElapsedSeconds()
     << "s]";
  return ss.str();
}
...
std::string AlarmOccurrenceEntity::timestampToString(
    const std::chrono::system_clock::time_point &tp) const {
  auto time_t = std::chrono::system_clock::to_time_t(tp);
  std::ostringstream ss;
  // KST 저장을 위해 localtime 사용 (TZ=Asia/Seoul 대응)
  ss << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S");
  return ss.str();
}

std::chrono::system_clock::time_point
AlarmOccurrenceEntity::stringToTimestamp(const std::string &str) const {
  std::tm tm = {};
...
std::shared_ptr<Repositories::AlarmOccurrenceRepository>
AlarmOccurrenceEntity::getRepository() const {
