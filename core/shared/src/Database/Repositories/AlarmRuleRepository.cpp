...
AlarmRuleEntity AlarmRuleRepository::mapRowToEntity(const std::map<std::string, std::string>& row) {
    ...
    try {
        // 🔥 디버그 로그
        LogManager::getInstance().log("AlarmRuleRepository", LogLevel::DEBUG,
                                    "mapRowToEntity - Processing " + std::to_string(row.size()) + " columns");
        ...
        auto safeStringToInt = [this](const std::string& str, const std::string& field) -> int {
            if (str.empty() || str == "NULL") return 0;
            try {
                return std::stoi(str);
            } catch (...) {
                LogManager::getInstance().log("AlarmRuleRepository", LogLevel::WARN,
        ...
        auto safeStringToDouble = [this](const std::string& str, const std::string& field) -> double {
            if (str.empty() || str == "NULL") return 0.0;
            try {
                return std::stod(str);
            } catch (...) {
                LogManager::getInstance().log("AlarmRuleRepository", LogLevel::WARN,
        ...
        if (target_type_str == "data_point") {
            ...
            LogManager::getInstance().log("AlarmRuleRepository", LogLevel::WARN,
                                        "Unknown target_type: '" + target_type_str + "', using DATA_POINT");
            entity.setTargetType(PulseOne::Alarm::TargetType::DATA_POINT);
        }
        ...
        if (alarm_type_str == "analog") {
            ...
            LogManager::getInstance().log("AlarmRuleRepository", LogLevel::WARN,
                                        "Unknown alarm_type: '" + alarm_type_str + "', using ANALOG");
            entity.setAlarmType(PulseOne::Alarm::AlarmType::ANALOG);
        }
        ...
        if (severity_str == "critical") {
            ...
            LogManager::getInstance().log("AlarmRuleRepository", LogLevel::WARN,
                                        "Unknown severity: '" + severity_str + "', using MEDIUM");
            entity.setSeverity(PulseOne::Alarm::AlarmSeverity::MEDIUM);
        }
        ...
        entity.setNotificationEnabled(safeStringToBool(getValue("notification_enabled")));
        ...
        LogManager::getInstance().log("AlarmRuleRepository", LogLevel::DEBUG,
                                    "Successfully mapped alarm rule: " + entity.getName());
        return entity;
        ...
        fallback_entity.setTargetType(PulseOne::Alarm::TargetType::DATA_POINT);  // 올바른 enum 사용
        fallback_entity.setAlarmType(PulseOne::Alarm::AlarmType::ANALOG);        // 올바른 enum 사용
        fallback_entity.setSeverity(PulseOne::Alarm::AlarmSeverity::MEDIUM);     // 올바른 enum 사용
