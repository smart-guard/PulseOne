  ...
  try {
    ...
    for (const auto &row : results) {
      try {
        ...
        LogManager::getInstance().Warn("Failed to map row: " +
                                       std::string(e.what()));
      }
    }
...
std::optional<CurrentValueEntity> CurrentValueRepository::findById(int id) {
  try {
    ...
    LogManager::getInstance().Error("findById failed for ID " +
                                    std::to_string(id) + ": " +
                                    std::string(e.what()));
    return std::nullopt;
  }
...
bool CurrentValueRepository::deleteById(int id) {
  try {
    ...
    LogManager::getInstance().Error("deleteById failed: " +
                                    std::string(e.what()));
    return false;
  }
}
...
std::vector<CurrentValueEntity>
CurrentValueRepository::findByIds(const std::vector<int> &ids) {
  try {
    if (ids.empty()) {
      return {};
    }
    ...
    LogManager::getInstance().Error("findByIds failed: " +
                                    std::string(e.what()));
    return {};
  }
}
...
std::vector<CurrentValueEntity>
CurrentValueRepository::findByDeviceId(int device_id) {
  try {
    if (!ensureTableExists()) {
      return {};
    }
    ...
    LogManager::getInstance().Error("findByDeviceId failed: " +
                                    std::string(e.what()));
    return {};
  }
}

std::optional<CurrentValueEntity>
CurrentValueRepository::findByDataPointId(int data_point_id) {
  return findById(data_point_id);
}

std::vector<CurrentValueEntity>
CurrentValueRepository::findByQuality(PulseOne::Enums::DataQuality quality) {
  try {
    if (!ensureTableExists()) {
      return {};
    }
    ...
    LogManager::getInstance().Error("findByQuality failed: " +
                                    std::string(e.what()));
    return {};
  }
}
...
bool CurrentValueRepository::incrementReadCount(int point_id) {
  try {
    ...
    LogManager::getInstance().Error("incrementReadCount failed: " +
                                    std::string(e.what()));
    return false;
  }
}
...
bool CurrentValueRepository::incrementWriteCount(int point_id) {
  try {
    ...
    LogManager::getInstance().Error("incrementWriteCount failed: " +
                                    std::string(e.what()));
    return false;
  }
}
...
bool CurrentValueRepository::incrementErrorCount(int point_id) {
  try {
    ...
    LogManager::getInstance().Error("incrementErrorCount failed: " +
                                    std::string(e.what()));
    return false;
  }
}
...
void CurrentValueRepository::clearCache() { IRepository::clearCache(); }
...
int CurrentValueRepository::getTotalCount() {
  try {
    ...
    LogManager::getInstance().Error("getTotalCount failed: " +
                                    std::string(e.what()));
    return 0;
  }
}
...
CurrentValueEntity CurrentValueRepository::mapRowToEntity(
  ...
  try {
    // 기본 정보
    entity.setPointId(
        std::stoi(RepositoryHelpers::getRowValue(row, "point_id")));
    entity.setCurrentValue(
        RepositoryHelpers::getRowValue(row, "current_value"));
    ...
    // 품질 정보
    int quality_code =
        std::stoi(RepositoryHelpers::getRowValue(row, "quality_code", "0"));
    entity.setQuality(static_cast<PulseOne::Enums::DataQuality>(quality_code));

    // 타임스탬프들 (있는 것만 설정)
    if (row.find("value_timestamp") != row.end()) {
      entity.setValueTimestamp(
          Utils::StringToTimestamp(row.at("value_timestamp")));
    }
    ...
    // 통계 카운터들
    entity.setReadCount(
        std::stoi(RepositoryHelpers::getRowValue(row, "read_count", "0")));
    entity.setWriteCount(
        std::stoi(RepositoryHelpers::getRowValue(row, "write_count", "0")));
    entity.setErrorCount(
        std::stoi(RepositoryHelpers::getRowValue(row, "error_count", "0")));

    return entity;

  } catch (const std::exception &e) {
    LogManager::getInstance().Error("mapRowToEntity failed: " +
                                    std::string(e.what()));
    throw;
  }
}
...
CurrentValueRepository::entityToParams(const CurrentValueEntity &entity) {
  ...
  try {
    ...
    // value_type 소문자 변환 및 안전 처리
    std::string v_type = entity.getValueType();
    if (v_type.empty())
      v_type = "string"; // Fallback
    std::transform(v_type.begin(), v_type.end(), v_type.begin(), ::tolower);
    params["value_type"] = v_type;
    params["quality_code"] =
        std::to_string(static_cast<int>(entity.getQualityCode()));
    params["quality"] = entity.getQuality();
    params["value_timestamp"] =
        Utils::TimestampToString(entity.getValueTimestamp());
    ...
    params["updated_at"] =
        Utils::TimestampToString(Utils::GetCurrentTimestamp());

    return params;

  } catch (const std::exception &e) {
    LogManager::getInstance().Error("entityToParams failed: " +
                                    std::string(e.what()));
    throw;
  }
}

bool CurrentValueRepository::validateEntity(
    const CurrentValueEntity &entity) const {
  return validateCurrentValue(entity);
}

bool CurrentValueRepository::validateCurrentValue(
    const CurrentValueEntity &entity) const {
  if (entity.getPointId() <= 0) {
    LogManager::getInstance().Warn("Invalid point_id: " +
                                   std::to_string(entity.getPointId()));
    return false;
  }

  if (entity.getCurrentValue().empty()) {
    LogManager::getInstance().Warn("Empty current_value for point_id: " +
                                   std::to_string(entity.getPointId()));
    return false;
  }

  if (entity.getValueType().empty()) {
    LogManager::getInstance().Warn("Empty value_type for point_id: " +
                                   std::to_string(entity.getPointId()));
    return false;
  }
