...
std::vector<EdgeServerEntity> EdgeServerRepository::findAll() {
    try {
        ...
        logger_->Error("EdgeServerRepository::findAll failed: " + std::string(e.what()));
        return {};
    }
}
...
std::optional<EdgeServerEntity> EdgeServerRepository::findById(int id) {
    try {
        DbLib::DatabaseAbstractionLayer db_layer;
        std::string query = RepositoryHelpers::replaceParameter(SQL::EdgeServer::FIND_BY_ID, std::to_string(id));
        ...
        logger_->Error("EdgeServerRepository::findById failed: " + std::string(e.what()));
        return std::nullopt;
    }
}
...
bool EdgeServerRepository::save(EdgeServerEntity& entity) {
    ...
    try {
        ...
        if (entity.getId() > 0) data["id"] = std::to_string(entity.getId());
        ...
        data["updated_at"] = RepositoryHelpers::formatTimestamp(std::chrono::system_clock::now());
        data["is_deleted"] = "0";
        ...
        logger_->Error("EdgeServerRepository::save failed: " + std::string(e.what()));
        return false;
    }
}
...
bool EdgeServerRepository::deleteById(int id) {
    try {
        DbLib::DatabaseAbstractionLayer db_layer;
        std::string query = "DELETE FROM edge_servers WHERE id = " + std::to_string(id);
        return db_layer.executeNonQuery(query);
    } catch (const std::exception& e) {
        logger_->Error("EdgeServerRepository::deleteById failed: " + std::string(e.what()));
        return false;
    }
}
...
bool EdgeServerRepository::exists(int id) {
    try {
        DbLib::DatabaseAbstractionLayer db_layer;
        std::string query = "SELECT COUNT(*) as count FROM edge_servers WHERE id = " + std::to_string(id);
        auto results = db_layer.executeQuery(query);
        if (!results.empty()) {
        ...
        logger_->Error("EdgeServerRepository::exists failed: " + std::string(e.what()));
        return false;
    }
}

EdgeServerEntity EdgeServerRepository::mapRowToEntity(const std::map<std::string, std::string>& row) {
    ...
    try {
        ...
        if (row.count("status")) entity.setEnabled(row.at("status") == "active");
        if (row.count("ip_address")) entity.setIpAddress(row.at("ip_address"));
        if (row.count("port")) entity.setPort(std::stoi(row.at("port")));
    } catch (const std::exception& e) {
        logger_->Error("EdgeServerRepository::mapRowToEntity failed: " + std::string(e.what()));
    }
    return entity;
}

std::vector<EdgeServerEntity> EdgeServerRepository::mapResultToEntities(const std::vector<std::map<std::string, std::string>>& result) {
