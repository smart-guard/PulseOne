 ...
 * @brief Export Schedule Repository 구현 (DbLib::DatabaseAbstractionLayer API
 * 준수)
...
std::vector<ExportScheduleEntity> ExportScheduleRepository::findAll() {
  try {
    if (!ensureTableExists()) {
      logger_->Error(
          "ExportScheduleRepository::findAll - Table creation failed");
      return {};
    }
    ...
    logger_->Error("ExportScheduleRepository::findAll failed: " +
                   std::string(e.what()));
    return {};
  }
}
...
std::optional<ExportScheduleEntity> ExportScheduleRepository::findById(int id) {
  try {
    ...
    std::string query = RepositoryHelpers::replaceParameter(
        SQL::ExportSchedule::FIND_BY_ID, std::to_string(id));
    ...
    logger_->Error("ExportScheduleRepository::findById failed: " +
                   std::string(e.what()));
    return std::nullopt;
  }
}
...
bool ExportScheduleRepository::save(ExportScheduleEntity &entity) {
  try {
    ...
    query = RepositoryHelpers::replaceParameter(
        query,
        "'" + RepositoryHelpers::escapeString(params["description"]) + "'");
    query = RepositoryHelpers::replaceParameter(
        query,
        "'" + RepositoryHelpers::escapeString(params["cron_expression"]) + "'");
    query = RepositoryHelpers::replaceParameter(
        query, "'" + RepositoryHelpers::escapeString(params["timezone"]) + "'");
    query = RepositoryHelpers::replaceParameter(
        query,
        "'" + RepositoryHelpers::escapeString(params["data_range"]) + "'");
    query =
        RepositoryHelpers::replaceParameter(query, params["lookback_periods"]);
    query = RepositoryHelpers::replaceParameter(query, params["is_enabled"]);
    query = RepositoryHelpers::replaceParameter(
        query, "'" + params["next_run_at"] + "'");
    ...
    logger_->Error("ExportScheduleRepository::save failed: " +
                   std::string(e.what()));
    return false;
  }
}
...
bool ExportScheduleRepository::update(const ExportScheduleEntity &entity) {
  try {
    ...
    logger_->Error("ExportScheduleRepository::update failed: " +
                   std::string(e.what()));
    return false;
  }
}
...
bool ExportScheduleRepository::deleteById(int id) {
  try {
    ...
    // ✅ ExportSQLQueries 사용 + RepositoryHelpers로 파라미터 치환
    std::string query = RepositoryHelpers::replaceParameter(
        SQL::ExportSchedule::DELETE_BY_ID, std::to_string(id));
    bool success = db_layer.executeNonQuery(query);
    ...
    logger_->Error("ExportScheduleRepository::deleteById failed: " +
                   std::string(e.what()));
    return false;
  }
}
...
bool ExportScheduleRepository::exists(int id) {
  try {
    ...
    // ✅ ExportSQLQueries 사용 + RepositoryHelpers로 파라미터 치환
    std::string query = RepositoryHelpers::replaceParameter(
        SQL::ExportSchedule::EXISTS_BY_ID, std::to_string(id));
    auto results = db_layer.executeQuery(query);
    ...
    logger_->Error("ExportScheduleRepository::exists failed: " +
                   std::string(e.what()));
    return false;
  }
}

std::vector<ExportScheduleEntity>
ExportScheduleRepository::findByIds(const std::vector<int> &ids) {
  try {
    ...
    std::string query =
        "SELECT * FROM export_schedules WHERE id IN (" + id_list.str() + ")";
    auto results = db_layer.executeQuery(query);
    ...
    logger_->Error("ExportScheduleRepository::findByIds failed: " +
                   std::string(e.what()));
    return {};
  }
}
...
std::vector<ExportScheduleEntity> ExportScheduleRepository::findByConditions(
  ...
  try {
    ...
    for (const auto &condition : conditions) {
      ...
      // 값 타입에 따라 처리
      if (condition.value.find_first_not_of("0123456789.-") ==
          std::string::npos) {
...
int ExportScheduleRepository::countByConditions(
    const std::vector<QueryCondition> &conditions) {
  try {
    if (!ensureTableExists()) {
      return 0;
...
std::vector<ExportScheduleEntity> ExportScheduleRepository::findEnabled() {
  try {
    ...
    logger_->Error("ExportScheduleRepository::findEnabled failed: " +
                   std::string(e.what()));
    return {};
  }
}
...
std::vector<ExportScheduleEntity>
ExportScheduleRepository::findByTargetId(int target_id) {
  try {
    if (!ensureTableExists()) {
      return {};
    }
    ...
    // ✅ ExportSQLQueries 사용 + RepositoryHelpers로 파라미터 치환
    std::string query = RepositoryHelpers::replaceParameter(
        SQL::ExportSchedule::FIND_BY_TARGET_ID, std::to_string(target_id));
    auto results = db_layer.executeQuery(query);
    ...
    logger_->Error("ExportScheduleRepository::findByTargetId failed: " +
                   std::string(e.what()));
    return {};
  }
}
...
std::vector<ExportScheduleEntity> ExportScheduleRepository::findPending() {
  try {
    ...
    logger_->Error("ExportScheduleRepository::findPending failed: " +
                   std::string(e.what()));
    return {};
  }
}
...
int ExportScheduleRepository::countEnabled() {
  try {
    ...
    logger_->Error("ExportScheduleRepository::countEnabled failed: " +
                   std::string(e.what()));
    return 0;
  }
}
...
bool ExportScheduleRepository::updateRunStatus(
  ...
  try {
    ...
    query =
        RepositoryHelpers::replaceParameter(query, std::to_string(schedule_id));
...
bool ExportScheduleRepository::ensureTableExists() {
  ...
  try {
    ...
    std::vector<std::string> indexes = {
        "CREATE INDEX IF NOT EXISTS idx_export_schedules_target_id ON "
        "export_schedules(target_id)",
        "CREATE INDEX IF NOT EXISTS idx_export_schedules_is_enabled ON "
        "export_schedules(is_enabled)",
        "CREATE INDEX IF NOT EXISTS idx_export_schedules_next_run_at ON "
        "export_schedules(next_run_at)"};
...
ExportScheduleEntity ExportScheduleRepository::mapRowToEntity(
  ...
  try {
    ...
    auto it_profile = row.find("profile_id");
    if (it_profile != row.end() && !it_profile->second.empty() &&
        it_profile->second != "null") {
      entity.setProfileId(std::stoi(it_profile->second));
    } else {
      entity.setProfileId(0); // 0 indicates no profile
    }
    ...
    // ✅ created_at, updated_at은 Entity 생성 시 자동 설정되므로 별도 setter
    // 불필요 (ExportScheduleEntity에 setCreatedAt/setUpdatedAt 없음)
...
ExportScheduleRepository::entityToParams(const ExportScheduleEntity &entity) {
...
bool ExportScheduleRepository::validateEntity(
    const ExportScheduleEntity &entity) const {
