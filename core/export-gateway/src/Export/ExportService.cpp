...
bool ExportService::Initialize() {
    try {
        ...
        failed_data_dir_ = ConfigManager::getInstance().getOrDefault("EXPORT_FAILED_DIR", "/app/data/export_failed");
...
bool ExportService::Start() {
    ...
    LogManager::getInstance().Info("ExportService: 시작됨 (간격: " + std::to_string(export_interval_ms_) + "ms)");
    return true;
}
    ...
    try {
        ...
        for (const auto& db_target : db_targets) {
            ...
            try {
                ...
                LogManager::getInstance().Error("ExportService: 타겟 " + target.name + " config 파싱 실패: " + e.what());
                continue;
            }
            ...
            LogManager::getInstance().Info("ExportService: 타겟 로드됨 - " + target.name + " (" + target.type + ")");
        }
        
        LogManager::getInstance().Info("ExportService: " + std::to_string(active_targets_.size()) + "개 타겟 로드 완료");
...
bool ExportService::EnableTarget(int target_id, bool enable) {
    ...
    if (it != active_targets_.end()) {
        it->enabled = enable;
        LogManager::getInstance().Info("ExportService: 타겟 " + std::to_string(target_id) + (enable ? " 활성화" : " 비활성화"));
        return true;
    }
...
void ExportService::workerThread() {
    ...
    while (is_running_.load()) {
        try {
            ...
            LogManager::getInstance().Debug("ExportService: " + std::to_string(batch_result.values.size()) + "개 데이터 포인트 읽음");
...
bool ExportService::exportToTargetWithRetry(const ExportTargetConfig& target, const std::vector<Shared::Data::CurrentValue>& data) {
    int retry_count = 0;
    bool success = false;
    ...
    while (retry_count <= max_retry_count_ && !success) {
        try {
            if (retry_count > 0) {
                LogManager::getInstance().Warn("ExportService: 재시도 " + std::to_string(retry_count) + "/" + std::to_string(max_retry_count_) + " - " + target.name);
            ...
            last_error = e.what();
            LogManager::getInstance().Error("ExportService: 전송 예외 - " + target.name + ": " + last_error);
        }
    ...
    if (!success) {
        ...
        LogManager::getInstance().Error("ExportService: 전송 최종 실패 (" + std::to_string(max_retry_count_ + 1) + "회 시도) - " + target.name);
    }
    
    return success;
}

bool ExportService::exportToTarget(const ExportTargetConfig& target, const std::vector<Shared::Data::CurrentValue>& data) {
    try {
        ...
        LogManager::getInstance().Error("ExportService::exportToTarget 예외: " + std::string(e.what()));
        return false;
    }
}

void ExportService::saveFailedData(const ExportTargetConfig& target, const std::vector<Shared::Data::CurrentValue>& data, const std::string& error_message) {
    try {
        ...
        filename << "failed_" << std::put_time(&tm, "%Y%m%d_%H%M%S") << "_" << "target" << target.id << "_" << data.size() << "points.json";
        ...
        LogManager::getInstance().Error("ExportService::saveFailedData 예외: " + std::string(e.what()));
    }
}

std::vector<int> ExportService::collectPointIdsForExport(const std::vector<ExportTargetConfig>& targets) {
    ...
    try {
        ...
        if (!point_ids.empty()) {
            LogManager::getInstance().Debug("ExportService: " + std::to_string(point_ids.size()) + "개 포인트 수집됨");
        }
        
        return point_ids;
        
    } catch (const std::exception& e) {
        LogManager::getInstance().Error("ExportService::collectPointIdsForExport 실패: " + std::string(e.what()));
        return {};
    }
}

void ExportService::logExportResult(int target_id, const std::string& status, int data_count, const std::string& error_message) {
    try {
        ...
        if (status == "success") {
            LogManager::getInstance().Info("ExportService: 전송 성공 - 타겟 " + std::to_string(target_id) + ", " + std::to_string(data_count) + "개 포인트");
        } else {
            LogManager::getInstance().Error("ExportService: 전송 실패 - 타겟 " + std::to_string(target_id) + ": " + error_message);
        }
        
    } catch (const std::exception& e) {
        LogManager::getInstance().Error("ExportService::logExportResult 예외: " + std::string(e.what()));
    }
}
