...
// using namespace PulseOne::Export; // 이미 Export 네임스페이스 내부임
...
bool isValidTargetConfig(const json & /* config */,
 ...
 */
bool isValidTargetConfigExtended(const json &config,
                                 const std::string &target_type) {
  // 기본 검증
  ...
  if (config.empty() || !config.is_object()) {
    LogManager::getInstance().Error(
        "타겟 설정이 비어있거나 올바른 JSON 객체가 아닙니다");
    return false;
  }
  ...
  if (target_type == "http" || target_type == "HTTP") {
    ...
    if (url.find("http://") != 0 && url.find("https://") != 0) {
      LogManager::getInstance().Error(
          "HTTP endpoint는 http:// 또는 https://로 시작해야 합니다");
      return false;
    }
    ...
    if (!std::filesystem::exists(
      ...
      LogManager::getInstance().Error(
          "FILE 타겟의 base_path 상위 디렉토리가 존재하지 않음: " + base_path);
      return false;
    }
  }
 ...
 */
std::string replaceVariables(const std::string &template_str,
 ...
 */
json createAlarmJsonArray(const std::vector<AlarmMessage> &alarms) {
  json j_array = json::array();
  for (const auto &alarm : alarms) {
    j_array.push_back(createAlarmJson(alarm));
  }
  return j_array;
}
 ...
 */
json createValueJsonArray(const std::vector<ValueMessage> &values) {
  json j_array = json::array();
  for (const auto &value : values) {
    j_array.push_back(value.to_json());
  }
  return j_array;
}
...
size_t getFileSize(const std::string &file_path) {
  try {
    ...
    LogManager::getInstance().Error("파일 크기 조회 실패: " + file_path +
                                    " - " + e.what());
  }
  return 0;
}
...
getFileModificationTime(const std::string &file_path) {
  try {
    if (std::filesystem::exists(file_path)) {
    ...
    LogManager::getInstance().Error("파일 수정 시간 조회 실패: " + file_path +
                                    " - " + e.what());
  }
...
std::string readFileContents(const std::string &file_path) {
  try {
    ...
    LogManager::getInstance().Error("파일 읽기 예외: " + file_path + " - " +
                                    std::string(e.what()));
    return "";
  }
}
 ...
 * @brief 파일 안전하게 쓰기
 */
bool writeFileContents(const std::string &file_path, const std::string &content,
                       bool create_dirs) {
  try {
    if (create_dirs) {
    ...
    LogManager::getInstance().Error("파일 쓰기 예외: " + file_path + " - " +
                                    std::string(e.what()));
    return false;
  }
}
