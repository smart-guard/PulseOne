...
AlarmMessage AlarmMessage::from_alarm_occurrence(
  ...
  try {
    ...
    // site_id는 AlarmOccurrenceEntity에 직접 없으므로
    // 상위 레이어(ExportCoordinator)에서 lookup 하여 채워주거나
    // occurrence_time 등 메타데이터와 함께 전달됨.
    // 여기서는 occurrence 내부의 device_id 등을 활용할 수 있음 (Entity가
    // 제공한다면)
    ...
    if (!occurrence.getTriggerValue().empty()) {
      try {
        ...
        LogManager::getInstance().Debug("Failed to parse trigger_value: " +
                                        occurrence.getTriggerValue());
      }
    } else {
      msg.vl = 0.0;
    }
    ...
    msg.tm = time_to_csharp_format(occurrence.getOccurrenceTime(),
                                   true); // 로컬시간 사용
    ...
    if (!occurrence.getAlarmMessage().empty()) {
      ...
      std::string severity_str =
          (occurrence.getSeverity() == PulseOne::Alarm::AlarmSeverity::CRITICAL)
              ? "CRITICAL"
          : (occurrence.getSeverity() == PulseOne::Alarm::AlarmSeverity::HIGH)
              ? "HIGH"
          : (occurrence.getSeverity() == PulseOne::Alarm::AlarmSeverity::MEDIUM)
              ? "MEDIUM"
              : "LOW";
    ...
    LogManager::getInstance().Error("Exception in from_alarm_occurrence: " +
                                    std::string(e.what()));
 ...
 */
bool AlarmMessage::simulate_csp_api_call(const AlarmMessage &msg,
                                         std::string &response) {
  try {
    // C# JSON 포맷으로 시뮬레이션
    ...
    json api_response = {{"statusCode", 200},
                         {"Status", "OK"},
...
#ifdef HAS_SHARED_LIBS
    LogManager::getInstance().Debug("API Call simulated for: " + msg.nm + "/" +
                                    msg.get_alarm_status_string());
#endif
    return true;

  } catch (const std::exception &e) {
#ifdef HAS_SHARED_LIBS
    LogManager::getInstance().Error("API simulation failed: " +
                                    std::string(e.what()));
#endif

    // 실패 응답 생성
    json error_response = {{"statusCode", 500},
                           {"Status", "ERROR"},
...
bool AlarmMessage::validate_for_csp_api() const {
  ...
  if (bd <= 0) {
#ifdef HAS_SHARED_LIBS
    LogManager::getInstance().Error("Invalid building_id: " +
                                    std::to_string(bd));
#endif
    return false;
  }
  ...
  if (al != 0 && al != 1) {
#ifdef HAS_SHARED_LIBS
    LogManager::getInstance().Error("Invalid alarm status: " +
                                    std::to_string(al));
#endif
    return false;
  }
 ...
 */
AlarmMessage AlarmMessage::create_sample(int building_id,
  ...
  if (is_alarm_active) {
    msg.des =
        point_name + " 알람 발생 (값: " + std::to_string(trigger_value) + ")";
  } else {
    msg.des =
        point_name + " 알람 해제 (값: " + std::to_string(trigger_value) + ")";
  }
