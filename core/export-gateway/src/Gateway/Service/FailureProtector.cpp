 ...
 * @brief 실패 방지기 구현 - CircuitBreaker 패턴
 * @author PulseOne Development Team
 * @date 2025-09-24
 * @version 1.1.0 (Migrated to Gateway::Service)
 */
...
FailureProtector::FailureProtector(const std::string &target_name,
                                   const FailureProtectorConfig &config)
    : target_name_(target_name), config_(config), state_(State::CLOSED),
      failure_count_(0), success_count_(0),
      last_failure_time_(std::chrono::steady_clock::now()),
      last_state_change_(std::chrono::steady_clock::now()),
      half_open_attempts_(0), total_attempts_(0), total_successes_(0),
      total_failures_(0) {

  LogManager::getInstance().Info("FailureProtector 초기화: " + target_name_);
  LogManager::getInstance().Debug(
      "설정 - failure_threshold: " + std::to_string(config_.failure_threshold) +
      ", recovery_timeout: " + std::to_string(config_.recovery_timeout_ms) +
      "ms");
}

FailureProtector::~FailureProtector() {
  LogManager::getInstance().Info(
      "FailureProtector 종료: " + target_name_ +
...
bool FailureProtector::canExecute() {
  ...
  case State::OPEN:
    ...
    if (isRecoveryTimeReached()) {
      LogManager::getInstance().Info("복구 시간 도달 - HALF_OPEN으로 전환: " +
                                     target_name_);
      transitionToHalfOpen();
      return true;
    }
    LogManager::getInstance().Debug("회로 열림 상태 - 요청 차단: " +
                                    target_name_);
    return false;
  ...
  case State::HALF_OPEN:
    ...
    if (half_open_attempts_ < config_.half_open_max_attempts) {
      half_open_attempts_++;
      LogManager::getInstance().Debug(
          "HALF_OPEN 시도: " + std::to_string(half_open_attempts_) + "/" +
          std::to_string(config_.half_open_max_attempts) + " (" + target_name_ +
          ")");
      return true;
    }
    LogManager::getInstance().Debug("HALF_OPEN 최대 시도 초과 - 요청 차단: " +
                                    target_name_);
    return false;
  }
...
void FailureProtector::recordSuccess() {
  ...
  LogManager::getInstance().Debug(
      "성공 기록: " + target_name_ +
      " (연속 성공: " + std::to_string(success_count_.load()) + ")");
  ...
  case State::HALF_OPEN:
    ...
    if (success_count_ >= config_.half_open_success_threshold) {
      LogManager::getInstance().Info(
          "HALF_OPEN 성공 임계치 달성 - CLOSED로 전환: " + target_name_);
      transitionToClosed();
    }
    break;
  ...
  case State::OPEN:
    ...
    LogManager::getInstance().Warn("OPEN 상태에서 성공 기록 - 비정상적 상황: " +
                                   target_name_);
    break;
  }
}
...
void FailureProtector::recordFailure() {
  ...
  LogManager::getInstance().Debug(
      "실패 기록: " + target_name_ +
      " (연속 실패: " + std::to_string(failure_count_.load()) + ")");
  ...
  case State::CLOSED:
    ...
    if (failure_count_ >= config_.failure_threshold) {
      LogManager::getInstance().Warn(
          "실패 임계치 초과 - OPEN으로 전환: " + target_name_ + " (" +
  ...
  case State::HALF_OPEN:
    ...
    LogManager::getInstance().Warn(
        "HALF_OPEN 상태에서 실패 - OPEN으로 즉시 전환: " + target_name_);
    transitionToOpen();
    break;
...
void FailureProtector::reset() {
  ...
  LogManager::getInstance().Info("리셋 완료 - CLOSED 상태로 초기화: " +
                                 target_name_);
}
...
bool FailureProtector::isHealthy() const { return getState() == State::CLOSED; }
...
FailureProtectorStats FailureProtector::getStats() const {
  ...
  stats.success_rate =
      total > 0 ? (static_cast<double>(stats.total_successes) / total * 100.0)
                : 0.0;
...
double FailureProtector::getSuccessRate() const {
  ...
  return total > 0
             ? (static_cast<double>(total_successes_.load()) / total * 100.0)
             : 0.0;
}
...
bool FailureProtector::isRecoveryTimeReached() const {
  ...
  auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
      now - last_failure_time_loaded);

  // 🚨 FIX: use (total - threshold) as exponent so it starts at 0 (2^0 = 1x
  // timeout)
  uint32_t effective_failures = 0;
  if (failure_count_.load() > config_.failure_threshold) {
    effective_failures = failure_count_.load() - config_.failure_threshold;
  }

  uint32_t backoff_multiplier =
      std::min(effective_failures, static_cast<uint32_t>(10));
  uint64_t adjusted_timeout =
      static_cast<uint64_t>(config_.recovery_timeout_ms) *
      static_cast<uint64_t>(
          std::pow(config_.backoff_multiplier, backoff_multiplier));

  adjusted_timeout = std::min(
      adjusted_timeout, static_cast<uint64_t>(config_.max_recovery_timeout_ms));
...
void FailureProtector::transitionToClosed() {
  ...
  LogManager::getInstance().Info("상태 전환: " + stateToString(old_state) +
                                 " → CLOSED (" + target_name_ + ")");
}
...
void FailureProtector::transitionToOpen() {
  ...
  LogManager::getInstance().Warn(
      "상태 전환: " + stateToString(old_state) + " → OPEN (" + target_name_ +
      ") - 복구 대기 시간: " + std::to_string(calculateNextRecoveryTimeout()) +
      "ms");
}
...
void FailureProtector::transitionToHalfOpen() {
  ...
  LogManager::getInstance().Info(
      "상태 전환: " + stateToString(old_state) + " → HALF_OPEN (" +
      target_name_ +
...
uint64_t FailureProtector::calculateNextRecoveryTimeout() const {
  ...
  uint64_t timeout = static_cast<uint64_t>(config_.recovery_timeout_ms) *
                     static_cast<uint64_t>(std::pow(config_.backoff_multiplier,
                                                    backoff_multiplier));

  return std::min(timeout,
                  static_cast<uint64_t>(config_.max_recovery_timeout_ms));
}
...
void FailureProtector::updateConfig(const FailureProtectorConfig &new_config) {
  ...
  LogManager::getInstance().Debug(
      "기존 failure_threshold: " + std::to_string(config_.failure_threshold) +
  ...
  if (state_ == State::CLOSED && failure_count_ >= config_.failure_threshold) {
    LogManager::getInstance().Warn(
        "설정 변경으로 인한 상태 재평가 - OPEN으로 전환: " + target_name_);
    transitionToOpen();
  }
