...
public:
     ...
     * @return 처리 성공 여부
     */
    virtual bool handleEvent(const std::string& channel, const std::string& message) = 0;
...
private:
     ...
     * @return 매칭 여부
     */
    bool matchChannelPattern(const std::string& pattern, const std::string& channel) const;
     ...
     */
    void handleScheduleEvent(const std::string& channel, const std::string& message);
     ...
     */
    void handleSystemEvent(const std::string& channel, const std::string& message);
    ...
    // 이벤트 핸들러 맵
    std::unordered_map<std::string, std::shared_ptr<IEventHandler>> event_handlers_;
