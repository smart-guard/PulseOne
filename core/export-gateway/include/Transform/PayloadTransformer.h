...
public:
  ...
  // 메인 변환
  ordered_json transform(const ordered_json &template_json,
                         const TransformContext &context);
  std::string transformString(const std::string &template_str,
                              const TransformContext &context);

  // 시스템별 기본 템플릿
  ordered_json getInsiteDefaultTemplate();
  ordered_json getHDCDefaultTemplate();
  ordered_json getBEMSDefaultTemplate();
  ordered_json getGenericDefaultTemplate();
  ordered_json getDefaultTemplateForSystem(const std::string &system_type);
  ...
  void expandJsonRecursive(ordered_json &obj,
