...
private:
  void setupRoutes() {
    server_->Post("/webhook", [this](const httplib::Request &req,
      ...
      try {
        ...
        LogManager::getInstance().Error("Mock 서버 에러: " +
                                        std::string(e.what()));
        res.status = 400;
...
public:
  ...
  ~CompleteIntegrationTest() { cleanup(); }
  ...
  bool runAllTests() {
    ...
    try {
      ...
      if (testScheduleFlow())
        passed_tests++;
      if (testDynamicTargetManager())
        passed_tests++;
      if (testPayloadTransformer())
        passed_tests++;
      if (testFailureProtector())
        passed_tests++;
      if (testFileTargetHandler())
        passed_tests++;
      if (testMultipleTargetsConcurrent())
        passed_tests++;
      if (testFailureAndRetry())
        passed_tests++;
      if (testExportLogRepository())
        passed_tests++;

      LogManager::getInstance().Info(
          "========================================");
      LogManager::getInstance().Info(
          "✅ 테스트 완료: " + std::to_string(passed_tests) + "/" +
      ...
      LogManager::getInstance().Info(
          "통과한 테스트: " + std::to_string(passed_tests) + "/" +
...
private:
  bool setupEnvironment() {
    ...
    try {
      ...
      LogManager::getInstance().Error("환경 설정 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool createTestTargets() {
    try {
...
#ifdef HAVE_HTTPLIB
      json alarm_config = {{"url", "http://localhost:18080/webhook"},
                           {"method", "POST"},
                           {"timeout", 5000},
                           {"export_mode", "alarm"}};
#else
      json alarm_config = {{"url", "http://httpbin.org/post"},
                           {"method", "POST"},
                           {"timeout", 5000},
                           {"export_mode", "alarm"}};
#endif
...
#ifdef HAVE_HTTPLIB
      json schedule_config = {{"url", "http://localhost:18080/webhook"},
                              {"method", "POST"},
                              {"timeout", 5000},
                              {"export_mode", "schedule"}};
#else
      json schedule_config = {{"url", "http://httpbin.org/post"},
                              {"method", "POST"},
                              {"timeout", 5000},
                              {"export_mode", "schedule"}};
#endif
      ...
      LogManager::getInstance().Info(
          "✅ 테스트 타겟 생성 (알람: " + std::to_string(alarm_target_id_) +
          ", 스케줄: " + std::to_string(schedule_target_id_) + ")");

      return true;

    } catch (const std::exception &e) {
      LogManager::getInstance().Error("타겟 생성 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testAlarmFlow() {
    ...
    try {
      ...
      TestHelper::assertCondition(!received.empty(),
                                  "알람 전송 확인 (" +
                                      std::to_string(received.size()) + "건)");
#endif
      ...
      LogManager::getInstance().Error("알람 플로우 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testScheduleFlow() {
    ...
    try {
      ...
      LogManager::getInstance().Info(
          "✅ 스케줄 생성 (ID: " + std::to_string(schedule.getId()) + ")");
      ...
      LogManager::getInstance().Error("스케줄 플로우 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testDynamicTargetManager() {
    ...
    try {
      ...
      TestHelper::assertCondition(manager.isRunning(),
                                  "DynamicTargetManager 실행 중");

      auto targets = manager.getAllTargets();
      TestHelper::assertCondition(!targets.empty(),
                                  "타겟 로드됨 (" +
                                      std::to_string(targets.size()) + "개)");

      auto stats = manager.getStatistics();
      LogManager::getInstance().Info("📊 통계:");
      LogManager::getInstance().Info(
          "  요청: " + std::to_string(stats["total_requests"].get<uint64_t>()));
      LogManager::getInstance().Info(
          "  성공: " +
      ...
      LogManager::getInstance().Error("DynamicTargetManager 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testPayloadTransformer() {
    ...
    try {
      ...
      auto context =
          transformer.createContext(alarm, "Field1", "Description", "42.0");
      ...
      TestHelper::assertCondition(
          !result.empty() && result["building_id"] == 1001, "템플릿 변환 성공");

      std::string str_template = "Building {{building_id}}: {{point_name}}";
      std::string str_result =
          transformer.transformString(str_template, context);

      TestHelper::assertCondition(str_result.find("Building 1001") !=
                                      std::string::npos,
                                  "문자열 템플릿 변환 성공");
      ...
      LogManager::getInstance().Error("PayloadTransformer 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testFailureProtector() {
    ...
    try {
      ...
      TestHelper::assertCondition(protector.canExecute(), "초기 CLOSED 상태");

      protector.recordFailure();
      protector.recordFailure();
      protector.recordFailure();

      TestHelper::assertCondition(!protector.canExecute(), "OPEN 상태 전환");

      LogManager::getInstance().Info("  ⏰ 0.7초 대기...");
      std::this_thread::sleep_for(std::chrono::milliseconds(700));

      TestHelper::assertCondition(protector.canExecute(),
                                  "HALF_OPEN 상태 전환");

      protector.recordSuccess();
      TestHelper::assertCondition(protector.canExecute(), "CLOSED 복구");

      LogManager::getInstance().Info("✅ FailureProtector 완료\n");
      return true;

    } catch (const std::exception &e) {
      LogManager::getInstance().Error("FailureProtector 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testFileTargetHandler() {
    ...
    try {
      ...
      json config = {{"base_path", test_dir},
                     {"file_format", "json"},
                     {"filename_template", "alarm_{{building_id}}.json"},
                     {"create_subdirs", true}};

      TestHelper::assertCondition(handler.initialize(config),
      ...
      TestHelper::assertCondition(result.success && !result.file_path.empty(),
                                  "파일 저장 성공");

      TestHelper::assertCondition(std::filesystem::exists(result.file_path),
                                  "파일 존재 확인");
      ...
      LogManager::getInstance().Error("FileTargetHandler 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testMultipleTargetsConcurrent() {
    ...
    try {
      ...
      LogManager::getInstance().Info("  📤 " + std::to_string(alarm_count) +
                                     "개 알람 발행");
      ...
      LogManager::getInstance().Error("다중 타겟 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testFailureAndRetry() {
    ...
    try {
      ...
      json fail_config = {{"url", "http://localhost:99999/fail"},
                          {"method", "POST"},
                          {"timeout", 1000},
                          {"max_retries", 2}};
      ...
      LogManager::getInstance().Info("  🎯 실패 타겟 생성: ID=" +
                                     std::to_string(fail_id));
      ...
      LogManager::getInstance().Error("실패 시나리오 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  bool testExportLogRepository() {
    ...
    try {
      ...
      LogManager::getInstance().Info("  ✅ 로그 저장: ID=" +
                                     std::to_string(log_id));

      auto retrieved = log_repo.findById(log_id);
      TestHelper::assertCondition(retrieved.has_value(), "로그 조회 성공");

      if (retrieved.has_value()) {
        LogManager::getInstance().Info("  📋 로그 정보:");
        LogManager::getInstance().Info(
            "    타겟 ID: " + std::to_string(retrieved->getTargetId()));
        LogManager::getInstance().Info("    로그 타입: " +
                                       retrieved->getLogType());
        LogManager::getInstance().Info("    상태: " + retrieved->getStatus());
        LogManager::getInstance().Info(
            "    HTTP 코드: " + std::to_string(retrieved->getHttpStatusCode()));
        LogManager::getInstance().Info(
            "    처리시간: " +
            std::to_string(retrieved->getProcessingTimeMs()) + "ms");
      }
      ...
      LogManager::getInstance().Error("ExportLogRepository 실패: " +
                                      std::string(e.what()));
      return false;
    }
  }
