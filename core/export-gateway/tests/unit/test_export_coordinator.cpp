...
#include "Utils/ConfigManager.h" // ConfigManager 추가
#include <atomic>
#include <chrono>
#include <iomanip>
#include <iostream>
...
AlarmMessage createTestAlarm(int building_id = 1001,
                             ...
                             double value = 25.5, int alarm_flag = 1,
                             int status = 0) {
  AlarmMessage alarm;
...
ExportCoordinatorConfig createTestConfig() {
  ...
  // ✅ 테스트 환경 노이즈 제거: ConfigManager를 통해 Redis Host 강제 설정
  // 이렇게 하면 DNS 조회 에러 대신 단순 연결 실패(Connection Refused)가
  // 발생하여 로그가 깔끔해짐
  ConfigManager::getInstance().updateConfig("REDIS_PRIMARY_HOST", "127.0.0.1");
...
void test_batch_alarm_handling() {
  ...
  for (int i = 0; i < 5; i++) {
    AlarmMessage alarm =
        createTestAlarm(1001 + i, "POINT_" + std::to_string(i), 20.0 + i, 1, 0);
...
void test_alarm_transformation() {
  ...
  auto context = transformer.createContext(alarm, "Temperature_Sensor_01",
                                           "1층 온도 센서", "85.5");
  ...
  ASSERT(result.contains("bd"),
         "bd 없음"); // Generic template uses 'bd', not 'building_id'
  ASSERT(result.contains("nm"),
         "nm 없음"); // Generic template uses 'nm', not 'point_name'
...
void test_processing_stats() {
  ...
  for (int i = 0; i < 3; i++) {
    AlarmMessage alarm =
        createTestAlarm(1000 + i, "POINT_" + std::to_string(i), 25.0 + i);
...
void test_redis_subscription_config() {
  ...
  // ✅ 테스트 환경 노이즈 제거를 위해 127.0.0.1로 변경됨
  ASSERT(config.redis_host == "127.0.0.1", "Redis 호스트 불일치");
...
int main() {
  ...
  std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
  if (failed_count == 0) {
    std::cout << "  🎉 결과: " << passed_count << "/" << test_count
              << " passed - PERFECT! 🎉\n";
  } else {
    std::cout << "  ⚠️  결과: " << passed_count << "/" << test_count
              << " passed";
